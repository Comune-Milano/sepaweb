...
Partial Class CAMBI_RisultatoDistinte
    ...
    Protected Sub btnVisualizza_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnVisualizza.Click
        ...
        Else
            ...
            par.cmd.CommandText = "select DOMANDE_BANDO_cambi.ID,DOMANDE_BANDO_cambi.PG,TO_CHAR(TO_DATE(DOMANDE_BANDO_cambi.DATA_PG,'YYYYmmdd'),'DD/MM/YYYY') AS ""DATA_PG1"",DOMANDE_BANDO_cambi.isbarc_r,COMP_NUCLEO_cambi.COGNOME,COMP_NUCLEO_cambi.NOME from DOMANDE_BANDO_cambi,DICHIARAZIONI_cambi,COMP_NUCLEO_cambi where DOMANDE_BANDO_cambi.PROGR_COMPONENTE=COMP_NUCLEO_cambi.PROGR AND COMP_NUCLEO_cambi.ID_DICHIARAZIONE=DOMANDE_BANDO_cambi.ID_DICHIARAZIONE AND DICHIARAZIONI_cambi.ID_CAF=" & Session.Item("ID_CAF") & " AND DOMANDE_BANDO_cambi.ID_DICHIARAZIONE=DICHIARAZIONI_cambi.ID and DOMANDE_BANDO_cambi.N_DISTINTA=" & LBLID.Text & " ORDER BY DOMANDE_BANDO_cambi.ID ASC"
    ...
    Private Sub Cerca()
        ...
        If sValoreDA <> "" Then
            ...
            End If
            bTrovato = True
            sStringaSql = sStringaSql & " DOMANDE_DISTINTE_cambi.DATA >= '" & par.PulisciStrSql(sValore) & "' "
        End If
        ...
        If sValoreDIS <> "" Then
            ...
            End If
            bTrovato = True
            sStringaSql = sStringaSql & " DOMANDE_DISTINTE_cambi.NUMERO " & sCompara & " " & par.PulisciStrSql(sValore) & " "
        End If
        If sValoreOP <> "-1" Then
            If sValoreOP <> "" Then
                If bTrovato = True Then sStringaSql = sStringaSql & " AND "
                ...
                End If
                bTrovato = True
                sStringaSql = sStringaSql & " DOMANDE_DISTINTE_cambi.ID_OPERATORE" & sCompara & " " & par.PulisciStrSql(sValore) & " "
            End If
        End If

        sStringaSQL1 = "SELECT DOMANDE_DISTINTE_cambi.NUMERO,TO_CHAR(TO_DATE(DOMANDE_DISTINTE_cambi.DATA,'YYYYmmdd'),'DD/MM/YYYY')" _
                & " AS  ""DATA_DIS""," _
                & "OPERATORI.OPERATORE AS ""DESCRIZIONE""  " _
                & " FROM DOMANDE_DISTINTE_cambi,OPERATORI WHERE " _
                & "  DOMANDE_DISTINTE_cambi.ID_OPERATORE = OPERATORI.ID "

        If sStringaSql <> "" Then
            sStringaSQL1 = sStringaSQL1 & " AND " & sStringaSql
        End If
        sStringaSQL1 = sStringaSQL1 & " ORDER BY DOMANDE_DISTINTE_cambi.NUMERO DESC"
        BindGrid()
    End Sub
    ...
    Private Sub BindGrid()
        ...
        da.Fill(ds, "DOMANDE_DISTINTE_cambi,OPERATORI_cambi")
