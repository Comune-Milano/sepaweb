...
Partial Class CAMBI_DomandeCarico
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            cmbAnnoDa.SelectedIndex = -1
            cmbAnnoDa.Items.FindByText(Year(Now)).Selected = True

            cmbMDa.SelectedIndex = -1
            cmbMDa.Items.FindByValue(Format(Month(Now), "00")).Selected = True

            cmbGDa.SelectedIndex = -1
            cmbGDa.Items.FindByText(Format(Day(Now), "00")).Selected = True
    ...
    Protected Sub btnVisualizza_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnVisualizza.Click
        ...
        Try
            ...
            If cmbTipo.SelectedValue = "0" Then
                par.cmd.CommandText = "select DOMANDE_BANDO_cambi.ID,DOMANDE_BANDO_cambi.PG,TO_CHAR(TO_DATE(DOMANDE_BANDO_cambi.DATA_PG,'YYYYmmdd'),'DD/MM/YYYY') AS ""DATA_PG1"",DOMANDE_BANDO_cambi.isbarc_r,COMP_NUCLEO_cambi.COGNOME,COMP_NUCLEO_cambi.NOME,DOMANDE_BANDO_cambi.N_DISTINTA from DOMANDE_BANDO_cambi,DICHIARAZIONI_cambi,COMP_NUCLEO_cambi where DOMANDE_BANDO_cambi.PROGR_COMPONENTE=COMP_NUCLEO_cambi.PROGR AND COMP_NUCLEO_cambi.ID_DICHIARAZIONE=DOMANDE_BANDO_cambi.ID_DICHIARAZIONE AND DICHIARAZIONI_cambi.ID_CAF=" & Session.Item("ID_CAF") & " AND DOMANDE_BANDO_cambi.ID_DICHIARAZIONE=DICHIARAZIONI_cambi.ID and DOMANDE_BANDO_cambi.N_DISTINTA=0 AND (DOMANDE_BANDO_cambi.ID_STATO='4' OR DOMANDE_BANDO_cambi.ID_STATO='7a') AND DICHIARAZIONI_cambi.ID_STATO<>2 ORDER BY DOMANDE_BANDO_cambi.ID ASC"
            Else
                par.cmd.CommandText = "select DOMANDE_BANDO_cambi.ID,DOMANDE_BANDO_cambi.PG,TO_CHAR(TO_DATE(DOMANDE_BANDO_cambi.DATA_PG,'YYYYmmdd'),'DD/MM/YYYY') AS ""DATA_PG1"",DOMANDE_BANDO_cambi.isbarc_r,COMP_NUCLEO_cambi.COGNOME,COMP_NUCLEO_cambi.NOME,DOMANDE_BANDO_cambi.N_DISTINTA from DOMANDE_BANDO_cambi,DICHIARAZIONI_cambi,COMP_NUCLEO_cambi where DOMANDE_BANDO_cambi.PROGR_COMPONENTE=COMP_NUCLEO_cambi.PROGR AND COMP_NUCLEO_cambi.ID_DICHIARAZIONE=DOMANDE_BANDO_cambi.ID_DICHIARAZIONE AND DICHIARAZIONI_cambi.ID_CAF=" & Session.Item("ID_CAF") & " AND DOMANDE_BANDO_cambi.ID_DICHIARAZIONE=DICHIARAZIONI_cambi.ID and DOMANDE_BANDO_cambi.N_DISTINTA<>0 AND DICHIARAZIONI_cambi.ID_STATO<>2 ORDER BY DOMANDE_BANDO_cambi.ID ASC"
            End If
            ...
            If cmbTipo.SelectedValue = "0" Then
                sPagina = sPagina & "<p align='center'><b><font face='Arial' size='3'>BANDO CAMBI - Elenco delle Domande in carico dal " & par.FormattaData(INTERVALLO_DA) & " al " & par.FormattaData(INTERVALLO_A) & "</font></b><p>&nbsp;</td>" & vbCrLf
            Else
                sPagina = sPagina & "<p align='center'><b><font face='Arial' size='3'>BANDO CAMBI - Elenco delle Domande scaricate dal " & par.FormattaData(INTERVALLO_DA) & " al " & par.FormattaData(INTERVALLO_A) & "</font></b><p>&nbsp;</td>" & vbCrLf
            End If
            sPagina = sPagina & "</tr>" & vbCrLf
            sPagina = sPagina & "<tr>" & vbCrLf
