...
Partial Class Contratti_SpostamentoRU_RiepilogoAssegnazioneG
    ...
    Private Sub CaricaInfoGiur()
        Try
            ...
            myReader0 = par.cmd.ExecuteReader()
            If myReader0.Read Then
                lblRagSociale.Text = par.IfNull(myReader0("RAGIONE_SOCIALE"), "")
                lblIVA.Text = par.IfNull(myReader0("PARTITA_IVA"), "")
                lblCapGiur.Text = par.IfNull(myReader0("CAP_RESIDENZA"), "")
                lblComGiur.Text = par.IfNull(myReader0("COMUNE_RESIDENZA"), "")
                lblProvGiur.Text = par.IfNull(myReader0("PROVINCIA_RESIDENZA"), "")
                lblViaGiur.Text = par.IfNull(myReader0("INDIRIZZO_RESIDENZA"), "")
                lblCivGiur.Text = par.IfNull(myReader0("CIVICO_RESIDENZA"), "")
                lblTelGiur.Text = par.IfNull(myReader0("TELEFONO"), "")
                ...
                If par.IfNull(myReader0("tipo_r"), "0") = "0" Then
                    lblLegale.Text = "SI"
                    lblProcuratore.Text = "--"
                Else
                    lblProcuratore.Text = "SI"
                    lblLegale.Text = "--"
                End If
    ...
    Private Sub InsertUIAssegn()
        Try
            ...
            If lblRagSociale.Text = "" Then
                cognome = lblCognome.Text
                nome = lblNome.Text
            Else
                cognome = lblRagSociale.Text
                nome = ""
            End If
    ...
    Private Sub ChiudiContratto()
        Try
            ...
            For Each row As Data.DataRow In dt1.Rows
                ...
                & ", '" & par.IfNull(row.Item("PRESSO"), "") & "', '" & Format(Now, "yyyyMMdd") & "', '" & Format(Now, "yyyyMMdd") & "', '" & par.IfNull(row.Item("FL_STAMPATO"), "") & "', " & par.IfNull(row.Item("ID_COMPLESSO"), "") & ", '', " & par.VirgoleInPunti((par.IfNull(row.Item("IMPORTO_PAGATO"), 0) * (-1))) & ", '', " & par.IfNull(row.Item("ANNO"), "") & ", '', " & par.IfNull(row.Item("ID_EDIFICIO"), "") & "" _
