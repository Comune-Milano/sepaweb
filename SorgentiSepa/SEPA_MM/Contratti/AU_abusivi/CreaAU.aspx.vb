...
Partial Class ANAUT_CreaAU
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsPostBack = False Then
            ...
            Select Case IDBANDO.Value
                Case 1
                    lblTitolo.Text = "Inserimento AU 2007 Art.15"
                Case 2
                    lblTitolo.Text = "Inserimento AU 2009 Art.15"
                Case 3
                    lblTitolo.Text = "Inserimento AU 2011 Art.15"
            End Select
            'Else
    ...
    Private Function PrendiUltima()
        Try
            ...
            par.cmd.CommandText = "SELECT UTENZA_DICHIARAZIONI.*,UTENZA_DICHIARAZIONI.ART_15 AS ART15,UTENZA_BANDI.DESCRIZIONE AS NOME_BANDO,UTENZA_BANDI.ANNO_ISEE FROM siscom_mi.rapporti_utenza,UTENZA_DICHIARAZIONI,UTENZA_BANDI WHERE rapporti_utenza.id=" & idc.Value & " and NVL(FL_GENERAZ_AUTO,0)=0 AND (UTENZA_DICHIARAZIONI.NOTE_WEB IS NULL OR UTENZA_DICHIARAZIONI.NOTE_WEB<>'GENERATA_AUTOMATICAMENTE') AND UTENZA_BANDI.ID = UTENZA_DICHIARAZIONI.ID_BANDO " _
            & "AND RAPPORTO=rapporti_utenza.cod_contratto ORDER BY UTENZA_DICHIARAZIONI.DATA_FINE_VAL DESC,UTENZA_DICHIARAZIONI.ID DESC"

            Dim myReader2 As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
            If myReader2.Read Then
                Dim ART15 As String = ""

                If par.IfNull(myReader2("ART15"), "0") = "1" Then
                    ART15 = "-ART. 15"
                End If
                id_dichia1 = myReader2("ID")
                data_Fine1 = par.IfNull(myReader2("DATA_FINE_VAL"), Format(Now, "yyyyMMdd"))
                MESSAGGIO1 = "<br/>La situazione anagrafica e reddituale più recente corrisponde a: ""<u>" & myReader2("NOME_BANDO") & ART15 & "</u>"" (redditi " & myReader2("ANNO_ISEE") & ")."
            End If
            myReader2.Close()
            ...
            Else
                If data_Fine >= data_Fine1 Then
                    ...
                    Label3.Text = "<br/>Premendo il pulsante PROCEDI sarà importata la situazione reddituale e anagrafica nella <b>nuova scheda AU. ART.15</b><br/>"
    ...
    Private Function CreaAU(ByVal idc As String, ByVal t As String)
        Try
            ...
            If myReader.Read Then
                ...
                par.cmd.CommandText = "Insert into UTENZA_DICHIARAZIONI  (ID, PG, DATA_PG, LUOGO, DATA, NOTE, ID_STATO,ANNO_SIT_ECONOMICA,ID_BANDO,DATA_INIZIO_VAL,DATA_FINE_VAL,ART_15) values (seq_utenza_dichiarazioni.nextval, '" & Format(lValoreCorrente, "0000000000") & "','" & Format(Now, "yyyyMMdd") & "','Milano','" & Format(Now, "yyyyMMdd") & "','', 0," & sAnnoIsee & "," & lIndice_Bando & ",'" & CInt(sAnnoIsee) + 2 & "0101','" & CInt(sAnnoIsee) + 3 & "1231',1)"
                par.cmd.ExecuteNonQuery()
            ...
            strScript = "<script language='javascript'>var conf = window.confirm('Operazione effettuata con successo. Cliccare su OK per visualizzare la dichiarazione.');if (conf){window.open('../../ANAUT/DichAUnuova.aspx?ID=" & lIdDichiarazioneAU & "&CHIUDI=1&CH=1&CR=1&ANNI=" & anni & "','AU1','');self.close();}" _
    ...
    Private Function CreaAUAU(ByVal t As String)
        ...
        Try
                ...
                If anomaliaReddAU = False Then
                    ...
                    While myReader3.Read
                        par.cmd.CommandText = "INSERT INTO UTENZA_COMP_PATR_MOB (ID,ID_COMPONENTE,COD_INTERMEDIARIO,INTERMEDIARIO,IMPORTO,IBAN,ID_TIPO) " _
                            ...
                            & "'" & par.PulisciStrSql(par.IfNull(myReader3("INTERMEDIARIO"), "")) & "'," & par.VirgoleInPunti(par.IfNull(myReader3("IMPORTO"), "")) & ",'" & par.IfNull(myReader3("IBAN"), "") & "'," & par.IfNull(myReader3("ID_TIPO"), "NULL") & ")"
                        par.cmd.ExecuteNonQuery()
                    End While
                    myReader3.Close()
                    ...
                    While myReader4.Read
                        par.cmd.CommandText = "INSERT INTO UTENZA_COMP_REDDITO (ID,ID_COMPONENTE,REDDITO_IRPEF,PROV_AGRARI) " _
                            & "VALUES (SEQ_UTENZA_COMP_REDDITO.NEXTVAL," & new_id_compon & "," & par.VirgoleInPunti(par.IfNull(myReader4("REDDITO_IRPEF"), "")) & "," & par.VirgoleInPunti(par.IfNull(myReader4("PROV_AGRARI"), 0)) & ")"
                        par.cmd.ExecuteNonQuery()
                        ...
                        While myReader5.Read
                            ...
                            par.cmd.CommandText = "UPDATE UTENZA_COMP_REDDITO SET REDDITO_IRPEF=" & par.VirgoleInPunti(par.IfNull(myReader4("REDDITO_IRPEF"), "") + reddAltro) & " WHERE ID=" & idReddComp
                            par.cmd.ExecuteNonQuery()
                        End While
                        myReader5.Close()
                    End While
                    ...
                    While myReader6.Read
                        ...
                        & "VALUES (SEQ_UTENZA_COMP_ELENCO_SPESE.NEXTVAL," & new_id_compon & "," & par.VirgoleInPunti(par.IfNull(myReader6("IMPORTO"), "")) & ",'" & par.IfNull(myReader6("DESCRIZIONE"), "") & "')"
                        par.cmd.ExecuteNonQuery()
                    End While
                    myReader6.Close()
                    ...
                    While myReader7.Read
                        ...
                        & "VALUES (SEQ_UTENZA_COMP_DETRAZIONI.NEXTVAL," & new_id_compon & "," & par.IfNull(myReader7("ID_TIPO"), "") & "," & par.VirgoleInPunti(par.IfNull(myReader7("IMPORTO"), "0")) & ")"
                        par.cmd.ExecuteNonQuery()
                    End While
                    myReader7.Close()
            ...
            strScript = "<script language='javascript'>var conf = window.confirm('Operazione effettuata con successo. Cliccare su OK per visualizzare la dichiarazione.');if (conf){window.open('../../ANAUT/DichAUnuova.aspx?ID=" & lIdDichiarazioneAU & "&CHIUDI=1&CH=1&CR=1&ANNI=" & anni & "','AU2','');self.close();}" _
             & "else{window.close();}</script>"
            Response.Write(strScript)
    ...
    Private Function CreaAUVSA(ByVal t As String)
        ...
        Try
            ...
            strScript = "<script language='javascript'>var conf = window.confirm('Operazione effettuata con successo. Cliccare su OK per visualizzare la dichiarazione.');if (conf){window.open('../../ANAUT/DichAUnuova.aspx?ID=" & lIdDichiarazioneAU & "&CHIUDI=1&CH=1&CR=1&ANNI=" & anni & "','AU3','');self.close();}" _
             & "else{window.close();}</script>"
            Response.Write(strScript)
