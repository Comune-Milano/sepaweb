...
Partial Class Contratti_DepositoCauzionaleUSD
    ...
    Private Function CaricaDatiRimborso()
        Try
            ...
            par.cmd.CommandText = " SELECT SOGGETTI_CONTRATTUALI.ID_ANAGRAFICA AS ID," _
                ...
                & " ,SISCOM_MI.GETDATA(DATA_FINE) AS DATA_FINE " _
                ...
                & " AND ID_ANAGRAFICA=" & Request.QueryString("IDI")

            Dim Lettore As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader
            If Lettore.Read Then
                RagioneSociale.Text = par.IfNull(Lettore("RAGIONE_SOCIALE"), "")
                Cognome.Text = par.IfNull(Lettore("COGNOME"), "")
                Nome.Text = par.IfNull(Lettore("NOME"), "")
                DAtaInizio.Text = par.IfNull(Lettore("DATA_INIZIO"), "")
                DataFine.Text = par.IfNull(Lettore("DATA_FINE"), "")
            End If
            Lettore.Close()

            par.cmd.CommandText = "select  RAPPORTI_UTENZA_DEP_CAUZ.*,nvl(bol_bollette.id_bolletta_storno,0) AS id_bolletta_storno from SISCOM_MI.RAPPORTI_UTENZA_DEP_CAUZ,SISCOM_MI.BOL_BOLLETTE WHERE RAPPORTI_UTENZA_DEP_CAUZ.ID_CONTRATTO=" & Request.QueryString("IDC") & " AND ID_BOLLETTA=BOL_BOLLETTE.ID AND COD_AFFITTUARIO=" & Request.QueryString("IDI")
    ...
    Private Sub CaricaInfoInteressi()
        Try
            ...
            '    & "AND adeguamento_interessi_voci.id_adeguamento = adeguamento_interessi.ID AND id_contratto=" & Request.QueryString("IDC") & " AND id_ANAGRAFICA=" & Request.QueryString("IDI") & " order by adeguamento_interessi_voci.dal asc"
            ...
            'End If
            'myReaderInt.Close()
            idAnagrafe = Request.QueryString("IDI")
                ...
                For Each row As Data.DataRow In dt.Rows
                    ...
                    If importoInteresse <> 0 Then
                        par.cmd.CommandText = "select bol_bollette_voci.*,bol_bollette.NUM_BOLLETTA,BOL_BOLLETTE.ID_BOLLETTA_STORNO " _
                            ...
                            & " AND COD_AFFITTUARIO=" & Request.QueryString("IDI") & " and bol_bollette.id=bol_bollette_voci.id_bolletta and bol_bollette_voci.id_voce=15 " _
                        ...
                        Else
                            par.cmd.CommandText = "select bol_bollette_voci.*,bol_bollette.NUM_BOLLETTA,BOL_BOLLETTE.ID_BOLLETTA_STORNO from siscom_mi.bol_bollette,siscom_mi.bol_bollette_voci where id_contratto=" & Request.QueryString("IDC") & " AND COD_AFFITTUARIO=" & Request.QueryString("IDI") & " and bol_bollette.id=bol_bollette_voci.id_bolletta and bol_bollette.id_tipo=3 and bol_bollette_voci.id_voce in (15, 350,351,352,353,354,355,356) /*and bol_bollette_voci.importo=" & par.VirgoleInPunti(importoInteresse) & "*/"
            ...
            par.cmd.CommandText = "SELECT sum(adeguamento_interessi_voci.importo) as totimporto FROM siscom_mi.rapporti_utenza,siscom_mi.adeguamento_interessi,siscom_mi.adeguamento_interessi_voci WHERE rapporti_utenza.ID = adeguamento_interessi.id_contratto " _
                & "AND adeguamento_interessi_voci.id_adeguamento = adeguamento_interessi.ID AND id_contratto=" & Request.QueryString("IDC") & " AND id_ANAGRAFICA=" & Request.QueryString("IDI")
            ...
            par.cmd.CommandText = "SELECT * FROM siscom_mi.adeguamento_interessi where id_contratto=" & Request.QueryString("IDC") & " AND id_ANAGRAFICA=" & Request.QueryString("IDI") & " order by data desc"
            Dim lettore As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
            If lettore.Read Then
            ...
            par.cmd.CommandText = "SELECT * FROM siscom_mi.bol_bollette where id_contratto=" & Request.QueryString("IDC") & " AND COD_AFFITTUARIO=" & Request.QueryString("IDI") & " AND ID_TIPO=9 and nvl(data_scadenza,'29991231')<>'29991231' AND ID_BOLLETTA_STORNO IS NULL AND (BOL_BOLLETTE.FL_ANNULLATA=0 OR (BOL_BOLLETTE.FL_ANNULLATA<>0 AND DATA_PAGAMENTO IS NOT NULL )) order BY ID DESC"
            ...
            par.cmd.CommandText = "SELECT BOL_BOLLETTE.id_bolletta_storno,STORICO_dEP_CAUZIONALE.*,siscom_mi.getdata(nvl(STORICO_dEP_CAUZIONALE.data_costituzione,STORICO_dEP_CAUZIONALE.data)) as data_di_costituzione FROM SISCOM_MI.STORICO_dEP_CAUZIONALE,SISCOM_MI.BOL_BOLLETTE WHERE BOL_BOLLETTE.ID=STORICO_dEP_CAUZIONALE.ID_BOLLETTA AND STORICO_dEP_CAUZIONALE.ID_CONTRATTO=" & Request.QueryString("IDC") & " AND STORICO_dEP_CAUZIONALE.ID_ANAGRAFICA=" & Request.QueryString("IDI") & " ORDER BY STORICO_dEP_CAUZIONALE.ID DESC"
            lettore = par.cmd.ExecuteReader()
            If lettore.Read Then
            ...
            par.cmd.CommandText = "SELECT siscom_mi.getdata(nvl(STORICO_dEP_CAUZIONALE.data_costituzione,STORICO_dEP_CAUZIONALE.data)) as data_di_costituzione FROM SISCOM_MI.STORICO_dEP_CAUZIONALE " _
                & " WHERE STORICO_dEP_CAUZIONALE.ID_CONTRATTO=" & Request.QueryString("IDC") & " AND STORICO_dEP_CAUZIONALE.ID_ANAGRAFICA=" & Request.QueryString("IDI")
            lettore = par.cmd.ExecuteReader()
            If lettore.Read Then
                txtDataCostituzione.Text = par.FormattaData(par.IfNull(lettore("DATA_DI_COSTITUZIONE"), ""))
            End If
            lettore.Close()
    ...
    Private Sub CaricaStoricoDep()
        Try
            ...
            par.cmd.CommandText = "SELECT STORICO_DEP_CAUZIONALE.*,RAPPORTI_UTENZA_DEP_PROV.LIBRO,RAPPORTI_UTENZA_DEP_PROV.BOLLA,siscom_mi.TAB_PROVENIENZA_DEP.DESCRIZIONE AS PROVENIENZA,siscom_mi.getdata(nvl(STORICO_dEP_CAUZIONALE.data_costituzione,STORICO_dEP_CAUZIONALE.data)) as data_di_costituzione FROM siscom_mi.TAB_PROVENIENZA_DEP,siscom_mi.STORICO_DEP_CAUZIONALE,SISCOM_MI.RAPPORTI_UTENZA_DEP_PROV WHERE siscom_mi.TAB_PROVENIENZA_DEP.ID (+)=RAPPORTI_UTENZA_DEP_PROV.PROVENIENZA AND RAPPORTI_UTENZA_DEP_PROV.ID_STORICO_DEP(+)=STORICO_DEP_CAUZIONALE.ID AND STORICO_DEP_CAUZIONALE.ID_CONTRATTO=" & Request.QueryString("IDC") & " AND STORICO_DEP_CAUZIONALE.ID_ANAGRAFICA=" & Request.QueryString("IDI") & " ORDER BY DATA DESC,ID DESC"
            ...
            If dt0.Rows.Count > 0 Then
                For Each row As Data.DataRow In dt0.Rows
                    ...
                    Else
                        ...
                        End If
                    End If
                    If RIGA.Item("DATA_PAGAMENTO") = "" Then
                        RIGA.Item("DATA_PAGAMENTO") = par.FormattaData(par.IfNull(row.Item("DATA_DI_COSTITUZIONE"), ""))
                    End If
                        myReader.Close()
