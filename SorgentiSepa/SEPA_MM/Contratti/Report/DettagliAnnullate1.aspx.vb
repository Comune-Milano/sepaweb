    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            Try
                ...
                DESCRIZIONE_T = "BOLLETTE ANNULLATE MA CHE RISULTANO INCASSATE.</br>"
                ...
                Do While myReaderAa.Read
                    ...
                    PAR.cmd.CommandText = "SELECT BOL_BOLLETTE.*,(select sum(importo) from siscom_mi.bol_bollette_voci where id_bolletta=bol_bollette.iD) as IMPORTO,RAPPORTI_UTENZA.COD_CONTRATTO FROM SISCOM_MI.RAPPORTI_UTENZA,siscom_mi.BOL_BOLLETTE WHERE BOL_BOLLETTE.ID_CONTRATTO=RAPPORTI_UTENZA.ID AND  BOL_BOLLETTE.ID=" & myReaderAa("ID")
                    Dim myReaderAB As Oracle.DataAccess.Client.OracleDataReader = PAR.cmd.ExecuteReader()
                    If myReaderAB.Read Then
                        Response.Write("<tr style='font-family: ARIAL; font-size: 9px; font-weight: bold;border-bottom-style: dashed; border-bottom-width: thin'><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & I & "</td><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & Format(PAR.IfNull(myReaderAB("ID"), "0"), "0000000000") & "</td><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & RATA(PAR.IfNull(myReaderAB("N_RATA"), "")) & "/" & PAR.IfNull(myReaderAB("ANNO"), "") & "</td><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & RATA(PAR.IfNull(myReaderAB("COD_CONTRATTO"), "")) & "</td><td style='border-bottom-style: dashed; border-bottom-width: thin'><a href=" & Chr(34) & "..\..\Contabilita\DatiUtenza.aspx?C=RisUtenza&IDCONT=" & PAR.IfNull(myReaderAB("ID_CONTRATTO"), "0") & "&IDANA=" & PAR.IfNull(myReaderAB("cod_affittuario"), "-1") & Chr(34) & " target=" & Chr(34) & "_blank" & Chr(34) & ">" & PAR.IfNull(myReaderAB("INTESTATARIO"), "") & "</a></td><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & PAR.FormattaData(PAR.IfNull(myReaderAB("DATA_EMISSIONE"), "")) & "</td><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & PAR.FormattaData(PAR.IfNull(myReaderAB("DATA_PAGAMENTO"), "")) & "</td><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & PAR.FormattaData(PAR.IfNull(myReaderAB("RIFERIMENTO_DA"), "")) & "--" & PAR.FormattaData(PAR.IfNull(myReaderAB("RIFERIMENTO_A"), "")) & "</td><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & PAR.FormattaData(PAR.IfNull(myReaderAB("DATA_SCADENZA"), "")) & "</td><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & PAR.IfNull(myReaderAB("INDIRIZZO"), "") & " " & PAR.IfNull(myReaderAB("CAP_CITTA"), "") & "</td><td style='border-bottom-style: dashed; border-bottom-width: thin'>" & PAR.IfNull(myReaderAB("NOTE"), "") & "</td><td style='text-align: right;border-bottom-style: dashed; border-bottom-width: thin'>" & Format(PAR.IfNull(myReaderAB("IMPORTO"), "0"), "0.00") & "</td></tr>")
                        RIGA = DT.NewRow()

                        RIGA.Item("N.BOLLETTA") = Format(PAR.IfNull(myReaderAB("ID"), "0"), "0000000000")
                        RIGA.Item("RATA") = RATA(PAR.IfNull(myReaderAB("N_RATA"), "")) & "/" & PAR.IfNull(myReaderAB("ANNO"), "")
                        RIGA.Item("INTESTATARIO") = PAR.IfNull(myReaderAB("INTESTATARIO"), "")
                        RIGA.Item("EMESSA") = PAR.FormattaData(PAR.IfNull(myReaderAB("DATA_EMISSIONE"), ""))
                        RIGA.Item("PAGATA") = PAR.FormattaData(PAR.IfNull(myReaderAB("DATA_PAGAMENTO"), ""))
                        RIGA.Item("PERIODO") = PAR.FormattaData(PAR.IfNull(myReaderAB("RIFERIMENTO_DA"), "")) & "--" & PAR.FormattaData(PAR.IfNull(myReaderAB("RIFERIMENTO_A"), ""))
                        RIGA.Item("SCADENZA") = PAR.FormattaData(PAR.IfNull(myReaderAB("DATA_SCADENZA"), ""))
                        RIGA.Item("INDIRIZZO") = PAR.IfNull(myReaderAB("INDIRIZZO"), "") & " " & PAR.IfNull(myReaderAB("CAP_CITTA"), "")
                        RIGA.Item("NOTE") = PAR.IfNull(myReaderAB("NOTE"), "")
                        RIGA.Item("IMPORTO") = Format(PAR.IfNull(myReaderAB("IMPORTO"), "0"), "0.00")
                        RIGA.Item("COD.CONTRATTO") = PAR.IfNull(myReaderAB("COD_CONTRATTO"), "")
                        DT.Rows.Add(RIGA)


                        TOTALE = TOTALE + PAR.IfNull(myReaderAB("IMPORTO"), 0)
                        I = I + 1
                    End If
                    myReaderAB.Close()
