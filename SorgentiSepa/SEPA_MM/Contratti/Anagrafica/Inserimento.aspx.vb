...
Partial Class Contratti_Anagrafica_Inserimento
        ...
        Try
            If Not IsPostBack Then
                ...
                Else
                    Ricerca = "False"
                End If
                idau.Value = Request.QueryString("ID")
                caricaattributi()
            End If
            ...
            Else
                ...
                If Session.Item("FL_ANAGRAFICA") <> "1" Then
                    ...
                    btnInserisciComponente.Visible = False
                Else
                    Me.txtCognome.Enabled = True
                    Me.txtNome.Enabled = True
                    Me.txtRagione.Enabled = True
                    Me.txtCF.Enabled = True
                    Me.txtPIva.Enabled = True
                    cmbCittadinanza.Enabled = True
                    Me.txtDataNascita.Enabled = True
                    CmbComune.Enabled = True
                    cmbSesso.Enabled = True
                End If
    ...
    Private Function SalvaDati()
        If txtCF.Text <> "" Then
            ...
            If par.RicavaSesso(UCase(txtCF.Text)) <> cmbSesso.SelectedItem.Text Then
                lblErrore.Visible = True
            ...
            If Val(Mid(UCase(txtCF.Text), 10, 2)) > 40 Then
                miaData = Format(Val(Mid(txtCF.Text, 10, 2)) - 40, "00")
            Else
                miaData = Mid(txtCF.Text, 10, 2)
            End If

            Select Case Mid(UCase(txtCF.Text), 9, 1)
                Case "A"
                    miaData = miaData & "/01"
                Case "B"
                    miaData = miaData & "/02"
        ...
        Try
            ...
            If (par.IfEmpty(Me.txtNome.Text, "Null") <> "Null" AndAlso par.IfEmpty(Me.txtCognome.Text, "Null") <> "Null" AndAlso par.IfEmpty(Me.txtCF.Text, "Null") <> "Null") Or (par.IfEmpty(Me.txtRagione.Text, "Null") <> "Null" AndAlso par.IfEmpty(Me.txtPIva.Text, "Null") <> "Null") Then
                ...
                If txtCF.Text <> "" And txtPIva.Text <> "" Then
                    par.cmd.CommandText = "SELECT * FROM SISCOM_MI.ANAGRAFICA WHERE COD_FISCALE='" & UCase(txtCF.Text) & "' AND PARTITA_IVA='" & txtPIva.Text & "'"
                    Dim myReader As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                    If myReader.HasRows = True Then
                        If nuovocomp.Value = "1" Then
                            If myReader.Read Then
                                IdSoggetto = myReader("id")
                                idau.Value = IdSoggetto
                            End If
                        Else
                            lblErrore.Visible = True
                ...
                If txtCF.Text <> "" And txtPIva.Text = "" Then
                    par.cmd.CommandText = "SELECT * FROM SISCOM_MI.ANAGRAFICA WHERE COD_FISCALE='" & UCase(txtCF.Text) & "' AND (PARTITA_IVA='' OR PARTITA_IVA IS NULL)"
                    Dim myReader As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                    If myReader.HasRows = True Then
                        If nuovocomp.Value = "1" Then
                            If myReader.Read Then
                ...
                If IdSoggetto = "" Then
                    ...
                    & "VALUES(SISCOM_MI.SEQ_ANAGRAFICA.NEXTVAL,'" & par.PulisciStrSql(LTrim(RTrim(UCase(Me.txtNome.Text)))) & "','" & par.PulisciStrSql(LTrim(RTrim(UCase(Me.txtCognome.Text)))) & "', '" & par.PulisciStrSql(UCase(Me.txtCF.Text)) & "', '" & par.PulisciStrSql(LTrim(RTrim(Me.txtPIva.Text))) & "'" _
        ...
        Try
            ...
            If par.OracleConn.State = Data.ConnectionState.Closed Then
                ...
                If (par.IfEmpty(Me.txtNome.Text, "Null") <> "Null" AndAlso par.IfEmpty(Me.txtCognome.Text, "Null") <> "Null" AndAlso par.IfEmpty(Me.txtCF.Text, "Null") <> "Null") Or (par.IfEmpty(Me.txtRagione.Text, "Null") <> "Null" AndAlso par.IfEmpty(Me.txtPIva.Text, "Null") <> "Null") Then
                    ...
                    Me.lblErrore.Visible = False
                    par.cmd.CommandText = "UPDATE SISCOM_MI.ANAGRAFICA SET NOME='" & par.PulisciStrSql(LTrim(RTrim(UCase(Me.txtNome.Text)))) & "', COGNOME='" & par.PulisciStrSql(LTrim(RTrim(UCase(Me.txtCognome.Text)))) & "', COD_FISCALE='" & par.PulisciStrSql(UCase(Me.txtCF.Text)) & "'" _
                    & ", PARTITA_IVA='" & par.PulisciStrSql(Me.txtPIva.Text) & "', RAGIONE_SOCIALE='" & par.PulisciStrSql(LTrim(RTrim(UCase(Me.txtRagione.Text)))) & "',CITTADINANZA='" & par.PulisciStrSql(Me.cmbCittadinanza.SelectedItem.Text) & "'" _
    ...
    Protected Sub btnInserisciComponente_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnInserisciComponente.Click
        If nuovocomp.Value = "1" Then
            SalvaDati()
        End If
        If IdSoggetto <> "" Then
            Session.Add("IDANA", IdSoggetto)
            Response.Write("<script>window.close();</script>")
        End If
    ...
    Private Sub CaricaAttributi()
        txtCognome.Attributes.Add("NOME", "COGNOME")
        txtNome.Attributes.Add("NOME", "NOME")
        txtCF.Attributes.Add("NOME", "CODICE FISCALE")
        cmbSesso.Attributes.Add("NOME", "SESSO")
        cmbCittadinanza.Attributes.Add("NOME", "CITTADINANZA")
        CmbComune.Attributes.Add("NOME", "COMUNE")
        txtRagione.Attributes.Add("NOME", "RAGIONE SOCIALE")
        txtPIva.Attributes.Add("NOME", "PARTITA IVA")
        txtDataNascita.Attributes.Add("NOME", "DATA DI NASCITA")

        txtCognome.Attributes.Add("CORRENTE", txtCognome.Text)
        txtNome.Attributes.Add("CORRENTE", txtNome.Text)
        txtCF.Attributes.Add("CORRENTE", txtCF.Text)
        cmbSesso.Attributes.Add("CORRENTE", cmbSesso.SelectedItem.Text)
        cmbCittadinanza.Attributes.Add("CORRENTE", cmbCittadinanza.SelectedItem.Text)
        CmbComune.Attributes.Add("CORRENTE", CmbComune.SelectedItem.Text)
        txtRagione.Attributes.Add("CORRENTE", txtRagione.Text)
        txtPIva.Attributes.Add("CORRENTE", txtPIva.Text)
        txtDataNascita.Attributes.Add("CORRENTE", txtDataNascita.Text)
    End Sub
    ...
    Private Function MemorizzaAttributi() As Boolean
        ...
        Try
            ...
            If txtCognome.Attributes("CORRENTE").ToString.ToUpper <> txtCognome.Text.ToUpper Then
                If ScriviLogOp(txtCognome.Attributes("NOME").ToUpper.ToString, txtCognome.Attributes("CORRENTE").ToUpper.ToString, txtCognome.Text.ToUpper) = False Then

                End If
            End If

            If txtNome.Attributes("CORRENTE").ToString.ToUpper <> txtNome.Text.ToUpper Then
                If ScriviLogOp(txtNome.Attributes("NOME").ToUpper.ToString, txtNome.Attributes("CORRENTE").ToUpper.ToString, txtNome.Text.ToUpper) = False Then

                End If
            End If

            If txtCF.Attributes("CORRENTE").ToString.ToUpper <> txtCF.Text.ToUpper Then
                If ScriviLogOp(txtCF.Attributes("NOME").ToUpper.ToString, txtCF.Attributes("CORRENTE").ToUpper.ToString, txtCF.Text.ToUpper) = False Then

                End If
            End If

            If txtDataNascita.Attributes("CORRENTE").ToString.ToUpper <> txtDataNascita.Text.ToUpper Then
                If ScriviLogOp(txtDataNascita.Attributes("NOME").ToUpper.ToString, txtDataNascita.Attributes("CORRENTE").ToUpper.ToString, txtDataNascita.Text.ToUpper) = False Then

                End If
            End If

            If txtRagione.Attributes("CORRENTE").ToString.ToUpper <> txtRagione.Text.ToUpper Then
                If ScriviLogOp(txtRagione.Attributes("NOME").ToUpper.ToString, txtRagione.Attributes("CORRENTE").ToUpper.ToString, txtRagione.Text.ToUpper) = False Then

                End If
            End If

            If txtPIva.Attributes("CORRENTE").ToString.ToUpper <> txtPIva.Text.ToUpper Then
                If ScriviLogOp(txtPIva.Attributes("NOME").ToUpper.ToString, txtPIva.Attributes("CORRENTE").ToUpper.ToString, txtPIva.Text.ToUpper) = False Then

                End If
            End If

            If cmbSesso.Attributes("CORRENTE").ToString.ToUpper <> UCase(cmbSesso.SelectedItem.Text) Then
                If ScriviLogOp(cmbSesso.Attributes("NOME").ToUpper.ToString, cmbSesso.Attributes("CORRENTE").ToUpper.ToString, cmbSesso.SelectedItem.Text) = False Then

                End If
            End If

            If cmbCittadinanza.Attributes("CORRENTE").ToString.ToUpper <> UCase(cmbCittadinanza.SelectedItem.Text) Then
                If ScriviLogOp(cmbCittadinanza.Attributes("NOME").ToUpper.ToString, cmbCittadinanza.Attributes("CORRENTE").ToUpper.ToString, cmbCittadinanza.SelectedItem.Text) = False Then

                End If
            End If

            If CmbComune.Attributes("CORRENTE").ToString.ToUpper <> UCase(CmbComune.SelectedItem.Text) Then
                If ScriviLogOp(CmbComune.Attributes("NOME").ToUpper.ToString, CmbComune.Attributes("CORRENTE").ToUpper.ToString, CmbComune.SelectedItem.Text) = False Then
    ...
    Private Function ScriviLogOp(ByVal CAMPO As String, ByVal VAL_PRECEDENTE As String, ByVal VAL_IMPOSTATO As String) As Boolean
        Try
            ...
            par.cmd.CommandText = "INSERT INTO SISCOM_MI.EVENTI_ANAGRAFICA (ID_ANAGRAFICA,ID_OPERATORE,DATA_ORA,COD_EVENTO,MOTIVAZIONE) " _
                    & "VALUES (" & IdSoggetto & "," & Session.Item("ID_OPERATORE") & ",'" & Format(Now, "yyyyMMddHHmmss") & "'," _
                    & "'F02','" & CAMPO & " - PRECEDENTE: " & VAL_PRECEDENTE & " CORRENTE:" & VAL_IMPOSTATO & "')"
            par.cmd.ExecuteNonQuery()
