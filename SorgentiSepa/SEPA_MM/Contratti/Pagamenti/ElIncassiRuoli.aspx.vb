...
Partial Class Contratti_Pagamenti_ElIncassiRuoli
    ...
    Private Sub CaricaIncassi()
        Try
            ...
            connData.apri(False)
            par.cmd.CommandText = "SELECT   INCASSI_RUOLI.ID, id_operatore, Getdataora (data_ora) AS data_ora, " _
                & " OPERATORI.operatore, " _
                & " TIPO_PAG_RUOLO.descrizione|| (CASE WHEN NVL (TIPO_PAG_RUOLO.ID, 0) = 5 THEN ' num. ' || numero_assegno ELSE '' END) AS tipo_pagamento, " _
                ...
                & " FL_ANNULLATA " _
                & " FROM SISCOM_MI.INCASSI_RUOLI, OPERATORI, SISCOM_MI.TIPO_PAG_RUOLO " _
                & " WHERE INCASSI_RUOLI.id_operatore = OPERATORI.ID " _
                & " AND TIPO_PAG_RUOLO.ID = INCASSI_RUOLI.id_tipo_pag " _
                & " AND INCASSI_RUOLI.id_contratto =" & idContratto.Value _
            ...
            Me.lblTitolo.Text = "Elenco Incassi Ruoli Registrati"
    ...
    Private Function EliminaPagamento() As Boolean
        ...
        par.cmd.CommandText = "update SISCOM_MI.INCASSI_RUOLI set fl_annullata = 1 where id = " & idSelected.Value
        par.cmd.ExecuteNonQuery()
        par.cmd.CommandText = "select data_pagamento from SISCOM_MI.INCASSI_RUOLI where id = " & idSelected.Value
        Dim dataPagIncasso As String = par.cmd.ExecuteScalar
        par.cmd.CommandText = "SELECT id_bolletta,sum(nvl(importo_pagato,0)) as importo_pagato,ID_TIPO_PAGAMENTO FROM SISCOM_MI.BOL_BOLLETTE_PAGAMENTI_RUOLO WHERE ID_INCASSO_RUOLO = " & idSelected.Value & " group by id_bolletta,ID_TIPO_PAGAMENTO"
        Dim da As New Oracle.DataAccess.Client.OracleDataAdapter(par.cmd)
        Dim dt As New Data.DataTable()
        da.Fill(dt)
