...
Partial Class Contratti_Pagamenti_IncassiIngiunzioni
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If contrLocked.Value = 1 Then
                FrmSoloLettura()
                If Not IsNothing(HttpContext.Current.Session.Item("PGMANUALEING" & vIdConnessione.Value)) Then
                    CType(HttpContext.Current.Session.Item("PGMANUALEING" & vIdConnessione.Value), CM.datiConnessione).chiudi(False)
                    HttpContext.Current.Session.Remove("PGMANUALEING" & vIdConnessione.Value)
                End If
            End If
        Else
    ...
    Private Sub CaricaInfo()
        Try
            ...
            HttpContext.Current.Session.Add("PGMANUALEING" & vIdConnessione.Value, connData)
    ...
    Private Sub CaricaBollNonPagate()
        Try
            ...
            par.cmd.CommandText = "select count(bol_bollette_voci.id) from siscom_mi.bol_bollette_voci,siscom_mi.bol_bollette " _
                                & " where id_bolletta = bol_bollette.id and FL_ANNULLATA = '0' " _
                                & " AND NVL(IMPORTO_INGIUNZIONE,0) > 0 " _
                                & " AND round(nvl(IMP_INGIUNZIONE_PAG,0) ,2) < round(IMPORTO_INGIUNZIONE ,2) " _
            ...
            par.cmd.CommandText = "SELECT  BOL_BOLLETTE.ID AS ID_BOLLETTA," _
                                ...
                                & "WHERE TIPO_BOLL_INGIUNZIONE.ID(+)=ID_TIPO_INGIUNZIONE And FL_ANNULLATA = '0' AND NVL(IMPORTO_INGIUNZIONE,0) > 0 " _
            ...
            Me.lblTotIngiunto.Text = "€." & Format(CDec(totPagabile.Value), "##,##0.00")
            Me.lblPagatoIngiunto.Text = "€." & Format(CDec(totPagato.Value), "##,##0.00")
            Me.lblIngiuntoDaPag.Text = "€." & Format(CDec(totResiduo.Value), "##,##0.00")
    ...
    Private Function scriviIncasso(ByRef errore As String) As Boolean
        ...
        If idIncasso.Value = 0 Then
            par.cmd.CommandText = "select SISCOM_MI.SEQ_INCASSI_INGIUNZIONE.nextval from dual"
            idIncasso.Value = par.cmd.ExecuteScalar

            par.cmd.CommandText = "insert into siscom_mi.INCASSI_INGIUNZIONE (id,id_tipo_pag,motivo_pagamento," _
                                & "id_contratto,data_pagamento,data_registrazione,fl_annullata,importo," _
                                & "id_operatore,fl_annullabile,data_ora) values " _
                                ...
                                & Session.Item("ID_OPERATORE") & ",1,'" & Format(Now, "yyyyMMddHHmmss") & "')"

            par.cmd.ExecuteNonQuery()
        Else
            Me.impWritePagamento.Value = CDec(Me.txtImpPagamento.Text.Replace(".", "") - PagInMorosita.Value)

            par.cmd.CommandText = "UPDATE siscom_mi.INCASSI_INGIUNZIONE SET IMPORTO = " & par.VirgoleInPunti(CDec(Me.txtImpPagamento.Text.Replace(".", ""))) & ", DATA_PAGAMENTO = '" & par.AggiustaData(Me.txtDataPagamento.Text) & "' , DATA_REGISTRAZIONE = '" & par.AggiustaData(Me.txtDataRegistrazione.Text) & "', " _
                                & "ID_TIPO_PAG =" & Me.cmbTipoPagamento.SelectedValue & " ,MOTIVO_PAGAMENTO = '" & par.PulisciStrSql(Me.txtnote.Text) & "', " _
                                & "WHERE ID = " & idIncasso.Value
            par.cmd.ExecuteNonQuery()

            par.cmd.CommandText = "UPDATE siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING SET DATA_PAGAMENTO = '" & par.AggiustaData(Me.txtDataPagamento.Text) & "', DATA_REGISTRAZIONE =  '" & par.AggiustaData(Me.txtDataRegistrazione.Text) & "', DATA_VALUTA = '" & par.AggiustaData(Me.txtDataRegistrazione.Text) & "' WHERE ID_INCASSO_ING  = " & idIncasso.Value
            par.cmd.ExecuteNonQuery()
        End If
