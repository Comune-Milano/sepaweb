...
Partial Class Contratti_Pagamenti_RptPgIngDett
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            Response.Flush()
            Me.lblTitolo.Text = "Report bollette INGIUNTE"
            Cerca()
        End If
    End Sub
    Private Sub Cerca()
        Try
            ...
            '    & " (SELECT BOL_BOLLETTE.ID FROM BOL_BOLLETTE_PAGAMENTI_ING" _
            '    & "    WHERE(BOL_BOLLETTE_PAGAMENTI_ING.ID_BOLLETTA = BOL_BOLLETTE.ID)" _
            '    & " AND EXISTS (SELECT INCASSI_INGIUNZIONE.ID FROM INCASSI_INGIUNZIONE WHERE INCASSI_INGIUNZIONE.ID=BOL_BOLLETTE_PAGAMENTI_ING.ID_INCASSO_ING)" _
            '    & "  )"
            'End If
            ...
            If par.IfEmpty(Request.QueryString("S"), "-1") <> "1" Then
                If par.IfEmpty(Request.QueryString("DPAG"), "Null") <> "Null" Then
                    ...
                    'sWhere = sWhere & " AND siscom_mi.INCASSI_INGIUNZIONE.data_pagamento >= '" & sValore & "' "
                End If
                ...
                End If
                If dPag <> "" Then
                    stringDate = "siscom_mi.INCASSI_INGIUNZIONE.data_pagamento >= '" & dPag & "' and "
                End If
                If dPagAl <> "" Then
                    stringDate &= "siscom_mi.INCASSI_INGIUNZIONE.data_pagamento <= '" & dPagAl & "' and "
                End If
                If dPag <> "" Or dPagAl <> "" Then
                    sWhere = sWhere & "   AND EXISTS " _
                   & " (SELECT BOL_BOLLETTE.ID FROM siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING" _
                   & "    WHERE(BOL_BOLLETTE_PAGAMENTI_ING.ID_BOLLETTA = BOL_BOLLETTE.ID)" _
                   & " AND EXISTS (SELECT INCASSI_INGIUNZIONE.ID FROM siscom_mi.INCASSI_INGIUNZIONE WHERE " & stringDate & " INCASSI_INGIUNZIONE.ID=BOL_BOLLETTE_PAGAMENTI_ING.ID_INCASSO_ING)" _
                   & "  )"
                End If
            End If
            ...
            par.cmd.CommandText = "SELECT RAPPORTI_UTENZA.ID, " _
                 ...
                 & "    (SELECT max(getdata(INCASSI_INGIUNZIONE.data_pagamento))" _
             & " FROM siscom_mi.INCASSI_INGIUNZIONE, siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING " _
             & " WHERE INCASSI_INGIUNZIONE.ID =" _
                   & "    BOL_BOLLETTE_PAGAMENTI_ING.ID_INCASSO_ING " _
                         & "    AND BOL_BOLLETTE_PAGAMENTI_ING.id_bolletta =" _
                                  & "  bol_bollette.id) as data_pagamento," _
                & "   (SELECT MAX(descrizione)" _
          & "  FROM siscom_mi.TIPO_PAG_INGIUNZIONE, siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING" _
          & "  WHERE ID = BOL_BOLLETTE_PAGAMENTI_ING.ID_TIPO_PAGAMENTO " _
         & "      AND BOL_BOLLETTE_PAGAMENTI_ING.id_bolletta = bol_bollette.id)" _
         & " AS TIPO,TIPO_BOLL_INGIUNZIONE.DESCRIZIONE AS INGIUNZIONE," _
                                 & "TRIM (TO_CHAR(nvl(importo_ingiunzione,0),'9G999G999G990D99')) AS importo_ingiunzione,TRIM (TO_CHAR(nvl(imp_ingiunzione_pag,0),'9G999G999G990D99')) AS imp_ingiunzione_pag," _
                                 & "TRIM (TO_CHAR ((nvl(importo_ingiunzione,0)-nvl(imp_ingiunzione_pag,0)), '9G999G999G990D99')) AS RESIDUO " _
                                 & "FROM SISCOM_MI.ANAGRAFICA,siscom_mi.SOGGETTI_CONTRATTUALI, " _
                                 & "SISCOM_MI.RAPPORTI_UTENZA, SISCOM_MI.BOL_BOLLETTE,SISCOM_MI.TIPO_BOLL_INGIUNZIONE" _
                                 & " " _
                                 & "WHERE " _
     & "    NVL (BOL_BOLLETTE.importo_ingiunzione, 0) <> 0" _
     & "    AND bol_bollette.id_contratto = rapporti_utenza.id AND TIPO_BOLL_INGIUNZIONE.ID(+)=ID_TIPO_INGIUNZIONE " _
            ...
            For Each row In dt.Rows
                TotIncass = TotIncass + CDbl(par.IfNull(row.Item("importo_ingiunzione"), 0))
                TotEcced = TotEcced + CDbl(par.IfNull(row.Item("imp_ingiunzione_pag"), 0))
                TotTotale = TotTotale + CDbl(par.IfNull(row.Item("RESIDUO"), 0))
            Next
            row = dt.NewRow()
            row.Item("TIPO") = "T O T A L E"
            row.Item("importo_ingiunzione") = Format(TotIncass, "##,##0.00")
            row.Item("imp_ingiunzione_pag") = Format(TotEcced, "##,##0.00")
        ...
        Catch ex As Exception
            If par.OracleConn.State = Data.ConnectionState.Open Then
                ...
                par.OracleConn.Close()
                Oracle.DataAccess.Client.OracleConnection.ClearAllPools()
                Session.Add("ERRORE", "Report Ing DETTAGLI:" & Page.Title & " - " & ex.Message)
    ...
    Protected Sub btnExport_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles btnExport.Click
        Try
            If dgvRptPagExtraMav.Items.Count > 0 Then
                ...
                Next
                Dim nomeFile As String = xls.EsportaExcelDaDataGridWithDT(ExcelSiSol.Estensione.Office2007_xlsx, "ExportPgIng", "ExportPgIng", dgvRptPagExtraMav, dtExport)
            ...
            End If
        Catch ex As Exception
            Session.Add("ERRORE", "Report Ing GENERALE:" & Page.Title & " - " & ex.Message)
            Response.Write("<script>top.location.href='../../Errore.aspx';</script>")
        End Try
    End Sub
