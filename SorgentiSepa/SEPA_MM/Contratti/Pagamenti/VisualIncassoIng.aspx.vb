Imports System.IO
Partial Class Contratti_Pagamenti_VisualIncassoIng
    ...
    Private Sub DatiIncasso(Optional isExport As Boolean = False)
        Try
            ...
            par.cmd.CommandText = "SELECT num_bolletta," _
                                ...
                                & "to_char((SELECT SUM(importo_pagato) FROM siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING b WHERE b.id_bolletta = BOL_BOLLETTE.ID AND b.ID_INCASSO_ING =  " & idIncasso.Value & " ) ,'9G999G999G990D99')AS PAG_INCASSO " _
                                & "FROM siscom_mi.BOL_BOLLETTE " _
                                & "WHERE " _
                                & "BOL_BOLLETTE.ID IN (SELECT DISTINCT id_bolletta FROM siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING WHERE ID_INCASSO_ING = " & idIncasso.Value & " ) " _
    ...
    Private Sub CaricaTitolo()
        Try
            ...
            par.cmd.CommandText = "SELECT TIPO_PAG_INGIUNZIONE.descrizione AS tipo_incasso,motivo_pagamento, " _
                                & "siscom_mi.Getintestatari(id_contratto) as inte,siscom_mi.Getdata(data_pagamento) AS data_pagamento, " _
                                & "siscom_mi.Getdataora(data_ora) AS operazione FROM siscom_mi.INCASSI_INGIUNZIONE,siscom_mi.TIPO_PAG_INGIUNZIONE " _
                                & "WHERE TIPO_PAG_INGIUNZIONE.ID = id_tipo_pag AND INCASSI_INGIUNZIONE.ID = " & idIncasso.Value
