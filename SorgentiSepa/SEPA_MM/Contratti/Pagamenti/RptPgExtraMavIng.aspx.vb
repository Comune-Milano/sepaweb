...
Partial Class Contratti_Pagamenti_RptPgExtraMavIng
    ...
    Private Sub FindIncassi()
        Try
            ...
            If Session.Item("LIVELLO") <> 1 Then
                If Session.Item("MOD_REPORT_INGIUNZ") = 0 Then
            ...
            If par.IfEmpty(Request.QueryString("DPAG"), "Null") <> "Null" Then
                ...
                sWhere = sWhere & " AND INCASSI_INGIUNZIONE.data_pagamento >= " & sValore & " "
            ...
            If par.IfEmpty(Request.QueryString("TIPINC"), "-1") <> "-1" Then
                ...
                sWhere = sWhere & " AND INCASSI_INGIUNZIONE.ID_TIPO_PAG = " & sValore & " "
            End If

            'tabella di generali più campi di dettaglio'

            par.cmd.CommandText = "SELECT  DISTINCT INCASSI_INGIUNZIONE.ID AS ID, " _
                                 & "GETDATAORA (INCASSI_INGIUNZIONE.DATA_ORA) AS DATA_ORA, " _
                                 & "TIPO_PAG_INGIUNZIONE.DESCRIZIONE AS TIPO, " _
                                 & "INCASSI_INGIUNZIONE.MOTIVO_PAGAMENTO AS DESCRIZIONE, " _
                                 & "TO_CHAR " _
                                 & "(TO_DATE (INCASSI_INGIUNZIONE.DATA_PAGAMENTO, 'YYYYMMDD'), " _
                                 ...
                                 & "FROM SISCOM_MI.INCASSI_INGIUNZIONE,SISCOM_MI.ANAGRAFICA,siscom_mi.SOGGETTI_CONTRATTUALI, " _
                                 & "OPERATORI, " _
                                 & "SISCOM_MI.RAPPORTI_UTENZA, " _
                                 & "SISCOM_MI.BOL_BOLLETTE_PAGAMENTI_ING, " _
                                 & "SISCOM_MI.TIPO_PAG_INGIUNZIONE " _
                                 & "WHERE OPERATORI.ID = INCASSI_INGIUNZIONE.ID_OPERATORE " _
                                 & "AND RAPPORTI_UTENZA.ID = INCASSI_INGIUNZIONE.ID_CONTRATTO " _
                                 & "AND INCASSI_INGIUNZIONE.ID= BOL_BOLLETTE_PAGAMENTI_ING.ID_INCASSO_ING and nvl(INCASSI_INGIUNZIONE.fl_Annullata,0)=0 " _
            ...
            End If
            lettor.Close()
            Me.lblTitolo.Text = "Report Pagamenti Manuali Ingiunz.<br/> " & titoloCond & "<br/> OPERATORE: " & NameOp
    ...
    Private Sub Cerca2(ByVal idIncasso As String, ByVal tipo_incasso As String, ByRef dtCopia As Data.DataTable)
        ...
        'varname1 = "SELECT DISTINCT INCASSI_INGIUNZIONE.ID, " _
        '         & "getdataora (INCASSI_INGIUNZIONE.data_ora) AS data_ora," _
        ...
        '         & "FROM siscom_mi.INCASSI_INGIUNZIONE, " _
        '         & "siscom_mi.anagrafica, " _
        '         & "siscom_mi.soggetti_contrattuali, " _
        '         & "siscom_mi.rapporti_utenza,siscom_mi.bol_bollette, " _
        '         & "siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING, " _
        '         & "siscom_mi.TIPO_PAG_INGIUNZIONE " _
        '         & "WHERE rapporti_utenza.ID = INCASSI_INGIUNZIONE.id_contratto " _
        '         & "AND INCASSI_INGIUNZIONE.ID = " _
        '         & "BOL_BOLLETTE_PAGAMENTI_ING.ID_INCASSO_ING " _
        '         & "AND TIPO_PAG_INGIUNZIONE.ID = INCASSI_INGIUNZIONE.id_tipo_pag " _
        '         & "AND siscom_mi.soggetti_contrattuali.id_contratto = siscom_mi.rapporti_utenza.ID " _
        '         & " AND bol_bollette.ID = siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING.id_bolletta " _
        '         & "AND siscom_mi.soggetti_contrattuali.id_anagrafica = anagrafica.ID " _
        '         & "AND siscom_mi.soggetti_contrattuali.cod_tipologia_occupante = " _
        '         & "'INTE' " _
        '         & "AND INCASSI_INGIUNZIONE.ID =" & idIncasso _
        ...
        varname1 = "SELECT bol_bollette.ID AS id_bolletta, " _
                 ...
                 & "BOL_BOLLETTE_PAGAMENTI_ING.importo_pagato AS IMPORTO  " _
                 & "FROM siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING, " _
                 & "siscom_mi.bol_bollette, " _
                 & "siscom_mi.soggetti_contrattuali " _
                 & "WHERE ID_INCASSO_ING = " & idIncasso _
                 & " AND bol_bollette.ID = siscom_mi.BOL_BOLLETTE_PAGAMENTI_ING.id_bolletta " _
