...
Partial Class Contratti_Pagamenti_PagaModificaRuolo
    ...
    Private Sub CaricaTipoPagamento()
        par.cmd.CommandText = "select ID_TIPO_PAG from siscom_mi.INCASSI_RUOLI where id = " & idIncasso.Value
        ...
        If IsNothing(Me.cmbTipoPagamento.Items.FindByValue(modalitaPag)) Then
            par.cmd.CommandText = "select descrizione from siscom_mi.TIPO_PAG_RUOLO where id = " & modalitaPag
    ...
    Private Sub CaricaDatiIncasso()
        Try
            ...
            End If
            par.cmd.CommandText = "select * from siscom_mi.INCASSI_RUOLI where id = " & idIncasso.Value
            ...
            If reader.Read Then
                ...
                If par.IfNull(reader("fl_sgravio"), "0") = "1" Then
                    chkSgravio.Checked = True
                Else
                    chkSgravio.Checked = False
                End If
            End If
            reader.Close()
            par.cmd.CommandText = "SELECT id_bolletta,sum(nvl(importo_pagato,0)) as importo_pagato FROM siscom_mi.BOL_BOLLETTE_PAGAMENTI_RUOLO WHERE ID_INCASSO_RUOLO = " & idIncasso.Value & " group by id_bolletta order by id_bolletta asc"
    ...
    Private Function EliminaPagamento() As Boolean
        ...
        par.cmd.CommandText = "update SISCOM_MI.INCASSI_RUOLI set fl_annullata = 1 where id = " & idIncasso.Value
        par.cmd.ExecuteNonQuery()
        par.cmd.CommandText = "select data_pagamento from SISCOM_MI.INCASSI_RUOLI where id = " & idIncasso.Value
        Dim dataPagIncasso As String = par.cmd.ExecuteScalar
        par.cmd.CommandText = "SELECT id_bolletta,sum(nvl(importo_pagato,0)) as importo_pagato,ID_TIPO_PAGAMENTO FROM SISCOM_MI.BOL_BOLLETTE_PAGAMENTI_RUOLO WHERE ID_INCASSO_RUOLO = " & idIncasso.Value & " group by id_bolletta,ID_TIPO_PAGAMENTO"
        Dim da As New Oracle.DataAccess.Client.OracleDataAdapter(par.cmd)
        Dim dt As New Data.DataTable()
        da.Fill(dt)
        For Each r As Data.DataRow In dt.Rows
            par.cmd.CommandText = "update SISCOM_MI.bol_bollette set imp_ruolo_pagato = (nvl(imp_ruolo_pagato,0) + " & par.VirgoleInPunti(r.Item("importo_pagato") * -1) & ") where id = " & r.Item("id_bolletta")
            par.cmd.ExecuteNonQuery()

            par.cmd.CommandText = "INSERT INTO SISCOM_MI.BOL_BOLLETTE_PAGAMENTI_RUOLO (ID_BOLLETTA,DATA_PAGAMENTO,IMPORTO_PAGATO,ID_TIPO_PAGAMENTO,ID_INCASSO_RUOLO,DATA_OPERAZIONE) VALUES " _
        ...
        par.cmd.CommandText = "update SISCOM_MI.INCASSI_RUOLI set fl_annullata = 0 where id = " & idIncasso.Value
        par.cmd.ExecuteNonQuery()
    ...
    Private Function AggiornaDatiIncasso() As Boolean
        ...
        par.cmd.CommandText = "UPDATE siscom_mi.INCASSI_RUOLI SET FL_SGRAVIO=" & Valore01(chkSgravio.Checked) & ",IMPORTO = " & par.VirgoleInPunti(Me.impWritePagamento.Value) & ", DATA_PAGAMENTO = '" & par.AggiustaData(Me.txtDataPagamento.Text) & "' , DATA_REGISTRAZIONE = '" & par.AggiustaData(Me.txtDataRegistrazione.Text) & "', " _
                            & "DATA_ORA='" & Format(Now, "yyyyMMddHHmmss") & "',ID_TIPO_PAG =" & Me.cmbTipoPagamento.SelectedValue & " ,MOTIVO_PAGAMENTO = '" & par.PulisciStrSql(Me.txtnote.Text) & "', numero_assegno = '" & par.PulisciStrSql(txtNumAssegno.Text.ToUpper) & "',ID_OPERATORE = " & Session.Item("id_operatore") & " " _
                            & "WHERE ID = " & idIncasso.Value
        par.cmd.ExecuteNonQuery()
        par.cmd.CommandText = "UPDATE siscom_mi.BOL_BOLLETTE_PAGAMENTI_RUOLO SET DATA_PAGAMENTO = '" & par.AggiustaData(Me.txtDataPagamento.Text) & "', DATA_OPERAZIONE =  '" & par.AggiustaData(Me.txtDataRegistrazione.Text) & "', DATA_VALUTA = '" & par.AggiustaData(Me.txtDataRegistrazione.Text) & "' WHERE ID_INCASSO_RUOLO  = " & idIncasso.Value
        par.cmd.ExecuteNonQuery()
