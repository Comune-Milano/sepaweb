...
Partial Class Contratti_Canone_Usi_Diversi
    ...
    Protected Sub ImgProcedi_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImgProcedi.Click
        Try
            If txtpg.Text <> "" And txtCanoneCorrente.Text <> "" And txtData.Text <> "" And par.AggiustaData(txtData.Text) <= Format(Now, "yyyyMMdd") Then
                ...
                CodiceContratto = CODICEUnita & Format((progressivo), "00")
                ...
                If myReader.Read = True Then
                    ...
                    par.cmd.CommandText = "Insert into siscom_mi.UNITA_ASSEGNATE (ID_DOMANDA, ID_UNITA, DATA_ASSEGNAZIONE, " _
                                        ...
                                        & "', '" & par.PulisciStrSql(cf) & "', '" & prov & "', 0," & par.VirgoleInPunti(txtCanoneCorrente.Text) & "," & Indice & ",'" & par.PulisciStrSql(txtpg.Text) & "','" & par.AggiustaData(txtData.Text) & "')"
                    par.cmd.ExecuteNonQuery()

                    par.cmd.CommandText = "update SISCOM_MI.UI_USI_DIVERSI set stato='8',assegnato='1',DATA_PRENOTATO='" & par.AggiustaData(txtData.Text) & "' where COD_ALLOGGIO='" & CODICEUnita & "'"
                    par.cmd.ExecuteNonQuery()
                    ...
                    Dim SriptJSContratto As String = "var chiediConferma;" _
                                              ...
                                              & "a = window.open('Contratto.aspx?ID=-1&IdDichiarazione=" & Indice & "&IdDomanda=-1&IdUnita=" & Unita & "&TIPO=3&Lett=" & prov & "','Contratto" & Format(Now, "hhss") & "','height=780,width=1160');" _
                                              & "window.close();}" _
                                              & "else {" _
                                              & "window.close();" _
                                              & "}"
