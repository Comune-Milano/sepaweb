...
Partial Class ListaScarico
    ...
    Protected Sub btnScarica_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnScarica.Click
        ...
        If lN_Distinta = -1 Then
            par.cmd.CommandText = "INSERT INTO DOMANDE_DISTINTE (NUMERO,DATA,ID_OPERATORE) VALUES (SEQ_DISTINTE.NEXTVAL,'" & Format(Now, "yyyyMMdd") & "'," & Session.Item("ID_OPERATORE") & ")"
            par.cmd.ExecuteNonQuery()
            par.cmd.CommandText = "SELECT SEQ_DISTINTE.CURRVAL FROM DUAL"
            Dim myReader As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
        ...
        For I = 0 To CheckOperatori.Items.Count - 1
            If CheckOperatori.Items(I).Selected = True Then
                par.cmd.CommandText = "UPDATE DOMANDE_BANDO SET N_DISTINTA=" & lN_Distinta & " WHERE ID='" & CheckOperatori.Items(I).Value & "'"
                par.cmd.ExecuteNonQuery()
                ...
                par.cmd.CommandText = "select DOMANDE_BANDO.ID,DOMANDE_BANDO.PG,TO_CHAR(TO_DATE(DOMANDE_BANDO.DATA_PG,'YYYYmmdd'),'DD/MM/YYYY') AS ""DATA_PG1"",DOMANDE_BANDO.isbarc_r,COMP_NUCLEO.COGNOME,COMP_NUCLEO.NOME from DOMANDE_BANDO,DICHIARAZIONI,COMP_NUCLEO where DOMANDE_BANDO.PROGR_COMPONENTE=COMP_NUCLEO.PROGR AND COMP_NUCLEO.ID_DICHIARAZIONE=DOMANDE_BANDO.ID_DICHIARAZIONE AND DOMANDE_BANDO.ID_DICHIARAZIONE=DICHIARAZIONI.ID and DOMANDE_BANDO.ID=" & CheckOperatori.Items(I).Value
                Dim myReader As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                ...
                J = J + 1
            Else
                par.cmd.CommandText = "UPDATE DOMANDE_BANDO SET N_DISTINTA=0 WHERE ID='" & CheckOperatori.Items(I).Value & "'"
                par.cmd.ExecuteNonQuery()

                par.cmd.CommandText = "DELETE FROM EVENTI_BANDI WHERE ID_DOMANDA=" & CheckOperatori.Items(I).Value & " AND TIPO_OPERATORE='I' AND COD_EVENTO='F143'"
                par.cmd.ExecuteNonQuery()
            End If
