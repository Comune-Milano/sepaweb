...
Partial Class RATEIZZAZIONE_ProspettoRateiz
    ...
    Public Property vIdRateiz() As String
        Get
            If Not (ViewState("par_IdRateiz") Is Nothing) Then
                Return CStr(ViewState("par_IdRateiz"))
            Else
                Return ""
            End If
        End Get
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        '' ''.......72 rate max consentite.......
        ' ''If Trim(txtNRate.Text) <> "" Then
        ' ''    txtNRate.Text = Min(72, CInt(txtNRate.Text))
        ' ''End If
        '' ''....................................

        '' ''..........controllo importo.........
        ' ''If txtImpRate.Text > txtCapitale.Text Then
        ' ''    txtImpRate.Text = ""
        ' ''End If
        '' ''....................................
        ...
        Try
            ' Me.txtCapitale.Text = par.IfEmpty(CapitaleRateiz.Value, 0)



            If Not IsPostBack Then
                vIdBolletta = Session.Item("IDBOLLETTE")
                vIdBolletta1 = par.Cripta(Session.Item("IDBOLLETTE"))

                If Not String.IsNullOrEmpty(vIdBolletta) Then
                    CaricaDettaglio()
                    SololetturaAccesso()
                    Me.btnPrintAccDebito.Visible = True
                ...
                txtNRate.Attributes.Add("onBlur", "javascript:valid(this,'notnumbers');MaxRata();")

                txtImpRate.Attributes.Add("onBlur", "javascript:valid(this,'notnumbers');AutoDecimal2(this);MaxImporto();")
                txtImpRate.Attributes.Add("onkeypress", "javascript:SostPuntVirg(event,this);")



                Me.rdbType.SelectedValue = 0
                Me.rdbType.Attributes.Add("onClick", "javascript:Scelta();")

                'If Me.chkPagato.Checked = True Then
                '    Sololettura()
                'End If
    ...
    '        'If Not String.IsNullOrEmpty(vIdBolletta) Then
    '        '    par.cmd.CommandText = "SELECT * FROM SISCOM_MI.BOL_RATEIZZAZIONI WHERE ID_BOLLETTA IN (" & vIdBolletta & ")"

    '        '    Dim Lettore As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader
    '        '    If Lettore.Read Then
    '        '        Me.txtVersAnticipo.Text = Format(par.IfNull(Lettore("IMP_ANTICIPO"), 0), "##,##0.00")
    '        '        Me.txtCapitale.Text = Format(par.IfNull(Lettore("IMP_RESIDUO"), 0), "##,##0.00")
    '        '        Me.txtscadBollettino.Text = par.FormattaData(Lettore("SCADENZA_ANTICIPO"))
    '        '        Me.txtData.Text = par.FormattaData(Lettore("DATA_EMISSIONE"))
    '        '        Me.rdbType.SelectedValue = par.IfNull(Lettore("TIPO_RATEIZZAZIONE"), 0)
    '        '        'If par.IfNull(Lettore("FL_SOSPESO"), 0) = 1 Then
    '        '        '    Me.chkPagato.Checked = True
    '        '        '    Me.chkPagato.Visible = True
    '        '        'Else
    '        '        '    Me.chkPagato.Checked = False
    '        '        'End If

    '        '        Me.txtNRate.Text = par.IfNull(Lettore("NUM_RATE"), "")
    '        '        Me.txtImpRate.Text = par.IfNull(Lettore("IMPORTO_RATA"), "0,00")

    '        '        vIdRateiz = Lettore("ID")
    '        '        Sololettura()
    '        '    Else
    '        '        vIdRateiz = 0
    '        '    End If
    ...
    '    Catch ex As Exception
    '        Me.lblErrore.Visible = True
    '        lblErrore.Text = "OpenExistingData - " & ex.Message
    '        '*********************CHIUSURA CONNESSIONE**********************
    '        par.OracleConn.Close()
    '        Oracle.DataAccess.Client.OracleConnection.ClearAllPools()
    ...
    'End Sub
    'Private Sub SettaFinestra()
    '    Try
    '        Select Case vIdTipoBol
    '            Case 4
    '                Me.btnSa.Visible = False
    '            Case 7
    '                Response.Write("<script>alert('Impossibile rateizzare questa tipologia di bolletta!');self.close();</script>")
    '            Case Else
    '                Me.btnPrintAccDebito.Visible = False
    ...
    '    Catch ex As Exception
    '        Me.lblErrore.Visible = True
    '        lblErrore.Text = "SettaFinestra - " & ex.Message
    ...
    Private Sub CaricaDettaglio()
        Try
            ...
            testoTabella = "<div style='overflow: auto; width: 100%; height: 150px'><table cellpadding='0' cellspacing='0' width='98%'>" & vbCrLf _
                ...
                & "<span style='font-size: 8pt; font-family: Arial'><strong>IMPORTO BOLLETTA</strong></span></td>" _
                & "<td style='height: 19px;text-align:right'>" _
            ...
            'par.cmd.CommandText = "SELECT BOL_BOLLETTE.*, (CASE WHEN ID_BOLLETTA_RIC IS NULL THEN TIPO_BOLLETTE.ACRONIMO ELSE TIPO_BOLLETTE.ACRONIMO ||'/RIC'END) AS TIPO,ID_TIPO , " _
            ...
            '                    & "WHERE  BOL_BOLLETTE.ID in (" & vIdBolletta & ") AND SOGGETTI_CONTRATTUALI.ID_CONTRATTO= BOL_BOLLETTE.ID_CONTRATTO AND SOGGETTI_CONTRATTUALI.DATA_FINE>= TO_CHAR(TO_DATE(CURRENT_DATE,'dd/mm/yyyy'),'yyyymmdd') " _
            ...
            ''        & "(CASE WHEN anagrafica.partita_iva IS NOT NULL THEN partita_iva ELSE COD_FISCALE END) AS CFIVA " _
            ''        & "FROM SISCOM_MI.BOL_BOLLETTE, SISCOM_MI.SOGGETTI_CONTRATTUALI, SISCOM_MI.TIPO_BOLLETTE, SISCOM_MI.ANAGRAFICA " _
            ''        & "WHERE  BOL_BOLLETTE.ID in (" & vIdBolletta & ") AND SOGGETTI_CONTRATTUALI.ID_CONTRATTO= BOL_BOLLETTE.ID_CONTRATTO  " _
            ''        & "AND TIPO_BOLLETTE.ID(+) = BOL_BOLLETTE.ID_TIPO AND SOGGETTI_CONTRATTUALI.COD_TIPOLOGIA_OCCUPANTE = 'INTE' AND SOGGETTI_CONTRATTUALI.ID_ANAGRAFICA = ANAGRAFICA.ID "
            ...
            Do While reader.Read
                TotDaPagare = 0
                'If par.IfNull(reader("IMPORTO_PAGATO"), 0) <> 0 Then
                '    Sololettura()
                '    Response.Write("<script>alert('Impossibile rateizzare.La bolletta risulta già pagata!');</script>")
                '    Exit Sub
                'End If
                ...
                End If
                'If par.IfNull(reader("IMPORTO_TOTALE"), 0) <= 0 Then
                '    Sololettura()
                '    Response.Write("<script>alert('Impossibile rateizzare.La bolletta ha importo negativo o pari a zero!');</script>")
                'End If
                ...
                'vidContratto = par.IfNull(reader("ID_CONTRATTO"), 0)
                ...
                If par.IfNull(reader("FL_ANNULLATA"), "0") = "0" Then
                    ...
                    Do While lettore2.Read
                        ...
                        'TotDaPagare = TotDaPagare + par.IfNull(lettore2("IMPORTO"), 0)
                    ...
                    'If par.IfNull(reader("TIPO"), "n.d.") <> "MOR" And par.IfNull(reader("TIPO"), "n.d.") <> "FIN" Then
                    ...
                    '    par.cmd.CommandText = "SELECT SUM(IMPORTO_PAGATO) AS TOT_MOROSITA FROM SISCOM_MI.BOL_BOLLETTE WHERE ID_BOLLETTA_RIC =" & par.IfNull(reader("ID"), 0)
                    '    lettore = par.cmd.ExecuteReader
                    '    If lettore.Read Then
                    '        TotFinalePagato = TotFinalePagato + (par.IfNull(reader("IMPORTO_PAGATO"), 0) - par.IfNull(lettore("TOT_MOROSITA"), 0))

                    '    Else
                    '        TotFinalePagato = TotFinalePagato + par.IfNull(reader("IMPORTO_PAGATO"), 0)
                    '    End If
                    '    lettore.Close()
                    'End If
                    ...
                    '' ''    testoTabella = testoTabella _
                    '' ''    & "<tr>" _
                    '' ''    & "<td style='background-color: #CCCCFF;height: 19px;border-bottom-style: ridge; border-bottom-width: 2px; border-bottom-color: #FFFFFF'>" _
                    '' ''    & "<span style='font-size: 8pt; font-family: Arial; vertical-align :top '>" & Contatore & ")</span></td>" _
                    '' ''    & "<td style='background-color: #CCCCFF;height: 19px;border-bottom-style: ridge; border-bottom-width: 2px; border-bottom-color: #FFFFFF'>" _
                    '' ''    & "<span style='font-size: 8pt; font-family: Arial; vertical-align :top '>" & Format(par.IfNull(reader("ID"), ""), "0000000000") & "</span></td>" _
                    '' ''    & "<td style='background-color: #CCCCFF;height: 19px;border-bottom-style: ridge; border-bottom-width: 2px; border-bottom-color: #FFFFFF'>" _
                    '' ''    & "<span style='font-size: 8pt; font-family: Arial'>" & vNbolletta & "</span></td>" _
                    '' ''    & "<td style='background-color: #CCCCFF;height: 19px;border-bottom-style: ridge; border-bottom-width: 2px; border-bottom-color: #FFFFFF'>" _
                    '' ''    & "<span style='font-size: 8pt; font-family: Arial; vertical-align :top '>" & par.FormattaData(par.IfNull(reader("DATA_EMISSIONE"), "")) & "</span></td>" _
                    '' ''    & "<td style='background-color: #CCCCFF;height: 19px;border-bottom-style: ridge; border-bottom-width: 2px; border-bottom-color: #FFFFFF'>" _
                    '' ''    & "<span style='font-size: 8pt; font-family: Arial; vertical-align :top '>" & par.FormattaData(par.IfNull(reader("DATA_SCADENZA"), "")) & "</span></td>" _
                    ...
                    '' ''        & "<span style='font-size: 6pt; font-family: Courier New'>" & par.IfNull(lettore2("DESCRIZIONE"), "") & "</a></span></td>" _
                    ...
                    '' ''    lettore2.Close()
                    '' ''    testoTabella = testoTabella & "</table></div></td>" _
                    '' ''    & "<td style='background-color: #CCCCFF;height: 19px;text-align:right;border-bottom-style: ridge; border-bottom-width: 2px; border-bottom-color: #FFFFFF'>" _
                    '' ''    & "<span style='font-size: 8pt; font-family: Arial; vertical-align :top '>€." & Format(TotDaPagare, "##,##0.00") & "</span></td>" _
                    '' ''    & "<td style='height: 19px;text-align:right;border-bottom-style: ridge; border-bottom-width: 2px; border-bottom-color: #FFFFFF'>" _
                    '' ''    & "<span style='font-size: 8pt; font-family: Arial; vertical-align :top '>€." & Format(par.IfNull(reader("IMPORTO_PAGATO"), 0), "##,##0.00") & "</span></td></tr>"

                    '' ''    'If par.IfNull(reader("TIPO"), "n.d.") <> "MOR" Then
                    '' ''    TotFinalePagato = TotFinalePagato + par.IfNull(reader("IMPORTO_PAGATO"), 0)
                    '' ''    '    Else
                    '' ''    '    Dim lettore As Oracle.DataAccess.Client.OracleDataReader
                    '' ''    '    par.cmd.CommandText = "SELECT SUM(IMPORTO_PAGATO) AS TOT_MOROSITA FROM SISCOM_MI.BOL_BOLLETTE WHERE ID_BOLLETTA_RIC =" & par.IfNull(reader("ID"), 0)
                ...
                'If vIdTipoBol = 4 Then
                '    Me.txtVersAnticipo.Enabled = False
                'End If
    ...
    Protected Sub BtnSimula_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles BtnSimula.Click
        ...
        ElseIf rdbType.SelectedValue = 1 Then
            If Not String.IsNullOrEmpty(Me.txtImpRate.Text) Then
    ...
    Private Function EmettiMavRata(ByVal idRata As String) As Boolean
        ...
        Try
            ...
            'par.cmd.CommandText = "selec distinct(id_contratto) from siscom_mi.bol_bollette where id in (" & vIdBolletta & ")"
            'Dim lettore As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader
            'If lettore.Read Then
            ...
            'par.cmd.CommandText = "SELECT BOL_BOLLETTE.*, ANAGRAFICA.ID AS ID_ANA , anagrafica.sesso, " _
            ...
            ' '' ''AND SOGGETTI_CONTRATTUALI.DATA_FINE>= TO_CHAR(TO_DATE(CURRENT_DATE,'dd/mm/yyyy'),'yyyymmdd')
            ...
            If dt.Rows.Count > 0 Then
                If Len(par.IfNull(dt.Rows(0).Item("PARTITA_IVA"), 0)) = 11 Or par.ControllaCF(par.IfNull(dt.Rows(0).Item("COD_FISCALE"), 0)) = True Then
                    ...
                    Else
                        ...
                        ' Response.Write("<script>window.open('../ALLEGATI/CONTRATTI/ELABORAZIONI/MAV/" & Format(ID_BOLLETTA, "0000000000") & ".xml')</script>")
                ...
                Else
                    ...
                    'Response.Write("<p style='color: #FF0000; font-weight: bold'>La Raccomandata e il bollettino di " & par.IfNull(dt.Rows(0).Item("INTECONTRATTO"), "") & " non sono stati stampati perchè il codice fiscale o la partita iva non hanno un formato corretto!</p>")
                    lblErrore.Visible = True
        ...
        Try
            ...
            If dt.Rows.Count > 0 Then
                ...
                If Len(par.IfNull(dt.Rows(0).Item("PARTITA_IVA"), 0)) = 11 Or par.ControllaCF(par.IfNull(dt.Rows(0).Item("COD_FISCALE"), 0)) = True Then
                    ...
                    'par.cmd.CommandText = "SELECT ID FROM SISCOM_MI.BOL_RATEIZZAZIONI_DETT WHERE ID_RATEIZZAZIONE = " & vIdRateiz & " AND quota_interessi is null and residuo is null"
                    ...
                    par.cmd.CommandText = "INSERT INTO SISCOM_MI.BOL_RATEIZZAZIONI_DETT (ID,ID_RATEIZZAZIONE,NUM_RATA,DATA_EMISSIONE,DATA_SCADENZA,IMPORTO_RATA,QUOTA_CAPITALI,QUOTA_INTERESSI,RESIDUO) VALUES " _
                                        & "(" & idRataAnticipo & ", " & vIdRateiz & ",0," & Format(Now, "yyyyMMdd") & "," & par.AggiustaData(Me.txtscadBollettino.Text) _
    ...
    Private Function ControlMavRate() As Boolean
        ...
        Try
            par.cmd.CommandText = "SELECT * FROM SISCOM_MI.BOL_RATEIZZAZIONI_DETT WHERE ID_RATEIZZAZIONE = " & vIdRateiz & " ORDER BY DATA_EMISSIONE ASC"
            Dim da As New Oracle.DataAccess.Client.OracleDataAdapter(par.cmd)
    ...
    Private Function PrenotaRateizzazione() As Boolean
        ...
        Try
            ...
            'CONTROLLO PELLEGRI 
            If Not String.IsNullOrEmpty(Me.txtImpRate.Text.Replace(".", "")) Then
                Dim msgAnomalia As String = ""
                par.VerificaImportoMinimo(0, Me.txtImpRate.Text.Replace(".", ""), Me.CapitaleRateiz.Value.Replace(".", ""), Me.txtData.Text, tasso, giorniScad, msgAnomalia)
            ...
            If Not String.IsNullOrEmpty(par.IfEmpty(Me.txtCapitale.Text.Replace(".", ""), 0)) And Not String.IsNullOrEmpty(par.IfEmpty(Me.txtData.Text, "")) Then
                ...
                ElseIf Not String.IsNullOrEmpty(Me.txtImpRate.Text.Replace(".", "")) Then
                    dt = par.AmmortamentoPerRata(Me.CapitaleRateiz.Value.Replace(".", ""), Me.txtImpRate.Text.Replace(".", ""), tasso, 12, Me.txtData.Text, giorniScad)
                End If
                ...
                '                    & "AND UNITA_IMMOBILIARI.ID_INDIRIZZO = INDIRIZZI.ID  AND scale_edifici.ID = unita_immobiliari.id_scala AND BOL_BOLLETTE.ID in (" & vIdBolletta & ")"
                ...
                ''par.cmd.CommandText = "SELECT COD_CONTRATTO FROM SISCOM_MI.RAPPORTI_UTENZA,SISCOM_MI.BOL_BOLLETTE WHERE RAPPORTI_UTENZA.ID = BOL_BOLLETTE.ID_CONTRATTO AND BOL_BOLLETTE.ID in (" & vIdBolletta & ")"
                ...
                If vIdRateiz > 0 Then
                    par.cmd.CommandText = "DELETE FROM SISCOM_MI.BOL_RATEIZZAZIONI_DETT WHERE ID_RATEIZZAZIONE = " & vIdRateiz
                    par.cmd.ExecuteNonQuery()
                End If
                ...
                For Each row As Data.DataRow In dt.Rows
                    par.cmd.CommandText = "INSERT INTO SISCOM_MI.BOL_RATEIZZAZIONI_DETT (ID,ID_RATEIZZAZIONE,NUM_RATA,DATA_EMISSIONE,DATA_SCADENZA,IMPORTO_RATA,QUOTA_CAPITALI,QUOTA_INTERESSI,RESIDUO,FL_ANNULLATA) VALUES " _
                                        & "(SISCOM_MI.SEQ_BOL_RATEIZZAZIONI_DETT.NEXTVAL, " & vIdRateiz & "," & row.Item("NUMRATA") & "," & par.AggiustaData(row.Item("EMISSIONE")) & "," & par.AggiustaData(row.Item("SCADENZA")) _
                ...
                par.cmd.CommandText = "UPDATE SISCOM_MI.BOL_BOLLETTE SET ID_RATEIZZAZIONE = " & vIdRateiz & " WHERE ID IN (" & vIdBolletta & ")"
                ...
                '' ''par.cmd.CommandText = "select sum(nvl(quota_capitali,0)) from siscom_mi.bol_rateizzazioni_dett where id_rateizzazione = " & vIdRateiz
                '' ''Dim SumCapitaliRat As Decimal = par.cmd.ExecuteScalar

                '' ''par.cmd.CommandText = "select nvl(nvl(imp_anticipo,0) + nvl(imp_residuo,0),0) from siscom_mi.bol_rateizzazioni where id = " & vIdRateiz
                ...
                '' ''    par.cmd.CommandText = "update siscom_mi.bol_rateizzazioni_dett set quota_capitali = quota_capitali + " & par.VirgoleInPunti(centUpd) & " where id_rateizzazione = " & vIdRateiz & " and num_rata = (select max(r2.num_rata) from siscom_mi.bol_rateizzazioni_dett r2 where r2.id_rateizzazione = " & vIdRateiz & ")"
                '' ''    par.cmd.ExecuteNonQuery()
                '' ''End If
    ...
    Private Sub PrintPdf(ByVal Subtitle As String, ByVal interesse As Decimal, ByVal numrate As Integer, ByVal impSingola As Decimal, ByVal codContratto As String)
        Try
            ...
            'File.Delete(strFile)
            'Response.Redirect("..\ALLEGATI\CONTRATTI\" & NomeZipfile & ".zip", False)
    ...
    Private Function SalvaDati() As Boolean
        ...
        Try
            ...
            par.cmd.CommandText = "SELECT SISCOM_MI.SEQ_BOL_RATEIZZAZIONI.NEXTVAL FROM DUAL"
            Dim lettore As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader
            If lettore.Read Then
                vIdRateiz = lettore(0)
            End If
            ...
            '    Me.chkPagato.Checked = True
            'End If
            par.cmd.CommandText = "INSERT INTO SISCOM_MI.BOL_RATEIZZAZIONI" _
                                 & "(ID," _
                                 & "IMP_ANTICIPO," _
                                 & "IMP_RESIDUO," _
                                 & "SCADENZA_ANTICIPO," _
                                 & "FL_SOSPESO," _
                                 & "DATA_EMISSIONE," _
                                 & "TIPO_RATEIZZAZIONE," _
                                 & "NUM_RATE," _
                                 & "IMPORTO_RATA,ID_CONTRATTO,DATA_STIPULA) " _
                                 & "VALUES " _
                                 & "(" & vIdRateiz & "," _
                                 & par.VirgoleInPunti(Me.txtVersAnticipo.Text.Replace(".", "")) & "," _
                                 & par.VirgoleInPunti(Me.CapitaleRateiz.Value.Replace(".", "")) & ",'" _
                                 & par.AggiustaData(Me.txtscadBollettino.Text) & "'," _
                                 & "0,'" _
                                 & par.AggiustaData(Me.txtData.Text) & "'," _
                                 & Me.rdbType.SelectedValue & "," _
                                 & par.IfEmpty(Me.txtNRate.Text, "null") & "," _
                                 & par.IfEmpty(par.VirgoleInPunti(Me.txtImpRate.Text.Replace(".", "")), "NULL") & "," & idContratto.Value & "," & Format(Now, "yyyyMMdd") & ")"
            par.cmd.ExecuteNonQuery()
        ...
        Try
            ...
            If vIdRateiz > 0 Then
                ...
                par.cmd.CommandText = "UPDATE SISCOM_MI.BOL_RATEIZZAZIONI SET " _
                                    ...
                                    & "TIPO_RATEIZZAZIONE = " & Me.rdbType.SelectedValue & "," _
    ...
    Protected Sub btnSa_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSa.Click
        Try
            ...
            If ConfermaSalva.Value = 1 Then
                ...
                If Not String.IsNullOrEmpty(Me.txtNRate.Text) Or Not String.IsNullOrEmpty(Me.txtImpRate.Text) Then
                    AggiungiDt("", "")

                    'If par.IfEmpty(Me.txtVersAnticipo.Text, 0) > 0 And Me.chkPagato.Checked = False Then
                    '    SalvaDati()
                    'ElseIf par.IfEmpty(Me.txtVersAnticipo.Text, 0) > 0 And Me.chkPagato.Checked = True Then
                    '    UpdateDati()
                    '    PrenotaRateizzazione()
                    'ElseIf par.IfEmpty(Me.txtVersAnticipo.Text, 0) = 0 Then
                    ...
                    'Response.Write("<script>self.close();window.open('RisultatiRate.aspx','Result','height=400px,top=0,left=0,width=500px');</script>")
                ...
                Else
                    ...
                    Else
                        Response.Write("<script>alert('Definire l\'importo della rata!');</script>")
        ...
        Catch ex As Exception
            ...
            Response.Write("<script>alert('ERRORE duarante il collegamento!Operazione annullata!');</script>")
            Session.Remove("dtResult")
            Session.Remove("titolo")
