...
Partial Class RILEVAZIONI_RisultatiUIrilev
    ...
    Private Sub BindGridUnita()
        Try
            ...
            Dim motivazNonRilevati As String = ""
            If Not IsNothing(Request.QueryString("MOT")) Then
                motivazNonRilevati = Request.QueryString("MOT")
            End If
            If rilevati <> "" Then
                ...
                End Select
            Else
                Select Case motivazNonRilevati
                    Case "S"
                        condizioneRilevati = " AND UNITA_IMMOBILIARI.ID in (select id_unita from siscom_mi.RILIEVO_UI where id_lotto is not null and id_unita not in (select id_unita_immobiliare from siscom_mi.RILIEVO_ALLOGGIO_SFITTO)) "
                    Case "L"
                        condizioneRilevati = " AND UNITA_IMMOBILIARI.ID in (select id_unita from siscom_mi.RILIEVO_UI where id_lotto is not null and id_unita in (select id_unita_immobiliare from siscom_mi.RILIEVO_ALLOGGIO_SFITTO where NON_VISITAB_LASTRATURA=1)) "
                    Case "O"
                        condizioneRilevati = " AND UNITA_IMMOBILIARI.ID in (select id_unita from siscom_mi.RILIEVO_UI where id_lotto is not null and id_unita in (select id_unita_immobiliare from siscom_mi.RILIEVO_ALLOGGIO_SFITTO where NON_VISITAB_OCCUPATO=1)) "
                    Case "M"
                        condizioneRilevati = " AND UNITA_IMMOBILIARI.ID in (select id_unita from siscom_mi.RILIEVO_UI where id_lotto is not null and id_unita in (select id_unita_immobiliare from siscom_mi.RILIEVO_ALLOGGIO_SFITTO where NON_VISITAB_MURATO=1)) "
                    Case "C"
                        condizioneRilevati = " AND UNITA_IMMOBILIARI.ID in (select id_unita from siscom_mi.RILIEVO_UI where id_lotto is not null and id_unita in (select id_unita_immobiliare from siscom_mi.RILIEVO_ALLOGGIO_SFITTO where NON_VISITAB_NO_CHIAVI=1)) "
                    Case Else
                        condizioneRilevati = " AND UNITA_IMMOBILIARI.ID in (select id_unita from siscom_mi.RILIEVO_UI where id_lotto is not null and (id_unita not in (select id_unita_immobiliare from siscom_mi.RILIEVO_ALLOGGIO_SFITTO) OR id_unita in (select id_unita_immobiliare from siscom_mi.RILIEVO_ALLOGGIO_SFITTO where (NON_VISITAB_OCCUPATO=1 or NON_VISITAB_LASTRATURA=1 or NON_VISITAB_MURATO=1 or NON_VISITAB_NO_CHIAVI=1)))) "
                End Select
            ...
            Str = "SELECT UNITA_IMMOBILIARI.ID,tab_filiali.nome as filiale,(select RILIEVO_TAB_UTENTI.descrizione from siscom_mi.RILIEVO_TAB_UTENTI,siscom_mi.rilievo_lotti where RILIEVO_TAB_UTENTI.id=siscom_mi.rilievo_lotti.id_utente and rilievo_lotti.id=RILIEVO_UI.ID_lotto) as STUDIOPROF, COD_UNITA_IMMOBILIARE," _
                ...
                & "TO_CHAR (TO_DATE (DATA_CARICAMENTO, 'YYYYmmdd'), 'DD/MM/YYYY') as DATA_CARICAMENTO, OPERATORI.OPERATORE,RILIEVO_ALLOGGIO_SFITTO.NOTE, '' AS DOWNLOAD_SCHEDA,'' AS ELIMINA_SCHEDA,RILIEVO_ALLOGGIO_SFITTO.ID  AS ID_ALL_SFITTO,RILIEVO_ALLOGGIO_SFITTO.ID_OPERATORE_INSERIMENTO FROM SISCOM_MI.EDIFICI," _
            ...
            If dt.Rows.Count > 0 Then
                For Each rowIdContr As Data.DataRow In dt.Rows
                    ...
                    If par.IfNull(rowIdContr.Item("ID_ALL_SFITTO"), 0) <> 0 Then
                        rowIdContr.Item("DOWNLOAD_SCHEDA") = "<a href='javascript:AfterSubmit()' onclick=" & Chr(34) & "document.getElementById('CPContenuto_IDui').value=" & rowIdContr.Item("ID") & ";document.getElementById('CPContenuto_btnDownload1').click();" & Chr(34) & "><img src=" & Chr(34) & "../Standard/Immagini/download_16.png" & Chr(34) & " title=" & Chr(34) & "Scarica scheda" & Chr(34) & " alt=" & Chr(34) & "Download" & Chr(34) & " border=" & Chr(34) & "0" & Chr(34) & "/></a>"
                        If Session.Item("ID_OPERATORE") = par.IfNull(rowIdContr.Item("ID_OPERATORE_INSERIMENTO"), 0) Then
                            rowIdContr.Item("ELIMINA_SCHEDA") = "<a href='javascript:AfterSubmit()' onclick=" & Chr(34) & "document.getElementById('CPContenuto_LBLID').value=" & rowIdContr.Item("ID_ALL_SFITTO") & ";document.getElementById('CPContenuto_btnCancella').click();" & Chr(34) & "><img src=" & Chr(34) & "../Standard/Immagini/delete_ico_16.png" & Chr(34) & " title=" & Chr(34) & "Cancella scheda" & Chr(34) & " alt=" & Chr(34) & "Download" & Chr(34) & " border=" & Chr(34) & "0" & Chr(34) & "/></a>"
                            operatoreInser = True
                        End If
                    End If
                    If rilevati = "1" Then
                    ...
                    End If
                Next
                If operatoreInser = False Then
                    DataGridUnita.Columns(par.IndDGC(DataGridUnita, "ELIMINA_SCHEDA")).Visible = False
                End If
            End If
    ...
    Protected Sub btnExport_Click(sender As Object, e As System.EventArgs) Handles btnExport.Click
        ...
        da = New Oracle.DataAccess.Client.OracleDataAdapter(Str, par.OracleConn)
        da.Fill(dt)
        da.Dispose()
        If dt.Rows.Count > 0 Then
            For Each rowIdContr As Data.DataRow In dt.Rows
        ...
        If DataGridUnita.Items.Count > 0 Then

            Dim nomeFile As String = xls.EsportaExcelDaDataGridWithDT(ExcelSiSol.Estensione.Office2007_xlsx, "ExportUI", "UI", Me.DataGridUnita, dt)
    ...
    Private Sub ScaricaFile()
        Try
            ...
            'as sede_territoriale,'' as mm_spa,'' as quartiere,'' as indirizzo," _
            '& "'' as scala,'' as piano, '' as n_ui,0 as sup_netta,0 as h_interna,'' as referente,'' as tel,'' as email," _
            '& "'' as data_sopralluogo,'' as data_caricamento,'' 
            ...
            ws.Cells("N3").Value = par.IfNull(dt1.Rows(0).Item("sede_territoriale"), "")
            ws.Cells("X3").Value = par.IfNull(dt1.Rows(0).Item("mm_spa"), "")
            ws.Cells("AJ3").Value = codUI
            ws.Cells("B6").Value = par.IfNull(dt1.Rows(0).Item("quartiere"), "")
            ws.Cells("N6").Value = par.IfNull(dt1.Rows(0).Item("descr"), "") & " " & par.IfNull(dt1.Rows(0).Item("civico"), "")
            ws.Cells("AJ6").Value = par.IfNull(dt1.Rows(0).Item("sc"), "")
            ws.Cells("AR6").Value = par.IfNull(dt1.Rows(0).Item("pian"), "")
            ws.Cells("AZ6").Value = par.IfNull(dt1.Rows(0).Item("n_ui"), "")
            ws.Cells("BH6").Value = par.IfNull(dt1.Rows(0).Item("s_netta"), 0)
            ...
            par.cmd.CommandText = "select * from siscom_mi.rilievo_alloggio_sfitto where id_unita_immobiliare=" & IDui.Value & " order by data_rilievo desc"
            myReader = par.cmd.ExecuteReader
            If myReader.Read Then
            ...
            '@puccettone xls to pdf
            'Dim wb As New Workbook
            'wb.LoadFromFile(fileName)
            'wb.SaveToFile(Server.MapPath("~\FileTemp\") & NomeFileXls & ".pdf", Spire.Xls.FileFormat.PDF)

            Dim zipfic As String
            Dim NomeFilezip As String = "SCHEDA_" & codUI & "_" & Format(Now, "yyyyMMddHHmmss") & ".zip"
            ...
            Dim sFile As String = "SCHEDA_" & codUI & "_" & Format(Now, "yyyyMMddHHmmss") & formatoFile
    ...
    Protected Sub btnEliminaElemento_Click(sender As Object, e As System.EventArgs) Handles btnEliminaElemento.Click
        Try
            ...
            par.cmd.CommandText = "DELETE FROM SISCOM_MI.RILIEVO_ALLOGGIO_SFITTO WHERE ID=" & LBLID.Value
        ...
        Catch ex As Exception
            connData.chiudi()
            Session.Add("ERRORE", "Provenienza: RisultatiUIRilev - btnEliminaElemento_Click - " & ex.Message)
            Response.Write("<script>top.location.href='../Errore.aspx';</script>")
        End Try
    End Sub
End Class
