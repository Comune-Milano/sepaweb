...
Partial Class Contabilita_Report_RisultatiReportSituazioneEmissioniDett
    ...
    Private Sub caricaDati()
        ...
        Try
            ...
            par.cmd.CommandText = " SELECT ''''||COD_CONTRATTO AS COD_CONTRATTO,INTESTATARIO,NUM_BOLLETTA,INITCAP(BOLLETTAZIONE) AS BOLLETTAZIONE,ANNO_ES_CONTABILE, " _
            ...
            'Session.Item("DT_EMISSIONE") = dTable


            'If dTable.Rows.Count > 0 Then

            '    DataGridEmissioni.CurrentPageIndex = 0
            '    bindData()
    ...
    Private Sub bindData()
        Dim dTable As Data.DataTable = CType(Session.Item("DT_EMISSIONE"), Data.DataTable)
        DataGridEmissioni.DataSource = dTable
        DataGridEmissioni.DataBind()
    End Sub

    Private Function CloneTable(tableToClone As Data.DataTable, countLimit As Integer) As Generic.List(Of Data.DataTable)
        Dim tables As New Generic.List(Of Data.DataTable)()
        
        Dim copyTable As New Data.DataTable
        copyTable = tableToClone.Clone
        Dim contatore As Integer = 0
        For Each dr As Data.DataRow In tableToClone.Rows
            contatore += 1
            copyTable.ImportRow(dr)
            If contatore Mod countLimit = 0 Then
                tables.Add(copyTable)
                copyTable = New Data.DataTable
                copyTable = tableToClone.Clone
            End If
        Next
        If copyTable.Rows.Count > 0 Then
            tables.Add(copyTable)
        End If
    ...
    Protected Sub ImageButtonExcel_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles ImageButtonExcel.Click
        ...
        'Dim nomeFile As String = (DataGridEmissioni, "ExportEmissioni", , , , False)
        ...
        'Dim tables As Generic.List(Of Data.DataTable) = CloneTable(dTable, 100000)

        'Dim listaDt As New Generic.Dictionary(Of String, Data.DataTable)

        'For i As Integer = 1 To tables.Count
        'For i As Integer = 1 To tables.Count - 1
        '    listaDt.Add("Foglio " & i, tables(i - 1))
        'Next
        '' ''Dim nfcsv = par.DataTableALCSV(dTable, "ExportEmissioniCSV", Chr(9), False)
        '' ''If IO.File.Exists(Server.MapPath("..\/..\/FileTemp\/") & nfcsv) Then
        '' ''    Response.Redirect("../../FileTemp/" & nfcsv, False)
        '' ''End If

        Dim xls As New ExcelSiSol
        Dim nomeFile = xls.EsportaExcelDaDT(ExcelSiSol.Estensione.Office2007_xlsx, "ExportEmissioni", "ExportEmissioni", dTable, True)
        If IO.File.Exists(Server.MapPath("..\/..\/FileTemp\/") & nomeFile) Then
            Response.Redirect("../../FileTemp/" & nomeFile, False)
            'ScriptManager.RegisterStartupScript(Me, Me.GetType(), "downloadFile", "avvio(); function avvio() { var altezza = (screen.height/2)-50; var larghezza = (screen.width/2)-100;window.open('../FileTemp/" & nomeFile & "','_blank','top='+altezza+',left='+larghezza+',height=100,width=200');}", True)
        Else
            If nomeFile = "M" Then
                Response.Write("<script>alert('Il file Excel non può essere generato per la mole elevata di dati da estrarre!');</script>")
    ...
    Protected Sub DataGridEmissioni_PageIndexChanged(source As Object, e As System.Web.UI.WebControls.DataGridPageChangedEventArgs) Handles DataGridEmissioni.PageIndexChanged
        If e.NewPageIndex >= 0 Then
            DataGridEmissioni.CurrentPageIndex = e.NewPageIndex
            bindData()
        End If
    End Sub
End Class
