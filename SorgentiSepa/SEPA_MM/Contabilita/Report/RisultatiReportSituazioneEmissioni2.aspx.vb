...
Partial Class Contabilita_Report_RisultatiReportSituazioneEmissioni2
    ...
    Private Sub caricaDati()
        ...
        If Not IsNothing(Request.QueryString("Aggiornamento")) Then
            dataAggiornamento = Request.QueryString("Aggiornamento")
        End If
        Dim condizioneAggiornamento As String = ""
        If dataAggiornamento <> "" Then
            condizioneAggiornamento = " AND DATA_aggiornamento<=NVL('" & dataAggiornamento & "',UID) "
        End If
        ...
        'Dim condizioneCondomini As String = ""
        ...
        '    selectEsercizioContabile = " ' ' AS CAPITOLO,"
        'End If
        ...
        Dim condizioneCondomini As String = ""
        Select Case Request.QueryString("Condomini")
            Case 1
                condizioneCondomini = " AND UNITA_IMMOBILIARI.ID IN (SELECT COND_UI.ID_UI FROM SISCOM_MI.COND_UI,SISCOM_MI.CONDOMINI WHERE CONDOMINI.ID=COND_UI.ID_CONDOMINIO AND CONDOMINI.TIPO_GESTIONE='D') "
            Case 2
                condizioneCondomini = " AND UNITA_IMMOBILIARI.ID IN (SELECT COND_UI.ID_UI FROM SISCOM_MI.COND_UI,SISCOM_MI.CONDOMINI WHERE CONDOMINI.ID=COND_UI.ID_CONDOMINIO)  "
            Case 0
                condizioneCondomini = " AND UNITA_IMMOBILIARI.ID IN (SELECT COND_UI.ID_UI FROM SISCOM_MI.COND_UI,SISCOM_MI.CONDOMINI WHERE CONDOMINI.ID=COND_UI.ID_CONDOMINIO AND CONDOMINI.TIPO_GESTIONE='I') "
            Case -1
                condizioneCondomini = ""
            Case Else
                condizioneCondomini = ""
        ...
        ElseIf Request.QueryString("Accertato") = 1 Then
            condizioneAccertato = " AND BOL_BOLLETTE_VOCI.FL_ACCERTATO=1 "
        ElseIf Request.QueryString("Accertato") = 0 Then
            condizioneAccertato = " AND BOL_BOLLETTE_VOCI.FL_ACCERTATO IS NULL "
        Else
            condizioneAccertato = ""
        End If
        ...
        ElseIf Request.QueryString("VociDaAccertare") = 1 Then
            condizioneVociDaAccertare = " AND T_VOCI_BOLLETTA.ACCERTARE_IN_EMISSIONE = 1 "
        ElseIf Request.QueryString("VociDaAccertare") = 0 Then
            condizioneVociDaAccertare = " AND T_VOCI_BOLLETTA.ACCERTARE_IN_EMISSIONE IS NULL "
        Else
            condizioneVociDaAccertare = ""
        End If
        ...
        Try
            ApriConnessione()
            par.cmd.CommandText = "create table emissioni_tmp" & Format(Now, "yyyyMMddHHmmss") & " as SELECT /*+ INDEX_JOIN(BOL_BOLLETTE) */INITCAP(nvl(TIPO_BOLLETTE.DESCRIZIONE,' ')) AS BOLLETTAZIONE, " _
                ...
                & " TRIM(TO_CHAR(SUM(NVL(IMPORTO,0)-NVL(IMPORTO_ANNULLATO,0)),'999G999G990D99')) AS TOTALE, " _
                & " case when fl_accertato=1 then 'Sì' else 'No' end as accertato " _
                ...
                & " AND TIPO_BOLLETTE.ID=BOL_BOLLETTE.ID_TIPO " _
                ...
                & ",(case when fl_accertato=1 then 'Sì' else 'No' end) " _
                & ") "
            ...
            queryBolBolletteVoci = " SELECT  " _
                & " bol_bollette_voci.id " _
                & " FROM SISCOM_MI.BOL_BOLLETTE, " _
                & " SISCOM_MI.BOL_BOLLETTE_VOCI, " _
                & " SISCOM_MI.T_VOCI_BOLLETTA, " _
                & " SISCOM_MI.T_VOCI_BOLLETTA_GRUPPI, " _
    ...
    Protected Sub ImageButtonStampaEAccerta_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles ImageButtonStampaAccerta.Click
        Dim Loading As String = "<div id=""divLoading5"" Style=""position:absolute;margin: 0px; width: 100%; height: 100%;" _
            & "top: 0px; left: 0px;background-color: #ffffff;z-index:1000;"">" _
            & "<div style=""position: absolute; top: 50%; left: 50%; width: 234px; height: 97px; margin-left: -117px;" _
            & "margin-top: -48px; background-image: url('../../NuoveImm/sfondo.png');"">" _
        ...
        Try
            ...
            par.cmd.ExecuteNonQuery()
            par.cmd.CommandText = "update(select bol_bollette_voci.fl_accertato from siscom_mi.bol_bollette_voci,voci_da_acc_" & univoco & " where bol_bollette_voci.id=voci_da_acc_" & univoco & ".id_voce) a set a.fl_accertato=1 "
            par.cmd.ExecuteNonQuery()
            ...
            For Each riga As DataGridItem In DataGridEmissioni.Items
                If riga.Cells(13).Text = "No" Then
                    riga.Cells(13).Text = "Sì"
                End If
            Next
