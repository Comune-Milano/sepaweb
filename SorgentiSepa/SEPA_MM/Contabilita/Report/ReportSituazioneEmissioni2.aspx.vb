...
Partial Class Contabilita_Report_ReportSituazioneEmissioni2
    ...
    Protected Sub ImageButtonAvviaRicerca_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles ImageButtonAvviaRicerca.Click
        ...
        ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), "msg", "window.open('RisultatiReportSituazioneEmissioni2.aspx" & parametriDaPassare & "','_blank','');", True)
    End Sub
        ...
        Try
            ...
            If competenzeSelezionate = "" Then
                If CategorieSelezionate = "" And macroCategorieSelezionate = "" Then
                    par.cmd.CommandText = "SELECT ID, DESCRIZIONE FROM SISCOM_MI.T_VOCI_BOLLETTA ORDER BY DESCRIZIONE"
                ElseIf CategorieSelezionate = "" And macroCategorieSelezionate <> "" Then
                    par.cmd.CommandText = "SELECT ID, DESCRIZIONE FROM SISCOM_MI.T_VOCI_BOLLETTA WHERE GRUPPO IN(" & macroCategorieSelezionate & ") ORDER BY DESCRIZIONE"
                ElseIf CategorieSelezionate <> "" And macroCategorieSelezionate = "" Then
                    par.cmd.CommandText = "SELECT ID, DESCRIZIONE FROM SISCOM_MI.T_VOCI_BOLLETTA WHERE TIPO_VOCE IN(" & CategorieSelezionate & ") ORDER BY DESCRIZIONE"
                ElseIf CategorieSelezionate <> "" And macroCategorieSelezionate <> "" Then
                    par.cmd.CommandText = "SELECT ID, DESCRIZIONE FROM SISCOM_MI.T_VOCI_BOLLETTA WHERE TIPO_VOCE IN(" & CategorieSelezionate & ") AND GRUPPO IN(" & macroCategorieSelezionate & ") ORDER BY DESCRIZIONE"
                End If
            Else
                If CategorieSelezionate = "" And macroCategorieSelezionate = "" Then
                    par.cmd.CommandText = "SELECT ID, DESCRIZIONE FROM SISCOM_MI.T_VOCI_BOLLETTA WHERE COMPETENZA IN (" & competenzeSelezionate & ") ORDER BY DESCRIZIONE"
                ElseIf CategorieSelezionate = "" And macroCategorieSelezionate <> "" Then
                    par.cmd.CommandText = "SELECT ID, DESCRIZIONE FROM SISCOM_MI.T_VOCI_BOLLETTA WHERE GRUPPO IN(" & macroCategorieSelezionate & ") AND COMPETENZA IN (" & competenzeSelezionate & ") ORDER BY DESCRIZIONE"
                ElseIf CategorieSelezionate <> "" And macroCategorieSelezionate = "" Then
                    par.cmd.CommandText = "SELECT ID, DESCRIZIONE FROM SISCOM_MI.T_VOCI_BOLLETTA WHERE TIPO_VOCE IN(" & CategorieSelezionate & ") AND COMPETENZA IN (" & competenzeSelezionate & ") ORDER BY DESCRIZIONE"
                ElseIf CategorieSelezionate <> "" And macroCategorieSelezionate <> "" Then
                    par.cmd.CommandText = "SELECT ID, DESCRIZIONE FROM SISCOM_MI.T_VOCI_BOLLETTA WHERE TIPO_VOCE IN(" & CategorieSelezionate & ") AND GRUPPO IN(" & macroCategorieSelezionate & ") AND COMPETENZA IN (" & competenzeSelezionate & ") ORDER BY DESCRIZIONE"
                End If
            End If
            Dim dataAdapter As New Oracle.DataAccess.Client.OracleDataAdapter(par.cmd)
            Dim dataTable As New Data.DataTable
    ...
    Private Sub caricaEsercizioContabile()
        Try
            ApriConnessione()
            par.cmd.CommandText = "SELECT to_char(to_date(VALIDITA_DA,'yyyyMMdd'),'dd/MM/yyyy')||' - '||to_char(to_date(VALIDITA_A,'yyyyMMdd'),'dd/MM/yyyy') AS VALIDITA,ANNO FROM SISCOM_MI.BOL_BOLLETTE_ES_CONTABILE order by 2"
            Dim dataAdapter As New Oracle.DataAccess.Client.OracleDataAdapter(par.cmd)
            Dim dataTable As New Data.DataTable
            dataAdapter.Fill(dataTable)
            chiudiConnessione()
            If dataTable.Rows.Count > 0 Then
                ...
                CheckBoxListEserciziContabili.DataValueField = "ANNO"
