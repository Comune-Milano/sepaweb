...
Partial Class Contratti_ElencoAllegati
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            par.cmd.CommandText = " SELECT anagrafica.id,CASE WHEN anagrafica.ragione_sociale is not null THEN ragione_sociale  ELSE RTRIM(LTRIM(COGNOME ||' ' ||NOME)) END AS ""INTESTATARIO"" " _
                                & " FROM SISCOM_MI.ANAGRAFICA,SISCOM_MI.SOGGETTI_CONTRATTUALI,siscom_mi.rapporti_utenza " _
                                & " WHERE rapporti_utenza.cod_CONTRATTO='" & Request.QueryString("cod") _
                                & "'  and SOGGETTI_CONTRATTUALI.id_contratto=rapporti_utenza.id " _
                                & "   and SOGGETTI_CONTRATTUALI.COD_TIPOLOGIA_OCCUPANTE='INTE' " _
                                & "   and ANAGRAFICA.ID=SOGGETTI_CONTRATTUALI.ID_ANAGRAFICA"
            ...
            par.cmd.CommandText = "select tipo_livello_piano.descrizione as miopiano, UNITA_IMMOBILIARI.interno, " _
                                      & " indirizzi.cap,comuni_nazioni.nome as comune,indirizzi.descrizione,indirizzi.civico,indirizzi.cap,SCALE_EDIFICI.DESCRIZIONE as SCALA " _
            & " from SISCOM_MI.tipo_livello_piano, SEPA.comuni_nazioni, SISCOM_MI.indirizzi, SISCOM_MI.UNITA_IMMOBILIARI, SISCOM_MI.edifici,SISCOM_MI.SCALE_EDIFICI " _
            & " where edifici.id = unita_immobiliari.id_edificio " _
            & "   and unita_immobiliari.cod_tipo_livello_piano=tipo_livello_piano.cod (+) " _
            & "   and indirizzi.cod_comune=comuni_nazioni.cod (+) " _
            & "   and unita_immobiliari.id_indirizzo=indirizzi.id (+) " _
            & "   and unita_immobiliari.ID_SCALA=SCALE_EDIFICI.ID (+) " _
            & "   and unita_immobiliari.cod_unita_immobiliare='" & Mid(Request.QueryString("cod"), 1, 17) & "'"
            ...
            If i > 0 Then
                For j = 0 To i - 1
                    ...
                    If erroreTrasferimento = False Then
                        MiaSHTML = MiaSHTML & "<td width='100px' bgcolor='" & MIOCOLORE & "'><font face='Arial' size='1'><a  alt='Download' href='../ALLEGATI/CONTRATTI/" & RicavaFile(ElencoFile(j)) & "' target='_blank'><img src='../ImmMaschere/MenuTopDownload.gif' border='0'></a></font></td>" & vbCrLf
                        If Request.QueryString("LT") <> "1" Then
