...
Partial Class ANAUT_com_patrimonioI
    ...
    Private Sub ScriviPatrImmobiliare()
        Try
            ...
            If distanzaKm = 0 Then
                If cmbTipo.SelectedValue = "0" Then
                    If cmbTipoImm.SelectedValue >= 0 And cmbTipoImm.SelectedValue < 11 Then
                        ScriptManager.RegisterStartupScript(Page, GetType(Page), "MyScriptKeyA", "alert('Attenzione...per il comune selezionato non è stata specificata la distanza in km. L\'eventuale calcolo del canone potrebbe essere ERRATO. Contattare il responsabile!');", True)
                    End If
                End If
            End If

            If txtOperazione.Text = "0" Then
                par.cmd.CommandText = "INSERT INTO COMP_PATR_IMMOB_VSA (ID, ID_COMPONENTE, ID_TIPO, PERC_PATR_IMMOBILIARE, VALORE, MUTUO, F_RESIDENZA, CAT_CATASTALE, COMUNE, N_VANI, SUP_UTILE, FL_70KM, REND_CATAST_DOMINICALE, ID_TIPO_PROPRIETA,DISTANZA_KM) " _
                    & " VALUES (SEQ_COMP_PATR_IMMOB_VSA.NEXTVAL," & cmbComponente.SelectedValue & "," & cmbTipo.SelectedValue & "," & txtPerc.Text & "," & txtValore.Text & "," & TxtMutuo.Text & ",'0','" & cmbTipoImm.SelectedItem.Text & "','" & par.PulisciStrSql(cmbComune.SelectedItem.Text) & "','" & txtNumVani.Text & "','" & txtSupUtile.Text & "','" & FL_70KM & "'," & txtRendita.Text & "," & tipoPropr & "," & par.VirgoleInPunti(distanzaKm) & ") "
                par.cmd.ExecuteNonQuery()
            Else
                par.cmd.CommandText = "SELECT * FROM COMP_PATR_IMMOB_VSA WHERE ID=" & txtRiga.Text
                ...
                If myReaderI.Read Then
                    par.cmd.CommandText = "UPDATE COMP_PATR_IMMOB_VSA" _
                        ...
                        & " PERC_PATR_IMMOBILIARE=" & txtPerc.Text & "," _
                        ...
                        & " DISTANZA_KM=" & par.VirgoleInPunti(distanzaKm) & "" _
            ...
            ScriptManager.RegisterStartupScript(Page, GetType(Page), "MyScriptKeyC", "CloseModal(" & salvaPatrImmob.Value & ");", True)
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Catch ex As Exception
            ...
            If Not IsNothing(par.myTrans) Then
                par.myTrans.Rollback()
            End If
            Session.Add("ERRORE", "Provenienza:" & Page.Title & "<br/>- LoadPatrImmob" & ex.Message)
            Response.Write("<script>top.location.href='../../Errore.aspx';</script>")
            Oracle.DataAccess.Client.OracleConnection.ClearAllPools()
        End Try
    End Sub
