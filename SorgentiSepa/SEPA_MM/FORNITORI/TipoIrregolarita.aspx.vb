...
Partial Class FORNITORI_Default
    ...
    Private Sub BindGrid()
        ...
        sStrSql = "SELECT ID,DESCRIZIONE FROM SISCOM_MI.SEGNALAZIONI_FO_TIPI_IRR WHERE UTILIZZABILE=1 ORDER BY DESCRIZIONE ASC"
    End Sub
    ...
    Protected Sub RadGridTipi_BatchEditCommand(sender As Object, e As Telerik.Web.UI.GridBatchEditingEventArgs) Handles RadGridTipi.BatchEditCommand
            ...
            If TotaleOperazioni > 0 Then
                ...
                For Each command As GridBatchEditingCommand In e.Commands
                    ...
                    If command.Type = GridBatchEditingCommandType.Insert Then
                        ...
                        par.cmd.CommandText = "INSERT INTO SISCOM_MI.SEGNALAZIONI_FO_TIPI_IRR (ID,DESCRIZIONE) VALUES " _
                                            & "(SISCOM_MI.SEQ_SEGNALAZIONI_FO_TIPI_IRR.NEXTVAL, " & par.insDbValue(DescrizioneCompleta, True) & ")"
                    ...
                    ElseIf command.Type = GridBatchEditingCommandType.Update Then
                        Dim DescrizioneCompleta As String = UCase(newValues("DESCRIZIONE"))
                        par.cmd.CommandText = "UPDATE SISCOM_MI.SEGNALAZIONI_FO_TIPI_IRR SET DESCRIZIONE = " & par.insDbValue(newValues("DESCRIZIONE"), True) & " " _
                    ...
                    ElseIf command.Type = GridBatchEditingCommandType.Delete Then
                        Try
                            par.cmd.CommandText = "DELETE FROM SISCOM_MI.SEGNALAZIONI_FO_TIPI_IRR WHERE ID = " & par.insDbValue(newValues("ID"), False)
                ...
                CType(Master.FindControl("RadWindowManager1"), RadWindowManager).RadAlert("Operazione effettuata" & "<br/>" _
                                       & "Elementi inseriti: " & NrInsert & ",<br/> " _
                                       & "Elementi modificati: " & NrUpdate & ",<br/> " _
                                       & "Elementi eliminato: " & NrDelete & ",<br/> " _
                                       & "Elementi non eliminati perchè utilizzati: " & NrDeleteNo & ".", 300, 150, "Info", Nothing, Nothing)
        ...
        Catch ex As Exception
            ...
            Session.Add("ERRORE", "Provenienza: Fornitori - GestTipiIrregolarita - BatchEditCommand - " & ex.Message)
    ...
    Protected Sub RadGridTipi_ItemCommand(sender As Object, e As Telerik.Web.UI.GridCommandEventArgs) Handles RadGridTipi.ItemCommand
        If UCase(e.CommandName) = "REBINDGRID" Then
            ...
            ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType, "Naviga", "validNavigation = true;", True)
        End If
        If UCase(e.CommandName) = "INITINSERT" Then
            Modificato.Value = "0"
            ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType, "Naviga", "validNavigation = true;", True)
        End If
    End Sub
