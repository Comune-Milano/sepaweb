...
Partial Class CICLO_PASSIVO_CicloPassivo_APPALTI_Tab_VaImporti
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            'par.caricaComboBox("SELECT ID, DESCRIZIONE FROM SISCOM_MI.PF_VOCI_IMPORTO WHERE ID IN (SELECT ID_PF_VOCE_IMPORTO FROM SISCOM_MI.APPALTI_LOTTI_SERVIZI WHERE ID_APPALTO = " & CType(Me.Page, Object).vIdAppalti & ") ORDER BY DESCRIZIONE", ddlVoceServizio, "ID", "DESCRIZIONE", True)
            'CaricaImpServizi()
            CaricaVarServizi()
    ...
    Public Sub CaricaVarServizi()
        Try
            ...
            par.cmd.CommandText = "SELECT DATA_ORA_OP, TRIM (TO_CHAR (IMPORTO_CONSUMO, '9G999G999G999G999G990D99')) AS IMPORTO_CONSUMO, " _
                                ...
                                & " FROM SISCOM_MI.APPALTI_LOTTI_SERVIZI_S " _
            ...
            '                    & "(SELECT PF_VOCI_IMPORTO.descrizione FROM siscom_mi.PF_VOCI_IMPORTO WHERE ID = id_pf_voce_importo) AS DESC_VOCE, " _
    ...
    '                        par.cmd.CommandText = "INSERT INTO SISCOM_MI.APPALTI_VARIAZIONI_IMPORTI (ID_VARIAZIONE,ID_PF_VOCE_IMPORTO,IMPORTO,PERCENTUALE, ID_GRUPPO_VARIAZIONE) VALUES (" & IdAppVariazione _
    ...
    '                                            & "SET    ID_PF_VOCE_IMPORTO 		= " & IdVoceServ & ", " _
        ...
        Try
            ...
            If Not IsNothing(txtData.SelectedDate) And Not String.IsNullOrEmpty(txtImportoVar.Text.ToString) Then
                ...
                Else
                    ...
                    If tipologia = 5 Then
                        par.cmd.CommandText = "SELECT SUM ((Importo_canone - (Importo_canone*(sconto_canone/100))))  AS ImpCont, sum(ONERI_SICUREZZA_CANONE) as sconto FROM  siscom_mi.APPALTI_LOTTI_SERVIZI WHERE id_appalto = " & CType(Me.Page, Object).vIdAppalti
                    ElseIf tipologia = 6 Then
                        par.cmd.CommandText = "SELECT SUM ((Importo_CONSUMO - (Importo_CONSUMO*(sconto_consumo/100))))  AS ImpCont, sum(ONERI_SICUREZZA_CONSUMO) as sconto FROM  siscom_mi.APPALTI_LOTTI_SERVIZI WHERE id_appalto = " & CType(Me.Page, Object).vIdAppalti
                    End If
                    Dim ImpContSenzaIva As Decimal = 0
                    ...
                    If ImpContSenzaIva > 0 Then
                        ...
                        Dim percOneri As String = ""
                        If tipologia = 5 Then
                            par.cmd.CommandText = "SELECT SUM(IMPORTO_CANONE), SUM(ONERI_SICUREZZA_CANONE) FROM SISCOM_MI.APPALTI_LOTTI_SERVIZI WHERE ID_APPALTO = " & CType(Me.Page, Object).vIdAppalti
                            tipoImporto = "IMPORTO_CANONE"
                            tipoOneri = "ONERI_SICUREZZA_CANONE"
                            'percOneri = "PERC_ONERI_SIC_CAN"
                        Else
                            par.cmd.CommandText = "SELECT SUM(IMPORTO_CONSUMO), SUM(ONERI_SICUREZZA_CONSUMO) FROM SISCOM_MI.APPALTI_LOTTI_SERVIZI WHERE ID_APPALTO = " & CType(Me.Page, Object).vIdAppalti
                            tipoImporto = "IMPORTO_CONSUMO"
                            tipoOneri = "ONERI_SICUREZZA_CONSUMO"
                            'percOneri = "PERC_ONERI_SIC_CON"
                        End If
                        myReader = par.cmd.ExecuteReader
                        If myReader.Read Then
                        ...
                        End If
                        myReader.Close()
                        par.cmd.CommandText = "SELECT ID_PF_VOCE_IMPORTO FROM SISCOM_mI.APPALTI_LOTTI_SERVIZI WHERE ID_APPALTO = " & CType(Me.Page, Object).vIdAppalti
                        ...
                        For Each rigaImporto As Data.DataRow In dtPfVociImporto.Rows
                            par.cmd.CommandText = "select id_appalto, id_pf_voce_importo from SISCOM_MI.appalti_lotti_Servizi where id_appalto in " _
                                    ...
                                    & "select id from SISCOM_MI.pf_voci_importo connect by prior id=id_old " _
                                    & "start with id= " & rigaImporto.Item("ID_PF_VOCE_IMPORTO").ToString _
                                    & "union " _
                                    & "select id from SISCOM_MI.pf_voci_importo connect by prior id_old=id " _
                                    & "start with id=" & rigaImporto.Item("ID_PF_VOCE_IMPORTO").ToString & ") "
                            ...
                            For Each riga1 As Data.DataRow In dtVoci.Rows
                                If sconto > 0 Then
                                    par.cmd.CommandText = "UPDATE SISCOM_MI.APPALTI_LOTTI_SERVIZI SET " _
                                                   & tipoOneri & "  = " & tipoOneri & " + round((" & par.VirgoleInPunti(par.IfEmpty(Me.txtImportoOneri.Text, 0)) & " / " & par.VirgoleInPunti(sommaOneri) & " * " & tipoOneri & "),2), " _
                                                    ...
                                                    & " AND ID_PF_VOCE_IMPORTO = " & riga1.Item("ID_PF_VOCE_IMPORTO") _
                                                    & " AND FL_MANUALE = 0"
                                    par.cmd.ExecuteNonQuery()
                                ...
                                Else
                                    ...
                                    par.cmd.ExecuteNonQuery()
                                End If
                                par.cmd.CommandText = "UPDATE SISCOM_MI.APPALTI_LOTTI_SERVIZI " _
                                                    & " SET PERC_ONERI_SIC_CAN = (CASE WHEN ONERI_SICUREZZA_CANONE  <> 0 THEN ROUND((ONERI_SICUREZZA_CANONE / IMPORTO_CANONE) * 100, 4) ELSE 0 END), " _
                                                    & " PERC_ONERI_SIC_CON = (CASE WHEN ONERI_SICUREZZA_CONSUMO <> 0 THEN ROUND((ONERI_SICUREZZA_CONSUMO / IMPORTO_CONSUMO) * 100, 4) ELSE 0 END) " _
                                                    & " WHERE ID_APPALTO = " & riga1.Item("ID_APPALTO") _
                                                    & " AND FL_MANUALE = 0"
                                par.cmd.ExecuteNonQuery()
                            Next
                        ...
                        par.cmd.ExecuteNonQuery()
                        Dim t = CType(Me.Page.FindControl("Tab_Servizio"), Object)
                        CType(t.FindControl("DataGrid3"), RadGrid).Rebind()
                        CType(Me.Page.FindControl("Tab_Servizio"), Object).BindGrid_Servizi()
                        CType(Me.Page.FindControl("Tab_Servizio"), Object).somma()
                        CType(Me.Page.FindControl("Tab_Servizio"), Object).CalcolaImpContrattuale()
                        CType(Me.Page.FindControl("Tab_Servizio"), Object).CalcolaResiduo()
                        par.cmd.CommandText = "UPDATE SISCOM_MI.APPALTI SET " _
                                            & " TOT_CANONE = " & par.VirgoleInPunti(par.IfEmpty(DirectCast(CType(Me.Page.FindControl("Tab_Appalto_generale"), Object).FindControl("txtImpTotPlusOneriCan"), TextBox).Text.Replace(".", ""), 0)) _
                                            & ", TOT_CONSUMO = " & par.VirgoleInPunti(par.IfEmpty(DirectCast(CType(Me.Page.FindControl("Tab_Appalto_generale"), Object).FindControl("txtImpTotPlusOneriCons"), TextBox).Text.Replace(".", ""), 0)) _
        ...
        If dtAppalti.Rows.Count > 1 Then
            ...
            For Each rVoci As Data.DataRow In dtAppalti.Rows
                par.cmd.CommandText = "select id_appalto, id_pf_voce_importo from SISCOM_MI.appalti_lotti_Servizi where id_appalto in " _
                                    ...
                                    & "and id_pf_Voce_importo in ( " _
                                    & "select id from SISCOM_MI.pf_voci_importo connect by prior id=id_old " _
                                    & "start with id= " & rVoci.Item("ID_PF_VOCE_IMPORTO") _
                                    & "union " _
                                    & "select id from SISCOM_MI.pf_voci_importo connect by prior id_old=id " _
                                    & "start with id=" & rVoci.Item("ID_PF_VOCE_IMPORTO") & ") "
                Dim da As New Oracle.DataAccess.Client.OracleDataAdapter(par.cmd)
                ...
                If dtInsert1.Rows.Count > 0 Then
                    ...
                    For Each rInsert As Data.DataRow In dtInsert1.Rows
                        If rInsert.Item("id_pf_voce_importo") <> rVoci.Item("id_pf_voce_importo") Then
                            If idappaltoIns <> par.IfEmpty(rInsert.Item("id_appalto").ToString, 0) Then
                                ...
                                '    par.cmd.ExecuteNonQuery()
                                '    par.cmd.CommandText = "INSERT INTO SISCOM_MI.APPALTI_VARIAZIONI_IMPORTI (ID_VARIAZIONE,ID_PF_VOCE_IMPORTO,IMPORTO,PERCENTUALE, id_gruppo_variazione) VALUES (" & IdAppVariazione _
                                '                & ", " & rInsert.Item("id_pf_voce_importo") & "," & par.VirgoleInPunti(rVoci.Item("importo")) & "," & par.IfEmpty(par.VirgoleInPunti(rVoci.Item("percentuale")), "null") & "," & HFidGruppoVariazione.Value & ")"
                                '    par.cmd.ExecuteNonQuery()
                                'Else
                                '    'UPDATE
                                '    par.cmd.CommandText = "UPDATE SISCOM_MI.APPALTI_VARIAZIONI " _
                                ...
                                '    Dim idApp As String = par.cmd.ExecuteScalar.ToString
                                '    par.cmd.CommandText = "UPDATE SISCOM_MI.APPALTI_VARIAZIONI_IMPORTI " _
                                '                        & "SET    ID_PF_VOCE_IMPORTO 		        = " & rInsert.Item("id_pf_voce_importo") & ", " _
    ...
    Private Sub CaricaImporti()
        Try
            ...
            par.cmd.CommandText = "SELECT ID, getdata(DATA_VARIAZIONE) AS DATA_VARIAZIONE,PROVVEDIMENTO, " _
                                & "ID_PF_VOCE_IMPORTO, ID_TIPOLOGIA, ID_TIPO_VARIAZIONE," _
            ...
            If myReader.Read Then
                ...
                'ddlVoceServizio.SelectedValue = par.IfEmpty(myReader("ID_PF_VOCE_IMPORTO").ToString, "")
