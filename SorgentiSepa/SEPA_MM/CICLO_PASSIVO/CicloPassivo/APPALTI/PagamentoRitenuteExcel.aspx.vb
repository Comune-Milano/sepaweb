...
Partial Class PagamentoRitenuteExcel
        ...
        If Not IsNothing(dt) Then
            ...
            For Each riga In dt.Rows
                ...
                If insert <> 0 Then
                    ...
                    Try
                        par.cmd.CommandText = "SELECT ID_PAGAMENTO FROM SISCOM_MI.PRENOTAZIONI WHERE ID =(SELECT ID_PRENOTAZIONE_RIT FROM SISCOM_MI.ODL WHERE ID_PAGAMENTO=(SELECT ID FROM SISCOM_MI.PAGAMENTI WHERE ANNO='" & ANNOCDP & "' AND PROGR='" & PROGCDP & "'))"
                        ...
                        If IDPagamento > 0 Then
                            par.cmd.CommandText = "SELECT ROUND(SUM(IMPORTO_APPROVATO),2) FROM SISCOM_MI.PRENOTAZIONI WHERE ID_PAGAMENTO= " & IDPagamento & "AND ID_VOCE_PF=" & ID_VocePF
                ...
                If insert <> 0 Then
                    ...
                    Else
                        If Trim(data_pag) = "" Then
                            par.cmd.CommandText = "SELECT ID_PAGAMENTO FROM SISCOM_MI.PAGAMENTI_LIQUIDATI WHERE ID_PAGAMENTO='" & IDPagamento & "' AND NUM_MANDATO='" & MAE & "' AND ANNO_MANDATO='" & annoMAE & "' AND DATA_MANDATO IS NULL AND IMPORTO='" & importo & "' AND ID_VOCE_PF='" & ID_VocePF & "'"
                        Else
                            par.cmd.CommandText = "SELECT ID_PAGAMENTO FROM SISCOM_MI.PAGAMENTI_LIQUIDATI " _
                                & "WHERE ID_PAGAMENTO='" & IDPagamento & "' " _
                        ...
                        Else
                            Try
                                ...
                                'par.cmd.CommandText = "SELECT SUM(IMPORTO) FROM SISCOM_MI.PAGAMENTI_LIQUIDATI WHERE ID_PAGAMENTO='" & IDPagamento & "'"
                                par.cmd.CommandText = "SELECT SUM(IMPORTO) FROM SISCOM_MI.PAGAMENTI_LIQUIDATI WHERE ID_PAGAMENTO='" & IDPagamento & "' AND ID_VOCE_PF=" & ID_VocePF
                                ...
                                If importoParziale + importo <= importoConsuntivato + tolleranza Then
                                    ...
                                    par.cmd.CommandText = "INSERT INTO SISCOM_MI.PAGAMENTI_LIQUIDATI(ID,ID_PAGAMENTO,NUM_MANDATO,ANNO_MANDATO,DATA_MANDATO,IMPORTO,ID_VOCE_PF) VALUES (SISCOM_MI.SEQ_PAGAMENTI_LIQUIDATI.NEXTVAL,'" & IDPagamento & "','" & MAE & "','" & annoMAE & "','" & data_pag & "','" & importo & "','" & ID_VocePF & "')"
            ...
            par.cmd.CommandText = " UPDATE SISCOM_MI.PRENOTAZIONI SET IMPORTO_LIQUIDATO = IMPORTO_APPROVATO - NVL (RIT_LEGGE_IVATA, 0) " _
                & " WHERE ID_PAGAMENTO IN (SELECT PAGAMENTI.ID FROM SISCOM_MI.PAGAMENTI WHERE ABS((SELECT SUM (IMPORTO) FROM SISCOM_MI.PAGAMENTI_LIQUIDATI WHERE ID_PAGAMENTO = PAGAMENTI.ID) -IMPORTO_CONSUNTIVATO)<1.5) " _
                & " AND IMPORTO_LIQUIDATO IS NULL "
            par.cmd.ExecuteNonQuery()

            par.cmd.CommandText = "UPDATE  SISCOM_MI.PAGAMENTI_LIQUIDATI SET " _
                & " ID_STRUTTURA=(SELECT DISTINCT ID_STRUTTURA FROM SISCOM_MI.PRENOTAZIONI WHERE PRENOTAZIONI.ID_PAGAMENTO=PAGAMENTI_LIQUIDATI.ID_PAGAMENTO AND PRENOTAZIONI.ID_VOCE_PF=PAGAMENTI_LIQUIDATI.ID_VOCE_PF) " _
