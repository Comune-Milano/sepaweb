...
Partial Class CICLO_PASSIVO_CicloPassivo_APPALTI_Tab_VariazLavNPl
    ...
    Public Sub CaricaImpLavori()
        Try
            ...
            par.cmd.CommandText = "SELECT ID_PF_VOCE AS ID_VOCE,''AS ID_SERVIZIO," _
                                & " '' AS SERVIZIO,(pf_voci.descrizione) AS DESCRIZIONE, " _
                                & " '' AS IMPORTO,((IMPORTO_CANONE*NVL(SCONTO_CANONE,0))/100) AS SCONTO_CANONE," _
                                & " ((IMPORTO_CONSUMO*NVL(SCONTO_CONSUMO,0))/100) AS SCONTO_CONSUMO," _
                                & " (((IMPORTO_CANONE*NVL(SCONTO_CANONE,0))/100)-(SELECT NVL(SUM(IMPORTO),0) FROM SISCOM_MI.APPALTI_VARIAZIONI_IMPORTI,SISCOM_MI.APPALTI_VARIAZIONI  " _
                                & " WHERE APPALTI_VARIAZIONI.ID=APPALTI_VARIAZIONI_IMPORTI.ID_VARIAZIONE AND APPALTI_VARIAZIONI.ID_TIPOLOGIA = 3 AND ID_PF_VOCE_IMPORTO = APPALTI_VOCI_PF.ID_PF_VOCE) ) AS MAXCANONE, " _
                                & " (((IMPORTO_CONSUMO*NVL(SCONTO_CONSUMO,0))/100)-(SELECT NVL(SUM(IMPORTO),0) " _
                                & " FROM SISCOM_MI.APPALTI_VARIAZIONI_IMPORTI,SISCOM_MI.APPALTI_VARIAZIONI " _
                                & " WHERE APPALTI_VARIAZIONI.ID=APPALTI_VARIAZIONI_IMPORTI.ID_VARIAZIONE " _
                                & " AND APPALTI_VARIAZIONI.ID_TIPOLOGIA = 4 " _
                                & " AND ID_PF_VOCE_IMPORTO = APPALTI_VOCI_PF.ID_PF_VOCE) ) AS MAXCONSUMO  " _
                                & " FROM siscom_mi.appalti_voci_pf, siscom_mi.pf_voci" _
                                & " WHERE  pf_voci.ID = ID_PF_VOCE AND id_appalto = " & CType(Me.Page, Object).vIdAppalti
                ...
                If ControllaImporti("CANONE") = True Then
                    ...
                    For i = 0 To Me.DataGridImportiVariaz.Items.Count - 1
                        ...
                        par.cmd.CommandText = "INSERT INTO SISCOM_MI.APPALTI_VARIAZIONI_IMPORTI (ID_VARIAZIONE,ID_PF_VOCE,IMPORTO,PERCENTUALE) VALUES (" & IdAppVariazione _
                            & "," & IdVoceServ & "," & par.VirgoleInPunti(par.IfEmpty(DirectCast(di.Cells(4).FindControl("txtPercVariazione"), TextBox).Text, 0)) & "," & par.VirgoleInPunti(Percentuale) & ")"
                        par.cmd.ExecuteNonQuery()
