...
Partial Class Tab_Manu_Consuntivo
    ...
    Function ControlloCampiConsuntivo() As Boolean
        ...
        If par.IfEmpty(Me.txtCodArticolo.Text, "Null") = "Null" Then
            Response.Write("<script>alert('Inserire il codice articolo!');</script>")
        ...
        If par.IfEmpty(Me.txtDescrizione.Text, "Null") = "Null" Then
            Response.Write("<script>alert('Inserire la descrizione articolo!');</script>")
            ControlloCampiConsuntivo = False
            txtCodArticolo.Focus()
            Exit Function
        End If
        ...
        If Me.txtCodArticolo.Text = "RIMBORSO OPERE SPECIALISTICHE" Then
            If par.IfEmpty(Me.txtTotale.Text, "Null") = "Null" Then
                Response.Write("<script>alert('Inserire il prezzo imponibile con IVA inclusa!');</script>")
                ControlloCampiConsuntivo = False
                Me.txtTotale.Focus()
                Exit Function
            End If
        Else
            If par.IfEmpty(Me.txtPrezzo.Text, "Null") = "Null" Then
                Response.Write("<script>alert('Inserire il prezzo unitario!');</script>")
                ControlloCampiConsuntivo = False
                txtPrezzo.Focus()
                Exit Function
            End If
        ...
        SommaTot1 = CalcolaImportiControllo(SommaTot, par.IfEmpty(CType(Me.Page.FindControl("txtPercOneri"), HiddenField).Value, 0), par.IfEmpty(CType(Me.Page.FindControl("txtScontoConsumo"), HiddenField).Value, 0), par.IfEmpty(CType(Me.Page.FindControl("txtPercIVA"), HiddenField).Value, 0), par.IfEmpty(CType(Me.Page.FindControl("txtFL_RIT_LEGGE"), HiddenField).Value, 0))

        'In caso di rimborsi, non viene applicato lo sconto ecc
        SommaTot1 = SommaTot1 + SommaTotRimborsi

        If Math.Round(SommaTot1, 5) > Math.Round(Ricalcola_ImportoResiduo(), 5) Then
            'If SommaTot1 > (Decimal.Parse(par.IfEmpty(Me.txtResiduoConsumo.Value, "0")) + TotalePrezzoPreventivato()) Then
    ...
    Sub SettaMisure()
        ...
        Try
            ...
            If myReaderTMP1.Read Then
                ...
                Me.txtPercIVA.Value = par.IfNull(myReaderTMP1("IVA_CONSUMO"), -1)           'MANUTENZIONI
    ...
    Private Function Ricalcola_ImportoResiduo() As Decimal
        ...
        Try
            ...
            sStr1 = "select * from SISCOM_MI.APPALTI_VOCI_PF " _
                 ...
                 & "   and ID_PF_VOCE in (select ID from SISCOM_MI.PF_VOCI where ID_VOCE_MADRE=(select ID_VOCE_MADRE from SISCOM_MI.PF_VOCI where ID=" & vIdPF_VOCE & ")) "
            ...
            sStr1 = "select SUM(IMPORTO_TOT) from SISCOM_MI.MANUTENZIONI " _
                 ...
                 & "   and ID_PF_VOCE in (select ID from SISCOM_MI.PF_VOCI where ID_VOCE_MADRE=(select ID_VOCE_MADRE from SISCOM_MI.PF_VOCI where ID=" & vIdPF_VOCE & ")) " _
                 & "   and STATO in (1,2,4)"
