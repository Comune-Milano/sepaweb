Partial Class EventiVariazioni
    ...
    Private Sub CaricaStrutture()
        ...
        Try
            ...
            If Session.Item("BP_VARIAZIONI") = 1 Then
                ...
                While myReader.Read
                    ddlStruttura.Items.Add(New ListItem(par.IfNull(myReader("NOME"), ""), par.IfNull(myReader("ID"), "")))
                End While
                myReader.Close()

                'SELEZIONO LA STRUTTURA DI APPARTENENZA DELL'OPERATORE
                ddlStruttura.SelectedValue = Session.Item("ID_STRUTTURA")
    ...
    Protected Sub ddlStruttura_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlStruttura.SelectedIndexChanged

        'MOSTRO GLI EVENTI PER LA STRUTTURA SELEZIONATA
        caricaEventi()
    ...
    Protected Sub caricaEventi()
        Try
            ...
            If par.OracleConn.State = Data.ConnectionState.Closed Then
                par.OracleConn.Open()
            End If
            Dim STRINGASQL As String = "SELECT DISTINCT SEPA.OPERATORI.OPERATORE,TO_DATE(SISCOM_MI.EVENTI_VARIAZIONI.DATA_ORA_EVENTO,'yyyyMMddHH24MISS') AS DATA_ORA_EVENTO," _
                                       & "(SELECT SISCOM_MI.TAB_EVENTI.DESCRIZIONE " _
                                       & "FROM SISCOM_MI.TAB_EVENTI " _
                                       & "WHERE SISCOM_MI.TAB_EVENTI.COD=SISCOM_MI.EVENTI_VARIAZIONI.COD_EVENTO) AS EVENTO," _
                                       & "SISCOM_MI.EVENTI_VARIAZIONI.MOTIVAZIONE," _
                                       & "(SELECT SISCOM_MI.PF_VOCI.CODICE || ' - ' || SISCOM_MI.PF_VOCI.DESCRIZIONE " _
                                       & "FROM SISCOM_MI.PF_VOCI WHERE SISCOM_MI.PF_VOCI.ID=SISCOM_MI.EVENTI_VARIAZIONI.ID_VOCE_DA) AS VOCE_DA," _
                                       & "(SELECT SISCOM_MI.PF_VOCI.CODICE || ' - ' || SISCOM_MI.PF_VOCI.DESCRIZIONE FROM SISCOM_MI.PF_VOCI " _
                                       & "WHERE SISCOM_MI.PF_VOCI.ID=SISCOM_MI.EVENTI_VARIAZIONI.ID_VOCE_A) AS VOCE_A," _
                                       & "TRIM(TO_CHAR(SISCOM_MI.EVENTI_VARIAZIONI.IMPORTO,'9G999G999G990D99')) AS IMPORTO " _
                                       & "FROM SISCOM_MI.EVENTI_VARIAZIONI,SEPA.OPERATORI,SISCOM_MI.PF_VOCI,SISCOM_MI.PF_MAIN,SISCOM_MI.PF_VOCI A " _
                                       & "WHERE SISCOM_MI.EVENTI_VARIAZIONI.ID_OPERATORE=SEPA.OPERATORI.ID " _
                                       & "AND SISCOM_MI.PF_VOCI.ID_PIANO_FINANZIARIO=SISCOM_MI.PF_MAIN.ID AND SISCOM_MI.PF_MAIN.ID_STATO>='5' " _
                                       & "AND SISCOM_MI.EVENTI_VARIAZIONI.ID_STRUTTURA='" & ddlStruttura.SelectedValue & "' " _
                                       & "AND A.ID_PIANO_FINANZIARIO='" & ddlEsercizio.SelectedValue & "' AND A.ID=EVENTI_VARIAZIONI.ID_VOCE_A " _
            ...
            da.Fill(ds)
            lblTotale.Text = "TOTALE EVENTI VARIAZIONI TROVATI PER LA STRUTTURA SELEZIONATA: " & ds.Rows.Count
            lblTitolo.Text = "EVENTI VARIAZIONI ESERCIZIO FINANZIARIO IN CORSO"
