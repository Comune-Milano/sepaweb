...
Partial Class Contabilita_CicloPassivo_Plan_NuovoLotto
    ...
    Private Sub RiempiPatrimonio(ByVal myReader1 As Oracle.DataAccess.Client.OracleDataReader)
        Try
            If par.IfNull(myReader1("ID_IMPIANTO"), 0) <> 0 Then
                par.cmd.CommandText = "SELECT * FROM SISCOM_MI.IMPIANTI WHERE ID=" & myReader1("ID_IMPIANTO")
        ...
        Try
            ...
            While myReader1.Read
                ...
                If Session.Item("LIVELLO") <> "1" Then
                    ...
                    Select Case par.IfNull(myReader1("ID_TIPO_ST"), "2")
                        Case "0"
                            tipostruttura.Value = " AND IMPIANTI.ID_COMPLESSO IN (SELECT ID FROM SISCOM_MI.COMPLESSI_IMMOBILIARI WHERE ID_FILIALE=" & par.IfNull(myReader1("ID"), -1) & ") "
                        Case "1"
                            tipostruttura.Value = " AND IMPIANTI.ID_COMPLESSO IN (SELECT ID FROM SISCOM_MI.COMPLESSI_IMMOBILIARI WHERE ID_FILIALE IN (SELECT ID FROM SISCOM_MI.TAB_FILIALI WHERE ID_TIPO_ST=0 AND ID_TECNICA=" & par.IfNull(myReader1("ID"), -1) & ")) "
                        Case "2"
                            tipostruttura.Value = ""
                    End Select
            ...
            If cmbTipoImpianto.SelectedItem.Value = "-1" Then
                ...
                par.cmd.CommandText = "SELECT SISCOM_MI.IMPIANTI.ID," _
                                      ...
                                      & "'' AS condominio  " _
                                      & "FROM SISCOM_MI.IMPIANTI WHERE " _
                                      & "IMPIANTI.ID<>1  " & tipostruttura.Value & " AND    " _
                                      & "IMPIANTI.ID NOT IN  (SELECT ID_IMPIANTO FROM siscom_mi.LOTTI_PATRIMONIO,siscom_mi.LOTTI,siscom_mi.LOTTI_SERVIZI WHERE LOTTI.id_esercizio_finanziario=" & idEsercizioF.Value & " AND " _
                                      & "LOTTI_PATRIMONIO.id_lotto=LOTTI.ID AND LOTTI_SERVIZI.id_lotto=LOTTI.ID  AND LOTTI_SERVIZI.id_servizio=" & idServizio.Value _
                                      & "AND LOTTI.ID<>" & idLotto.Value & " AND ID_IMPIANTO IS NOT NULL) " _
            ...
            Else
                ...
                par.cmd.CommandText = "SELECT SISCOM_MI.IMPIANTI.ID," _
                                                    ...
                                                    & " WHERE IMPIANTI.id<>1 " & tipostruttura.Value & " and " _
                                                    & " IMPIANTI.id not in  " _
                                                    & "(select ID_IMPIANTO from siscom_mi.lotti_patrimonio,siscom_mi.lotti,siscom_mi.lotti_servizi where " _
                                                    ...
                                                    & " and lotti_servizi.id_servizio=" & idServizio.Value & " and lotti.id<>" & idLotto.Value & " AND ID_IMPIANTO IS NOT NULL) " _
            ...
            While myReader3.Read
                ...
                lstcomplessi.Items.Add(New ListItem(Replace(Replace(Replace(par.IfNull(myReader3("DENOMINAZIONE"), " ") & par.IfNull(myReader3("CONDOMINIO"), " "), "'", "\'"), vbCr, " "), vbLf, ""), par.IfNull(myReader3("ID"), -1)))
            End While
            myReader3.Close()
    ...
    Function CaricaDati1()
        ...
        Try
            ...
            While myReader3.Read
                ...
                lstcomplessi.Items.Add(New ListItem(Replace(Replace(Replace(par.IfNull(myReader3("DENOMINAZIONE"), " "), "'", "\'"), vbCr, " "), vbLf, ""), par.IfNull(myReader3("ID"), -1)))
            End While
            myReader3.Close()
    ...
    Protected Sub tabcomplessi_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles tabcomplessi.ItemDataBound
        If e.Item.ItemType = ListItemType.Item Then
            ...
            e.Item.Attributes.Add("onclick", "this.style.backgroundColor='red';document.getElementById('txtseledifici').value='Hai selezionato: " & Replace(Replace(e.Item.Cells(1).Text, "'", "\'"), vbCrLf, "") & "';document.getElementById('txtIdComponente').value='" & e.Item.Cells(0).Text & "'")
        End If
    ...
    Sub Inserimento()
        Try
            ...
            If Buono = True Then
                ...
                par.cmd.CommandText = "insert into siscom_mi.lotti (id,ID_FILIALE,TIPO,DESCRIZIONE,ID_ESERCIZIO_FINANZIARIO,COD_TIPO_IMPIANTO) values (siscom_mi.seq_lotti.nextval," & id_filiale & ",'I','" & par.PulisciStrSql(UCase(txtDescrizione.Text)) & "'," & ID_ESERCIZIO_F & ",'" & cmbTipoImpianto.SelectedItem.Value & "')"
            ...
            Else
                ...
                Response.Write("<script>alert('Operazione NON Effettuata! Il problema potrebbe essere:\nEsiste gia un lotto con questa descrizione nell\'esercizio finanziario!\nNessun Impianto inserito nella lista!;');</script>")
            End If
    ...
    Function Aggiorna()
        Try
            ...
            If tabcomplessi.Items.Count > 0 Then
                ...
                par.cmd.CommandText = "UPDATE SISCOM_MI.LOTTI SET COD_TIPO_IMPIANTO='" & cmbTipoImpianto.SelectedItem.Value & "',tipo='I', DESCRIZIONE='" & par.PulisciStrSql(UCase(txtDescrizione.Text)) & "' WHERE ID=" & vId
                par.cmd.ExecuteNonQuery()
                ...
                Response.Write("<script>alert('Operazione Effettuata!');self.close();</script>")
            Else
                Response.Write("<script>alert('Nessun IMPIANTO inserito!');</script>")
            End If
        Catch ex As Exception
            par.OracleConn.Close()
            Oracle.DataAccess.Client.OracleConnection.ClearAllPools()
    ...
    Private Function myReader() As Oracle.DataAccess.Client.OracleDataReader
