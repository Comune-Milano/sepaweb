...
Partial Class CICLO_PASSIVO_CicloPassivo_MANUTENZIONI_ChiusuraSegnalazioni
    ...
    Private Sub caricaComplessi()
        Try
            ...
            End If
            'par.caricaComboBox("SELECT ID,COD_COMPLESSO||' - '||DENOMINAZIONE AS NOME,DENOMINAZIONE FROM SISCOM_MI.COMPLESSI_IMMOBILIARI WHERE ID<>1 " & condizioneSedeTerritoriale & condizioneBM & " ORDER BY DENOMINAZIONE", cmbComplesso, "ID", "NOME", True)
    ...
    Private Sub caricaEdifici()
        Try
            ...
            End If
            'par.caricaComboBox("SELECT ID,COD_EDIFICIO||' - '||DENOMINAZIONE AS NOME,DENOMINAZIONE FROM SISCOM_MI.EDIFICI WHERE ID<>1 " & condizioneComplesso & condizioneBM & " ORDER BY DENOMINAZIONE", cmbEdificio, "ID", "NOME", True)
 ...
 Protected Sub btnAnnulla_Click(sender As Object, e As System.EventArgs) Handles btnAnnulla.Click
   ...
   Protected Sub btnCerca_Click(sender As Object, e As System.EventArgs) Handles btnCerca.Click
        ...
        Response.Write("<script>document.location.href='RisultatiSegnChiusura.aspx?FOR=" _
                       ...
                       & "&STAT=" & listaStati _
                       & "&URG=" & DropDownListUrgenza.SelectedValue _
    ...
    Private Sub CaricaBManager()
        Try
            ...
            'par.caricaComboBox(par.cmd.CommandText, cmbBManager, "ID", "MANAGER", True)
    ...
    Private Sub controlloBM()
        Try
            ...
            par.cmd.CommandText = "SELECT ID_BM FROM SISCOM_MI.BUILDING_MANAGER_OPERATORI WHERE ID_OPERATORE=" & idOperatore
            Dim lettore As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader
            If lettore.Read Then
