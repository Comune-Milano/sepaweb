...
Partial Class CICLO_PASSIVO_CicloPassivo_PAGAMENTI_CosapCaricati
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        End If
    End Sub
    Private Sub CaricaCosap(ByVal filtro As String)
        Try
            ...
            par.cmd.CommandText = "select " _
                & "cosap.id,'false' as CHECKED, nome_file,GETDATA(data_caricamento) AS DATA_CARICAMENTO,FORNITORI.RAGIONE_SOCIALE AS FORNITORE," _
                ...
                & "COD_PATRIMONIO,NOTE AS NOTE,RTRIM(LTRIM(TO_CHAR(IMPORTO,'9G999G999G999D99'))) AS IMPORTO,(select progr||'/'||anno from siscom_mi.pagamenti where id = (select id_pagamento from siscom_mi.prenotazioni where id = cosap.id_prenotazione)) as CDP " _
                & " from siscom_mi.cosap,siscom_mi.FORNITORI,siscom_mi.PAGAMENTI_UTENZE_VOCI " _
                & " WHERE PAGAMENTI_UTENZE_VOCI.ID = cosap.ID_PARAM_UTENZA AND FORNITORI.ID = PAGAMENTI_UTENZE_VOCI.id_fornitore  " & filtro
            'Else
            'par.cmd.CommandText = "select " _
            ...
            Me.lblTitolo.Text = "Cosap - Cosap con CDP - tot record: " & dt.Rows.Count
            If Request.QueryString("PAGATE") = 0 Then
        ...
        Try
            ...
            If Not String.IsNullOrEmpty(idSelezionati.Value) Then
                ...
                par.cmd.CommandText = "select distinct PAGAMENTI_UTENZE_VOCI.id_piano_finanziario,PAGAMENTI_UTENZE_VOCI.id_fornitore " _
                                    & " from siscom_mi.cosap,SISCOM_MI.PAGAMENTI_UTENZE_VOCI,siscom_mi.pf_voci where " _
                                    & " PAGAMENTI_UTENZE_VOCI.ID = cosap.ID_PARAM_UTENZA AND pf_voci.id = PAGAMENTI_UTENZE_VOCI.id_voce_pf and cosap.id in (" & Session.Item("idSel") & ")"
                Dim da As New Oracle.DataAccess.Client.OracleDataAdapter(par.cmd)
                Dim dt As New Data.DataTable
                da.Fill(dt)
                da.Dispose()
                ...
                Select Case TipoAnomalia
                    Case 0
                        Dim script As String = "function f(){var oWnd=$find(""" + RadWindow1.ClientID + """);oWnd.setUrl('FatturePagaUt.aspx?TIPO=COSAP');oWnd.show(); Sys.Application.remove_load(f);}Sys.Application.add_load(f);"
            ...
            Else
                ScriptManager.RegisterStartupScript(Me.Page, Me.Page.GetType(), "msg", "alert('Nessun COSAP selezionata, impossibile creare il CDP');", True)
    ...
    Protected Sub btnEliminaFattura_Click(sender As Object, e As System.EventArgs) Handles btnEliminaFattura.Click
        If Not String.IsNullOrEmpty(idSelezionati.Value) Then
            ...
            connData.apri(True)
            par.cmd.CommandText = "delete from siscom_mi.cosap where id in (" & Me.idSelezionati.Value & ") and id_prenotazione is null"
        ...
        Else
            ScriptManager.RegisterClientScriptBlock(Me, Me.GetType(), "msg", "alert('Selezionare almeno un cosap per eliminarla!');", True)
    ...
    Protected Sub btnEliminaFattureTutte_Click(sender As Object, e As System.EventArgs) Handles btnEliminaFattureTutte.Click
        connData.apri(True)
        par.cmd.CommandText = "delete from siscom_mi.cosap where  id_prenotazione is null " & CreaFiltro()
        par.cmd.ExecuteNonQuery()
        connData.chiudi(True)
        CaricaCosap("")
    ...
    Protected Sub btnExportXls_Click(sender As Object, e As System.EventArgs) Handles btnExportXls.Click
        ...
        Dim nomeFile = xls.EsportaExcelDaDT(ExcelSiSol.Estensione.Office2007_xlsx, "Cosap", "Export", dtNEW)
    ...
    Private Sub dgvMulte_NeedDataSource(sender As Object, e As GridNeedDataSourceEventArgs) Handles dgvMulte.NeedDataSource
        Try
            ...
            par.cmd.CommandText = "select " _
                ...
                & "COD_PATRIMONIO,NOTE AS NOTE,RTRIM(LTRIM(TO_CHAR(IMPORTO,'9G999G999G999D99'))) AS IMPORTO,(select progr||'/'||anno from siscom_mi.pagamenti where id = (select id_pagamento from siscom_mi.prenotazioni where id = cosap.id_prenotazione)) as CDP " _
                & " from siscom_mi.cosap,siscom_mi.FORNITORI,siscom_mi.PAGAMENTI_UTENZE_VOCI " _
                & " WHERE PAGAMENTI_UTENZE_VOCI.ID = cosap.ID_PARAM_UTENZA AND FORNITORI.ID = PAGAMENTI_UTENZE_VOCI.id_fornitore  " & strFiltro
            'Else
            'par.cmd.CommandText = "select " _
            '                    & " id,nome_file,GETDATA(data_caricamento) AS DATA_CARICAMENTO,GETDATA(data_inizio_periodo) AS DATA_INIZIO_PERIODO,GETDATA(data_fine_periodo) AS DATA_FINE_PERIODO, " _
            '                    & "(case when id_complesso is not null then 'COMPLESSO' WHEN ID_EDIFICIO IS NOT NULL THEN 'EDIFICIO' WHEN ID_UNITA IS NOT NULL THEN 'UNITA' ELSE '' END) AS TIPO_PATRIMONIO, " _
            ...
            ScriptManager.RegisterStartupScript(Me, Me.GetType, "", "setDimensioni();", True)
            connData.chiudi()
            Me.lblTitolo.Text = "ELENCO COSAP CARICATI - tot record: " & dt.Rows.Count
            If Request.QueryString("PAGATE") = 0 Then
                dgvMulte.Columns(14).Visible = False
            End If
        Catch ex As Exception
