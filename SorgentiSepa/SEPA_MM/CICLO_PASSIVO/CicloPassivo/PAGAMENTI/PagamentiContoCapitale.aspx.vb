...
Partial Class CICLO_PASSIVO_CicloPassivo_PagamentiContoCapitale
    Inherits PageSetIdMode
    Dim par As New CM.Global
    Dim totale As Decimal = 0
    Dim condizioneStruttura As String = ""
    Dim condizioneStrutturaRitenute As String = ""
        ...
        Try
            ...
            If Not IsNothing(Request.QueryString("ID_STRUTTURA")) Then
                If Request.QueryString("ID_STRUTTURA") = "-1" Then
                    condizioneStruttura = ""
                    condizioneStrutturaRitenute = ""
                Else
                    condizioneStruttura = " AND PAGAMENTI_LIQUIDATI.ID_STRUTTURA=" & Request.QueryString("ID_STRUTTURA") & " "
                    condizioneStrutturaRitenute = " AND PAGAMENTI_RIT_LIQUIDATI.ID_STRUTTURA=" & Request.QueryString("ID_STRUTTURA") & " "
                End If
            End If
            ...
            par.cmd.CommandText = " SELECT PF_VOCI.CODICE, " _
                ...
                & " (CASE WHEN(PAGAMENTI.TIPO_PAGAMENTO=4) THEN(PAGAMENTI.DESCRIZIONE) ELSE(APPALTI.DESCRIZIONE || (CASE WHEN(PAGAMENTI.DESCRIZIONE) IS NULL THEN('') ELSE(', ' || PAGAMENTI.DESCRIZIONE) END)) END)  AS DESCRIZIONE_PAG, " _
                ...
                & " TRIM (TO_CHAR((NVL(IMPORTO, 0)),'999G999G990D99')) " _
                & " AS IMPORTO_LIQUIDATO, " _
                ...
                & " PAGAMENTI_LIQUIDATI.DATA_MANDATO, " _
                ...
                & " TRIM(TO_CHAR(PAGAMENTI.IMPORTO_CONSUNTIVATO,'999G999G990D99')) AS IMPORTO_CONSUNTIVATO,PAGAMENTI.ID AS ID_PAGAMENTO,'' AS SERVIZIO,'0' AS TIPORIT " _
                ...
                & " AND ID_VOCE_PF IN (SELECT ID FROM siscom_mi.pf_voci a WHERE CONNECT_BY_ISLEAF = 1 CONNECT BY PRIOR a.ID = a.id_voce_madre START WITH ID = " & Request.QueryString("ID_VOCE") & ") " _
                ...
                & " PAGAMENTI_RIT_LIQUIDATI.DATA_MANDATO, " _
                & " PAGAMENTI.PROGR ||'/'||PAGAMENTI.ANNO AS MAE, " _
                & " CASE WHEN PAGAMENTI.PROGR IS NOT NULL THEN PAGAMENTI.PROGR ||'/'|| PAGAMENTI.ANNO ELSE '' END AS ADP, " _
                & " TO_CHAR(TO_DATE(DATA_EMISSIONE,'yyyyMMdd'),'dd/MM/yyyy') AS DATA_ADP," _
                & " TRIM(TO_CHAR(PAGAMENTI.IMPORTO_CONSUNTIVATO,'999G999G990D99')) AS IMPORTO_CONSUNTIVATO,PAGAMENTI.ID AS ID_PAGAMENTO_RIT_LEGGE,'' AS SERVIZIO,'1' AS TIPORIT " _
            ...
            If dt.Rows.Count > 0 Then
                ...
                For Each RIGA As DataGridItem In DataGrid.Items
                    ...
                    Dim tiporit As String = RIGA.Cells(14).Text
                    If tiporit = 0 Then
                        ...
                        & " WHERE ID_PAGAMENTO = '" & RIGA.Cells(13).Text & "' " _
                    ...
                    Else
                        par.cmd.CommandText = "SELECT DISTINCT " _
                                                ...
                                                & " SISCOM_MI.PF_VOCI_IMPORTO, " _
                                                & " SISCOM_MI.TAB_sERVIZI " _
                                                & " WHERE ID_PAGAMENTO_rit_legge = '" & RIGA.Cells(13).Text & "' " _
                                                & " AND PF_VOCI_IMPORTO.ID = PRENOTAZIONI.ID_VOCE_PF_IMPORTO " _
                                                & " AND TAB_SERVIZI.ID = PF_VOCI_IMPORTO.ID_SERVIZIO "
    ...
    Protected Sub DataGrid_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles DataGrid.ItemDataBound
        Select Case e.Item.ItemType
            Case ListItemType.Item, ListItemType.AlternatingItem
                totale += CType((e.Item.Cells(11).Text), Double)
            Case ListItemType.Footer
                e.Item.Cells(11).Text = Format(totale, "#,##0.00")
        End Select
    End Sub
