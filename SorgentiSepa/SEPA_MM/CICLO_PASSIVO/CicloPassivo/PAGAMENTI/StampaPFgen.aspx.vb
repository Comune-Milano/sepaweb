...
Partial Class CicloPassivo_Pagamenti_StampaPFgen
        ...
        Try
            ...
            While LettoreElencoVoci.Read
                ...
                par.cmd.CommandText = "SELECT TRIM(TO_CHAR(SUM(NVL(IMPORTO_APPROVATO,0)-NVL(RIT_LEGGE_IVATA,0)),'999999990D9999')) AS APPROVATO FROM SISCOM_MI.PRENOTAZIONI " _
                    & "WHERE ((ID_STATO=1 AND DATA_CONSUNTIVAZIONE <='" & dataAl & "') " _
                    & "OR (ID_STATO=2  AND DATA_CONSUNTIVAZIONE <='" & dataAl & "' AND DATA_CERTIFICAZIONE >'" & dataAl & "') OR" _
                    & "(ID_STATO=-3 AND DATA_ANNULLO>'" & dataAl & "' AND DATA_CONSUNTIVAZIONE<='" & dataAl & "' AND (DATA_CERTIFICAZIONE IS NULL OR DATA_CERTIFICAZIONE>'" & dataAl & "')))" _
                    & "AND ANNO='" & ANNO & "' " _
                    & "AND ID_VOCE_PF IN (" _
                    & "SELECT ID FROM SISCOM_MI.PF_VOCI " _
                    & "WHERE CONNECT_BY_ISLEAF = 1 " _
                ...
                par.cmd.CommandText = "SELECT TRIM(TO_CHAR(SUM(NVL(IMPORTO_APPROVATO,0)-NVL(RIT_LEGGE_IVATA,0)),'999999990D9999')) AS APPROVATO FROM SISCOM_MI.PRENOTAZIONI " _
                    ...
                    & " OR (ID_STATO=-3 AND DATA_ANNULLO>'" & dataAl & "' AND DATA_CONSUNTIVAZIONE<='" & dataAl & "' AND DATA_CERTIFICAZIONE<='" & dataAl & "'))" _
                    & "AND ANNO='" & ANNO & "' " _
                    & "AND ID_VOCE_PF IN (" _
                    & "SELECT ID FROM SISCOM_MI.PF_VOCI " _
                    & "WHERE CONNECT_BY_ISLEAF = 1 " _
