...
Partial Class CICLO_PASSIVO_CicloPassivo_PAGAMENTI_GestionePod
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            HFGriglia.Value = RadGridPod.ClientID
            SvuotaCampi(-1)
            caricaAggregazioni()
        End If
    End Sub
    Protected Sub btnDelpod_Click(sender As Object, e As System.EventArgs) Handles btnDelPOD.Click
        Try
            If IsNumeric(idSel.Value) AndAlso idSel.Value > 0 Then
                connData.apri()
                par.cmd.CommandText = "SELECT COUNT(*) FROM SISCOM_MI.PATRIMONIO_POD WHERE ID_POD = " & idSel.Value
                Dim ris As Integer = par.IfNull(par.cmd.ExecuteScalar, 0)
                If ris > 0 Then
                    RadWindowManager1.RadAlert("POD non eliminabile perchè legato al patrimonio!\nVerrà reso non attivo!", 300, 150, "Attenzione", Nothing, Nothing)
                    par.cmd.CommandText = "update siscom_mi.POD set fl_eliminato = 1 where id = " & idSel.Value
                    ...
                    Exit Sub
                End If
                par.cmd.CommandText = "delete from siscom_mi.POD where id = " & idSel.Value
                par.cmd.ExecuteNonQuery()
                RadWindowManager1.RadAlert("POD eliminato correttamente!", 300, 150, "Attenzione", Nothing, Nothing)
                connData.chiudi()
                RadGridPod.Rebind()
                idSel.Value = ""
            End If
        ...
        Catch ex As Exception
            ...
            Session.Add("ERRORE", Page.Title & " btnDelPOD_Click - " & ex.Message)
            Response.Redirect("../../../Errore.aspx", False)
        End Try
    End Sub
    Protected Sub RadGridPod_DetailTableDataBind(sender As Object, e As Telerik.Web.UI.GridDetailTableDataBindEventArgs) Handles RadGridPod.DetailTableDataBind
            ...
            Case "Dettagli"
                If True Then
                    par.cmd.CommandText = "SELECT ID,ID_POD,(SELECT DENOMINAZIONE FROM SISCOM_MI.AGGREGAZIONE_POD WHERE AGGREGAZIONE_POD.ID=PATRIMONIO_POD.ID_AGGREGAZIONE) AS NOME_AGGREGAZIONE," _
                        ...
                        & " FROM SISCOM_MI.PATRIMONIO_POD WHERE ID_POD=" & dataItem("id").Text _
        ...
        End Select
    End Sub
    Protected Sub RadGridPod_ItemDataBound(sender As Object, e As Telerik.Web.UI.GridItemEventArgs) Handles RadGridPod.ItemDataBound
        Try
        ...
        End Try
    End Sub
    Protected Sub RadGridPOD_NeedDataSource(sender As Object, e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles RadGridPod.NeedDataSource
        Try
            par.cmd.CommandText = "SELECT ID,CONTRATTO,POD,DESCRIZIONE,(SELECT DESCRIZIONE FROM SISCOM_MI.TIPO_UTENZE WHERE ID = ID_TIPO_FORNITURA) AS FORNITURA , " _
                & "(SELECT RAGIONE_SOCIALE FROM SISCOM_MI.FORNITORI WHERE ID = ID_FORNITORE ) AS FORNITORE,DECODE(FL_ELIMINATO,0,'SI',1,'NO') AS ATTIVO " _
                & "FROM SISCOM_MI.POD ORDER BY FORNITURA,CONTRATTO,POD ASC"
        ...
        Catch ex As Exception
            ...
            End If
            Session.Item("LAVORAZIONE") = "0"
            Session.Add("ERRORE", Page.Title & " RadGridPOD_NeedDataSource - " & ex.Message)
            Response.Redirect("../../../Errore.aspx", False)
        End Try
    End Sub
    Protected Sub RadButtonDisattiva_Click(sender As Object, e As System.EventArgs) Handles RadButtonDisattiva.Click
        Try
            ...
            RadWindowManager1.RadAlert("POD disattivato correttamente!", 300, 150, "Attenzione", Nothing, Nothing)
            RadGridPod.Rebind()
            idSel.Value = ""
        Catch ex As Exception
            If connData.Connessione.State = Data.ConnectionState.Open Then
    ...
    Private Sub estraiTuttiAttuali()
        Try
            connData.apri()
            par.cmd.CommandText = "SELECT CONTRATTO," _
                & " POD," _
                & " DESCRIZIONE," _
                & " (SELECT TIPO_UTENZE.DESCRIZIONE FROM SISCOM_MI.TIPO_UTENZE WHERE TIPO_UTENZE.ID=ID_TIPO_FORNITURA) AS FORNITURA," _
                ...
                & " (CASE " _
                & " WHEN PATRIMONIO_POD.ID_EDIFICIO IS NOT NULL THEN 'EDIFICIO'" _
                & " WHEN PATRIMONIO_POD.ID_AGGREGAZIONE IS NOT NULL THEN 'AGGREGAZIONE'" _
                & " WHEN PATRIMONIO_POD.ID_COMPLESSO IS NOT NULL THEN 'COMPLESSO'" _
                & " ELSE " _
                & " '' " _
                & " END" _
                & " ) AS TIPO," _
                & " (CASE " _
                & " WHEN PATRIMONIO_POD.ID_EDIFICIO IS NOT NULL THEN (SELECT COD_EDIFICIO||' - '||DENOMINAZIONE FROM SISCOM_MI.EDIFICI WHERE EDIFICI.ID=ID_EDIFICIO)" _
                & " WHEN PATRIMONIO_POD.ID_AGGREGAZIONE IS NOT NULL THEN (SELECT DENOMINAZIONE FROM SISCOM_MI.AGGREGAZIONE_POD WHERE AGGREGAZIONE_POD.ID=ID_AGGREGAZIONE)" _
                & " WHEN PATRIMONIO_POD.ID_COMPLESSO IS NOT NULL THEN (SELECT COD_COMPLESSO||' - '||DENOMINAZIONE FROM SISCOM_MI.COMPLESSI_IMMOBILIARI WHERE COMPLESSI_IMMOBILIARI.ID=ID_COMPLESSO)" _
                & " ELSE" _
                & " ''" _
                & " END" _
                & " ) AS DENOMINAZIONE, " _
                & " SISCOM_MI.GETDATA(DATA_INIZIO) AS ""INIZIO"" " _
                & " FROM SISCOM_MI.POD, SISCOM_MI.PATRIMONIO_POD " _
                & " WHERE POD.ID = PATRIMONIO_POD.ID_POD(+) " _
                & " AND PATRIMONIO_POD.DATA_FINE IS NULL"
            ...
            Dim nomeFile = xls.EsportaExcelDaDT(ExcelSiSol.Estensione.Office2007_xlsx, "ExportPOD", "ExportPOD", dt)
    ...
    Private Sub estraiTutti()
        Try
            ...
            par.cmd.CommandText = "SELECT CONTRATTO," _
                  ...
                  & " FROM SISCOM_MI.POD, SISCOM_MI.PATRIMONIO_POD " _
                  & " WHERE POD.ID = PATRIMONIO_POD.ID_POD(+) "
    ...
    Private Sub caricaAggregazioni()
        Try
            connData.apri()
            par.cmd.CommandText = "SELECT ID, DENOMINAZIONE FROM SISCOM_MI.AGGREGAZIONE_POD ORDER BY DENOMINAZIONE ASC"
            Dim da As New Oracle.DataAccess.Client.OracleDataAdapter(par.cmd)
            Dim dt As New Data.DataTable
            da.Fill(dt)
            da.Dispose()
    ...
    Protected Sub RadButtonSalvaAggregazione_Click(sender As Object, e As System.EventArgs) Handles RadButtonSalvaAggregazione.Click
        Try
            If txtNomeAggregazione.Text <> "" Then
                connData.apri(True)
                par.cmd.CommandText = "SELECT COUNT(*) FROM SISCOM_MI.AGGREGAZIONE_POD WHERE upper(DENOMINAZIONE)='" & UCase(txtNomeAggregazione.Text.Replace("'", "''")) & "'"
                Dim ris As Integer = 0
                ris = par.IfNull(par.cmd.ExecuteScalar, 0)
                If ris = 0 Then
                    par.cmd.CommandText = "INSERT INTO SISCOM_MI.AGGREGAZIONE_POD (ID,DENOMINAZIONE) VALUES (SISCOM_MI.SEQ_AGGREGAZIONE_POD.NEXTVAL,'" & UCase(txtNomeAggregazione.Text.Replace("'", "''")) & "')"
                    par.cmd.ExecuteNonQuery()
                Else
    ...
    Protected Sub RadButtonEliminaAggregazione_Click(sender As Object, e As System.EventArgs) Handles RadButtonEliminaAggregazione.Click
        Try
            If IsNumeric(idSelAggr.Value) AndAlso idSelAggr.Value > 0 Then
                connData.apri(True)
                par.cmd.CommandText = "SELECT COUNT(*) FROM SISCOM_MI.PATRIMONIO_POD WHERE ID_AGGREGAZIONE=" & idSelAggr.Value
                Dim ris As Integer = 0
                ris = par.IfNull(par.cmd.ExecuteScalar, 0)
                If ris = 0 Then
                    par.cmd.CommandText = "DELETE FROM SISCOM_MI.AGGREGAZIONE_POD_dETT WHERE ID_AGGREGAZIONE=" & idSelAggr.Value
                    par.cmd.ExecuteNonQuery()
                    par.cmd.CommandText = "DELETE FROM SISCOM_MI.AGGREGAZIONE_POD WHERE ID=" & idSelAggr.Value
                    par.cmd.ExecuteNonQuery()
                    connData.chiudi(True)
                    caricaAggregazioni()
                ...
                Else
                    RadWindowManager1.RadAlert("Aggregazione legata a un POD! Non è possibile eliminarla!", 330, 180, "Attenzione!", "function(sender, args){openWindow(sender, args, 'RadWindowAggregazioni');}", Nothing)
                    connData.chiudi(True)
                    Exit Sub
                End If
            End If
    ...
    Private Sub caricaDettaglioAggregazioni(Optional ByVal apriDettaglio As Boolean = True)
        Try
            ...
            par.cmd.CommandText = "SELECT (" _
                ...
                & " FROM SISCOM_MI.AGGREGAZIONE_POD_DETT WHERE ID_AGGREGAZIONE=" & idSelAggr.Value
            Dim da As New Oracle.DataAccess.Client.OracleDataAdapter(par.cmd)
            Dim dt As New Data.DataTable
            da.Fill(dt)
            da.Dispose()
    ...
    Protected Sub RadButtonSalvaAggregazioneComplesso_Click(sender As Object, e As System.EventArgs) Handles RadButtonSalvaAggregazioneComplesso.Click
        Try
            If RadComboBoxAggregazioneComplesso.SelectedValue <> "-1" AndAlso idSelAggr.Value > "0" Then
                connData.apri()
                par.cmd.CommandText = "SELECT COUNT(*) FROM SISCOM_MI.AGGREGAZIONE_POD_DETT WHERE ID_AGGREGAZIONE=" & idSelAggr.Value & " AND ID_COMPLESSO=" & RadComboBoxAggregazioneComplesso.SelectedValue
                Dim ris As Integer = par.IfNull(par.cmd.ExecuteScalar, 0)
                If ris = 0 Then
                    par.cmd.CommandText = "INSERT INTO SISCOM_MI.AGGREGAZIONE_POD_DETT (ID_AGGREGAZIONE,ID_COMPLESSO) VALUES (" & idSelAggr.Value & "," & RadComboBoxAggregazioneComplesso.SelectedValue & ")"
                    par.cmd.ExecuteNonQuery()
                    connData.chiudi()
                Else
                    connData.chiudi()
        ...
        If RadComboBoxAggregazioneEdificio.SelectedValue <> "-1" Then
            Try
                If RadComboBoxAggregazioneEdificio.SelectedValue <> "-1" AndAlso idSelAggr.Value > "0" Then
                    connData.apri()
                    par.cmd.CommandText = "SELECT COUNT(*) FROM SISCOM_MI.AGGREGAZIONE_POD_DETT WHERE ID_AGGREGAZIONE=" & idSelAggr.Value & " AND ID_EDIFICIO=" & RadComboBoxAggregazioneEdificio.SelectedValue
                    Dim ris As Integer = par.IfNull(par.cmd.ExecuteScalar, 0)
                    If ris = 0 Then
                        par.cmd.CommandText = "INSERT INTO SISCOM_MI.AGGREGAZIONE_POD_DETT (ID_AGGREGAZIONE,ID_EDIFICIO) VALUES (" & idSelAggr.Value & "," & RadComboBoxAggregazioneEdificio.SelectedValue & ")"
                        par.cmd.ExecuteNonQuery()
                        connData.chiudi()
                    Else
                        connData.chiudi()
        ...
        If RadComboBoxAggregazioneScala.SelectedValue <> "-1" Then
            Try
                If RadComboBoxAggregazioneScala.SelectedValue <> "-1" AndAlso idSelAggr.Value > "0" Then
                    connData.apri()
                    par.cmd.CommandText = "SELECT COUNT(*) FROM SISCOM_MI.AGGREGAZIONE_POD_DETT WHERE ID_AGGREGAZIONE=" & idSelAggr.Value & " AND ID_SCALA=" & RadComboBoxAggregazioneScala.SelectedValue
                    Dim ris As Integer = par.IfNull(par.cmd.ExecuteScalar, 0)
                    If ris = 0 Then
                        par.cmd.CommandText = "INSERT INTO SISCOM_MI.AGGREGAZIONE_POD_DETT (ID_AGGREGAZIONE,ID_SCALA) VALUES (" & idSelAggr.Value & "," & RadComboBoxAggregazioneScala.SelectedValue & ")"
                        par.cmd.ExecuteNonQuery()
                        connData.chiudi()
                    Else
                        connData.chiudi()
        ...
        If RadComboBoxAggregazioneUnita.SelectedValue <> "-1" Then
            Try
                If RadComboBoxAggregazioneUnita.SelectedValue <> "-1" AndAlso idSelAggr.Value > "0" Then
                    connData.apri()
                    par.cmd.CommandText = "SELECT COUNT(*) FROM SISCOM_MI.AGGREGAZIONE_POD_DETT WHERE ID_AGGREGAZIONE=" & idSelAggr.Value & " AND ID_UNITA=" & RadComboBoxAggregazioneUnita.SelectedValue
                    Dim ris As Integer = par.IfNull(par.cmd.ExecuteScalar, 0)
                    If ris = 0 Then
                        par.cmd.CommandText = "INSERT INTO SISCOM_MI.AGGREGAZIONE_POD_DETT (ID_AGGREGAZIONE,ID_UNITA) VALUES (" & idSelAggr.Value & "," & RadComboBoxAggregazioneUnita.SelectedValue & ")"
                        par.cmd.ExecuteNonQuery()
                        connData.chiudi()
                    Else
                        connData.chiudi()
        ...
        If RadComboBoxAggregazioneImpianto.SelectedValue <> "-1" Then
            Try
                If RadComboBoxAggregazioneImpianto.SelectedValue <> "-1" AndAlso idSelAggr.Value > "0" Then
                    connData.apri()
                    par.cmd.CommandText = "SELECT COUNT(*) FROM SISCOM_MI.AGGREGAZIONE_POD_DETT WHERE ID_AGGREGAZIONE=" & idSelAggr.Value & " AND ID_IMPIANTO=" & RadComboBoxAggregazioneImpianto.SelectedValue
                    Dim ris As Integer = par.IfNull(par.cmd.ExecuteScalar, 0)
                    If ris = 0 Then
                        par.cmd.CommandText = "INSERT INTO SISCOM_MI.AGGREGAZIONE_POD_DETT (ID_AGGREGAZIONE,ID_IMPIANTO) VALUES (" & idSelAggr.Value & "," & RadComboBoxAggregazioneImpianto.SelectedValue & ")"
                        par.cmd.ExecuteNonQuery()
                        connData.chiudi()
                    Else
                        connData.chiudi()
    ...
    Protected Sub btnPOD_Click(sender As Object, e As System.EventArgs) Handles btnPOD.Click
        RadGridPod.Rebind()
        idSel.Value = ""
    End Sub
    ...
    Private Sub estraiTutteLeAggregazioni()
        Try
            connData.apri()
            par.cmd.CommandText = " SELECT AGGREGAZIONE_POD.DENOMINAZIONE AS AGGREGAZIONE, " _
                & " (CASE  " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_EDIFICIO IS NOT NULL THEN 'EDIFICIO' " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_SCALA IS NOT NULL THEN 'SCALA' " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_COMPLESSO IS NOT NULL THEN 'COMPLESSO' " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_UNITA IS NOT NULL THEN 'UNITA' " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_IMPIANTO IS NOT NULL THEN 'IMPIANTO' " _
                & " ELSE  " _
                & " ''  " _
                & " END " _
                & " ) AS TIPO, " _
                & " (CASE  " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_EDIFICIO IS NOT NULL THEN (SELECT COD_EDIFICIO||' - '||DENOMINAZIONE FROM SISCOM_MI.EDIFICI WHERE EDIFICI.ID=ID_EDIFICIO) " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_SCALA IS NOT NULL THEN (SELECT DESCRIZIONE FROM SISCOM_MI.SCALE_eDIFICI WHERE SCALE_EDIFICI.ID=ID_SCALA)||' (COD ED.'||(SELECT DISTINCT COD_eDIFICIO FROM SISCOM_MI.EDIFICI,SISCOM_MI.SCALE_EDIFICI WHERE SCALE_EDIFICI.ID_EDIFICIO=EDIFICI.ID AND SCALE_EDIFICI.ID=ID_sCALA)||')' " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_COMPLESSO IS NOT NULL THEN (SELECT COD_COMPLESSO||' - '||DENOMINAZIONE FROM SISCOM_MI.COMPLESSI_IMMOBILIARI WHERE COMPLESSI_IMMOBILIARI.ID=ID_COMPLESSO) " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_UNITA IS NOT NULL THEN (SELECT COD_UNITA_IMMOBILIARE FROM SISCOM_MI.UNITA_IMMOBILIARI WHERE UNITA_IMMOBILIARI.ID=ID_UNITA) " _
                & " WHEN AGGREGAZIONE_POD_DETT.ID_IMPIANTO IS NOT NULL THEN (SELECT COD_IMPIANTO||' - '||DENOMINAZIONE FROM SISCOM_MI.IMPIANTI WHERE IMPIANTI.ID=ID_IMPIANTO) " _
                & " ELSE " _
                & " '' " _
                & " END " _
                & " ) AS DENOMINAZIONE " _
                & "  " _
                & " FROM SISCOM_MI.AGGREGAZIONE_POD,SISCOM_MI.AGGREGAZIONE_POD_DETT " _
                & " WHERE AGGREGAZIONE_POD.ID=AGGREGAZIONE_POD_DETT.ID_AGGREGAZIONE " _
                & " ORDER BY 1,2,3 "
    ...
    Private Sub CICLO_PASSIVO_CicloPassivo_PAGAMENTI_GestionePod_LoadComplete(sender As Object, e As EventArgs) Handles Me.LoadComplete
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "", "setDimensioni();", True)
    End Sub
End Class
