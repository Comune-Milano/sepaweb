...
Partial Class CICLO_PASSIVO_CicloPassivo_PAGAMENTI_FattureCaricAnomlie
    ...
    Private Sub CaricaAnomalie()
        ...
        If Not IsNothing(Request.QueryString("NOME_FILE")) Then
            ...
            Me.lblTitolo.Text = "ANOMALIE CARICAMENTO FILE FATTURE - " & Request.QueryString("NOME_FILE")
        Else
            Me.lblTitolo.Text = "ANOMALIE CARICAMENTO FILE FATTURE"
        End If
        par.cmd.CommandText = "SELECT  TIPO_UTENZE.descrizione AS tipo_utenza ,FORNITORI.ragione_sociale AS fornitore, " _
                            & "Getdata(data_caricamento) as data_caricamento,data_caricamento as ordCaricamento,numero_fattura,anno_fattura,NOTE " _
                            & "FROM siscom_mi.FATTURE_UTENZE_ANOMALIE,siscom_mi.TIPO_UTENZE,siscom_mi.FORNITORI,siscom_mi.PAGAMENTI_UTENZE_VOCI " _
                            & "WHERE PAGAMENTI_UTENZE_VOCI.ID = ID_PARAM_UTENZA and FORNITORI.ID = id_fornitore AND TIPO_UTENZE.ID = id_tipo_utenza " _
                            & CondFileName & " order by ordCaricamento desc, nome_file asc"
