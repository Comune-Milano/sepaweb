'*** RICERCA PAGAMENTI in MODO DIRETTO

Partial Class PAGAMENTI_RicercaPagamentiD
 ...
 Protected Sub btnCerca_Click(sender As Object, e As System.EventArgs) Handles btnCerca.Click
        Try
            ...
            Response.Write("<script>location.replace('RisultatiPagamenti.aspx?ODL=" & Me.txtODL.Text _
                                                                         & "&ANNO=" & Me.txtAnno.Text _
                                                                         & "&EF_R=" & Me.cmbEsercizio.SelectedValue.ToString _
                                                                         & "&ST=RICERCA_DIRETTA" _
                                                                         & "');</script>")
        ...
        Try
            ...
            If sFiliale <> "-1" Then
                par.cmd.CommandText = "select SISCOM_MI.PF_MAIN.ID, TO_CHAR(TO_DATE(T_ESERCIZIO_FINANZIARIO.INIZIO,'YYYYmmdd'),'DD/MM/YYYY') as INIZIO, " _
                                   & "TO_CHAR(TO_DATE(T_ESERCIZIO_FINANZIARIO.INIZIO,'YYYYmmdd'),'DD/MM/YYYY') || '-' || TO_CHAR(TO_DATE(T_ESERCIZIO_FINANZIARIO.FINE,'YYYYmmdd'),'DD/MM/YYYY') || '(' || " _
                                   ...
                                   & "   and SISCOM_MI.PF_MAIN.ID in (select distinct(ID_PIANO_FINANZIARIO) " _
                                                                   ...
                                                                   & " where ID in (select distinct(ID_VOCE_PF) " _
                                                                                ...
                                                                                & " where ID_STRUTTURA=" & sFiliale & ") ) order by T_ESERCIZIO_FINANZIARIO.ID desc"
            ...
            Else
                par.cmd.CommandText = "select SISCOM_MI.PF_MAIN.ID, TO_CHAR(TO_DATE(T_ESERCIZIO_FINANZIARIO.INIZIO,'YYYYmmdd'),'DD/MM/YYYY') as INIZIO, " _
                                   ...
                                   & "   and SISCOM_MI.PF_MAIN.ID in (select distinct(ID_PIANO_FINANZIARIO) " _
                                                                   & " from SISCOM_MI.PF_VOCI " _
                                                                   & " where ID in (select distinct(ID_VOCE_PF) " _
                                                                                & " from SISCOM_MI.ODL ) ) order by T_ESERCIZIO_FINANZIARIO.ID desc"
