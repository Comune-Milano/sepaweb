...
Partial Class SPESE_REVERSIBILI_RicercaConsuntivi
    ...
    Private Sub caricaListaComplessi()
        ...
        Catch ex As Exception
            connData.chiudi()
            Session.Add("ERRORE", "Provenienza: Ricerca consuntivi - " & ex.Message)
    ...
    Private Sub caricaListaCivicoInternoPiano()
        If cmbIndirizzo.SelectedValue <> "-1" Then
            ...
            Catch ex As Exception
                connData.chiudi()
                Session.Add("ERRORE", "Provenienza: Ricerca consuntivi - " & ex.Message)
                RadWindowManager1.RadAlert("Errore durante il caricamento civico, interno e piano!", 300, 150, "Attenzione", "function f(sender,args){location.href='../Errore.aspx';}", "null")
            End Try
        Else
    ...
    Protected Sub ButtonAvviaricerca_Click(sender As Object, e As System.EventArgs) Handles ButtonAvviaricerca.Click
        'If cmbEdificio.SelectedValue <> "-1" Or DrLComplesso.SelectedValue <> "-1" Then
        Try
            ...
            'Response.Redirect("RisultatiRicercaConsuntiviDettaglio.aspx?EDIFICIO=" & cmbEdificio.SelectedValue _
            ...
            If chkRicercaDettagliata.Checked = True Then
                tipoRicerca = "TIPO=D&"
            Else
                tipoRicerca = "TIPO=ND&"
            End If
            Response.Redirect("RisultatiRicercaConsuntivi.aspx?" & tipoRicerca & "EDIFICIO=" & cmbEdificio.SelectedValue _
                & "&CIVICO=" & cmbCivico.SelectedValue _
                & "&INDIRIZZO=" & par.VaroleDaPassare(par.PulisciStrSql(cmbIndirizzo.SelectedValue)) _
                & "&COMPLESSO=" & par.VaroleDaPassare(par.PulisciStrSql(DrLComplesso.SelectedValue)) _
                & "&INTERNO=" & cmbInterno.SelectedValue _
                & "&SCALA=" & cmbScala.SelectedValue _
                & "&INTESTATARIO=" & txtIntestatario.Text _
                & "&CODUI=" & txtCodiceUI.Text _
                & "&ASCENSORE=" & cmbAscensore.SelectedValue & "&TIPOLOGIA=" & listaTipologieSelezionate, False)
            'End If
        Catch ex As Exception
            connData.chiudi()
