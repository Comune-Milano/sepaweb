    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            If Not IsPostBack Then
                ...
                'CARICA LE STRUTTURE 10/04/2012
                cmbStruttura.Items.Clear()
                cmbStruttura.Items.Add(New ListItem("TUTTE", "-1"))
                par.cmd.CommandText = "SELECT * FROM SISCOM_MI.TAB_FILIALI WHERE ID IN (SELECT ID_FILIALE FROM SISCOM_MI.COMPLESSI_IMMOBILIARI) ORDER BY NOME"
                Dim myReader As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                While myReader.Read
                    cmbStruttura.Items.Add(New ListItem(par.IfNull(myReader("NOME"), "--"), par.IfNull(myReader("id"), -1)))
                End While
                myReader.Close()
    ...
    Protected Sub btnAvanti_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnAvanti.Click '"&UI=" & chTipoUI.SelectedItem.Value &
        ...
        If Request.QueryString("U") = "1" Then
            If LBLok.Value = "1" Then
                Response.Write("<script>window.open('TotPatrimTipoUInew.aspx?F=" & par.PulisciStrSql(cmbStruttura.SelectedValue) & "&C=" & par.PulisciStrSql(cmbComplesso.SelectedItem.Value) & "&TOT=" & cmbTotali.SelectedItem.Value & "&UI=" & tipiUI & "')</script>")
            End If
        End If
        If Request.QueryString("U") = "2" Then
            If LBLok.Value = "1" Then
                Response.Write("<script>window.open('TotPatrimStatoUI.aspx?F=" & par.PulisciStrSql(cmbStruttura.SelectedValue) & "&C=" & par.PulisciStrSql(cmbComplesso.SelectedItem.Value) & "&T=" & par.PulisciStrSql(cmbTipoUI.SelectedItem.Value) & "&D=" & par.PulisciStrSql(cmbTipoUI.SelectedItem.Text) & "&TOT=" & cmbTotali.SelectedItem.Value & "')</script>")
            End If
        End If
        If Request.QueryString("U") = "3" Then
            If LBLok.Value = "1" Then
                Response.Write("<script>window.open('TotPatrimGruppiUI.aspx?F=" & par.PulisciStrSql(cmbStruttura.SelectedValue) & "&C=" & par.PulisciStrSql(cmbComplesso.SelectedItem.Value) & "&TOT=" & cmbTotali.SelectedItem.Value & "')</script>")
            End If
        End If
    End Sub
    ...
    Protected Sub cmbStruttura_SelectedIndexChanged(sender As Object, e As System.EventArgs) Handles cmbStruttura.SelectedIndexChanged
        Try
            ...
            If cmbStruttura.SelectedValue <> "-1" Then
                cmbComplesso.Items.Clear()
                cmbComplesso.Items.Add(New ListItem("TUTTI I COMPLESSI", "-10"))
                par.cmd.CommandText = "SELECT complessi_immobiliari.id,COD_COMPLESSO,denominazione FROM SISCOM_MI.complessi_immobiliari, SISCOM_MI.indirizzi where complessi_immobiliari.ID_INDIRIZZO_RIFERIMENTO=indirizzi.id and SISCOM_MI.complessi_immobiliari.id <> 1 AND ID_FILIALE='" & cmbStruttura.SelectedValue & "' order by denominazione asc "
                Dim myReader1 As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                While myReader1.Read
                    cmbComplesso.Items.Add(New ListItem(par.IfNull(myReader1("denominazione"), "--") & " - - " & " cod." & par.IfNull(myReader1("cod_complesso"), " "), par.IfNull(myReader1("id"), -1)))
                End While
