...
Partial Class CENSIMENTO_RptUnitaImm
    ...
    Protected Sub ddlscala_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlscala.SelectedIndexChanged
        Try
            ...
            End If
            If Me.ddlcivico.SelectedValue <> "-1" Then
                par.caricaComboBox("SELECT distinct unita_immobiliari.interno FROM SISCOM_MI.unita_immobiliari where unita_immobiliari.id_indirizzo in (select id from siscom_mi.indirizzi where civico='" & Me.ddlcivico.SelectedValue.ToString & "' and descrizione='" & par.PulisciStrSql(ddlindirizzo.SelectedItem.Text) & "') AND UNITA_IMMOBILIARI.id_scala=" & ddlscala.SelectedItem.Value & " order by interno asc", ddlinterno, "INTERNO", "INTERNO", True)
            Else
                par.caricaComboBox("SELECT distinct unita_immobiliari.interno FROM SISCOM_MI.unita_immobiliari where unita_immobiliari.id_indirizzo in (select id from siscom_mi.indirizzi where descrizione='" & par.PulisciStrSql(ddlindirizzo.SelectedItem.Text) & "') AND UNITA_IMMOBILIARI.id_scala=" & ddlscala.SelectedItem.Value & " order by interno asc", ddlinterno, "INTERNO", "INTERNO", True)
            End If
        Catch ex As Exception
            Session.Add("ERRORE", "Provenienza:" & Page.Title & " " & ex.Message)
            Response.Write("<script>top.location.href=""../Errore.aspx""</script>")
    ...
    Private Sub AvviaRicerca()
        Try
            If Controllo.Value = 0 Then
                ...
                RequestQueryString = RequestQueryString & "&CC=" & par.AggiustaData(txtdatachiusi.Text) & "&CA=" & par.AggiustaData(txtdataaperti.Text) & "&CB=" & par.AggiustaData(txtdatabozza.Text)
                RequestQueryString = RequestQueryString & "&SCC=" & SceltaCheck(Me.cblstatochiusi) & "&SCA=" & SceltaCheck(Me.cblstatoaperti) & "&ONFILT=" & SoloFiltro
                Response.Write("<script>window.open('RisultatiRptUnitaImm.aspx?" & RequestQueryString & "','ReportUnitaImm','scrollbars=yes,resizable=yes');</script>")
            Else
