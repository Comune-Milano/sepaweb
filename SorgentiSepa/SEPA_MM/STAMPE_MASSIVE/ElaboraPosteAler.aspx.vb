        ...
        Try
            If FileUpload1.HasFile = True Then
                ...
                Do While rdr.Read()
                    ...
                    CodTipoEsito = ""
                    par.cmd.CommandText = "SELECT DESCRIZIONE FROM SISCOM_MI.TIPOLOGIA_ESITI_POSTALER WHERE ID = " & par.IfEmpty(rdr.Item(15).ToString, -1)
            ...
            For Each oDataGridItem In Me.DataGridExcel.Items
                ...
                If ID > 0 And IdTipoLettera > 0 Then
                    ...
                    If myReader1.Read Then
                        ...
                        If IdTipoLettera = 1 Then
                            Dim EffettuatoEsitoManuale As Boolean = False

                            'CONTROLLO SE EFFETTUATO un ESITO MANUALE di POSTALER
                            par.cmd.CommandText = " select count(*) from SISCOM_MI.MOROSITA_EVENTI where ID_MOROSITA_LETTERE=" & ID_MOROSITA_LETTERE

                            Dim myReader2 As Oracle.DataAccess.Client.OracleDataReader
                            myReader2 = par.cmd.ExecuteReader()
                            If myReader2.Read Then
                                If myReader2(0) > 1 Then
                                    EffettuatoEsitoManuale = True
                                End If
                            End If
                            myReader2.Close()
                            '*****************************************************

                            If EffettuatoEsitoManuale = False Then
                                '******** LETTERA 1 (MAV MG (tipo lettera AB) - MAV MA (Tipo lettera CD) - MAV MG (Tipo Lettera EF)
                                ...
                                par.cmd.CommandText = " update  SISCOM_MI.MOROSITA_LETTERE " _
                                                    & " set COD_STATO='M" & Format(ID_TIPO, "00") & "'" _
                                                    & " where ID=" & ID_MOROSITA_LETTERE
                            ...
                            '******** LETTERA 2 (MAV MA (Tipo lettera AB)
                            par.cmd.CommandText = "select ID,COD_CONTRATTO,BOLLETTINO  " _
                                                & " from SISCOM_MI.MOROSITA_LETTERE " _
                                                & " where ID in (select ID_LETTERA_2 from SISCOM_MI.POSTALER where ID_TIPO_LETTERA=1 and ID_LETTERA=" & ID_MOROSITA_LETTERE & ")"

                            myReader2 = par.cmd.ExecuteReader()
                            If myReader2.Read Then

                                EffettuatoEsitoManuale = False
                                'CONTROLLO SE EFFETTUATO un ESITO MANUALE di POSTALER
                                par.cmd.CommandText = " select count(*) from SISCOM_MI.MOROSITA_EVENTI where ID_MOROSITA_LETTERE=" & par.IfNull(myReader2("ID"), -1)

                                Dim myReader3 As Oracle.DataAccess.Client.OracleDataReader
                                myReader3 = par.cmd.ExecuteReader()
                                If myReader3.Read Then
                                    If myReader3(0) > 1 Then
                                        EffettuatoEsitoManuale = True
                                    End If
                                End If
                                myReader3.Close()
                                ...
                                If EffettuatoEsitoManuale = False Then
                                    ...
                                    par.cmd.CommandText = "insert into SISCOM_MI.MOROSITA_EVENTI " _
                                                        & " (ID,ID_MOROSITA_LETTERE,ID_OPERATORE,DATA_ORA,COD_EVENTO,MOTIVAZIONE) " _
                                                        & "values (SISCOM_MI.SEQ_MOROSITA_EVENTI.NEXTVAL," & par.IfNull(myReader2("ID"), -1) & "," _
                                                        & Session.Item("ID_OPERATORE") & "," & Format(Now, "yyyyMMddHHmmss") & "," _
                                                        & "'M" & Format(ID_TIPO, "00") & "','" & sStr1 & "')"
