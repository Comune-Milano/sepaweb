...
Partial Class Contratti_Anagrafica_RisultatiRicerca
    ...
    Protected Sub DataGrid1_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles DataGrid1.ItemDataBound
        If e.Item.ItemType = ListItemType.Item Then
            ...
            e.Item.Attributes.Add("onclick", "if (Selezionato) {Selezionato.style.backgroundColor='';}Selezionato=this;this.style.backgroundColor='red';document.getElementById('txtmia').value='Hai selezionato: " & Replace(e.Item.Cells(0).Text, "'", "\'") & "';document.getElementById('txtid').value='" & e.Item.Cells(1).Text & "';")
    ...
    Protected Sub btnSeleziona_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles btnSeleziona.Click
        If Me.txtid.Text <> "" Then
            Response.Write("<script>window.opener.document.forms['form1'].elements['LBLintest'].value='" & txtid.Text & "';window.opener.document.getElementById('btnAggComp').click();self.close();</script>")
        Else
            Response.Write("<script>alert('Nessuna riga selezionata!')</script>")

            'if(!parent.window.opener.closed){if (parent.window.opener.document.getElementById('MainContent_Charge')) {parent.window.opener.document.getElementById('MainContent_Charge').value = 'U'};parent.window.opener.document.getElementById('form1').submit();}window.defaultStatus= 'closed';self.close();
        End If
    End Sub
End Class
