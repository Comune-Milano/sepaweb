...
Partial Class VSA_NuovaDomandaVSA_Tab_Decisioni
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            ElseIf CType(Me.Page.FindControl("tipoRichiesta"), HiddenField).Value = "10" Then
                ...
                btnRiduzCanone.Attributes.Add("onclick", "javascript:document.getElementById('H1').value='0';ConfRiduzCanone();")
            ElseIf CType(Me.Page.FindControl("tipoRichiesta"), HiddenField).Value = "12" Then
                ChkMotivi.Visible = True
                ChkMotiviRies.Visible = True
                btnRiduzCanone.Text = "AUTORIZZA RATEIZZ. STRAORDINARIA"
                lblTitolo.Text = "Decisioni per Rateizz. Straordinaria"
                btnRiduzCanone.Attributes.Add("onclick", "javascript:document.getElementById('H1').value='0';ConfRiduzCanone();")
            End If
    ...
    Private Sub CaricaMotivazioni()
        Try
            ...
            If CType(Me.Page.FindControl("btnSalva"), Button).Visible = False Then
                DisattivaTutto()
            End If
    ...
    Public Sub GestioneStatoD()
        Try
            ...
            If myReader.Read Then
                ...
                lblCredito.Text = "" & tipo & " calcolato: <b><a href='../DettaglioCredito.aspx?ID=" & CType(Me.Page, Object).lIdDomanda & "' target='_blank'>" & Format(Math.Abs(par.IfNull(myReader("CREDITO_TEORICO"), 0)), "##,##0.00") & "</a></b> - Dal: <b>" & par.FormattaData(myReader("CT_DAL")) & "</b> Al: <b>" & par.FormattaData(myReader("CT_AL")) & "</b>"
            ...
            par.cmd.CommandText = "SELECT * FROM EVENTI_BANDI_VSA WHERE ID_DOMANDA = " & CType(Me.Page, Object).lIdDomanda & " AND (COD_EVENTO = 'F179' or COD_EVENTO = 'F165' or COD_EVENTO = 'F196' or COD_EVENTO = 'F197' or COD_EVENTO = 'F204' or COD_EVENTO='F208' or COD_EVENTO='F210' or COD_EVENTO='F212' or COD_EVENTO='F213' or COD_EVENTO='F214')"
            lettore = par.cmd.ExecuteReader
            If lettore.Read Then
                GiaRidotto = 1
            ...
            If Me.Page.FindControl("btnSalva").Visible = False Or GiaRidotto = 1 Then
                DisattivaTutto()
            End If
            If CType(Me.Page.FindControl("tipoRichiesta"), HiddenField).Value = "12" And Me.rdbListDecisione.SelectedValue = "3" Then
                DisattivaTutto()
            End If
    ...
    Public Sub DisattivaTutto()
        ...
        Me.rdbListDecisione.Enabled = False
        Me.rdbListRiesame.Enabled = False
        If CType(Me.Page.FindControl("tipoRichiesta"), HiddenField).Value <> "12" Then
            Me.txtNoteDecisione.Enabled = False
        End If
        Me.txtNoteRiesame.Enabled = False
        Me.btnRiduzCanone.Enabled = False
    ...
    Protected Sub btnRiduzCanone_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnRiduzCanone.Click
        If CType(Me.Page.FindControl("ConfRidCan"), HiddenField).Value = 1 Then
            If CType(Me.Page.FindControl("lblISBAR"), TextBox).Text > 0 Or CType(Me.Page.FindControl("ISEEnullo"), HiddenField).Value = 1 Then
                If txtdataAUTORIZ.Text = "" Then
                    ...
                    If par.AggiustaData(txtdataAUTORIZ.Text) < par.AggiustaData(dataPr) Then
                        ...
                        CType(Me.Page, Object).MessJQuery("La data di autorizzazione non può essere antecedente alla data di presentazione della domanda!", 0, "Attenzione")
                    Else
                ...
                Else
                    If IsDate(txtdataAUTORIZ.Text) = False Then
                        ...
                        CType(Me.Page, Object).MessJQuery("La data di autorizzazione inserita non è valida!", 0, "Attenzione")
                    Else
                        If par.AggiustaData(txtdataAUTORIZ.Text) < par.AggiustaData(dataPr) Then
                            'Response.Write("<script>alert('Attenzione...La data di autorizzazione non può essere antecedente alla data di presentazione della domanda!')</script>")
                            CType(Me.Page, Object).MessJQuery("La data di autorizzazione non può essere antecedente alla data di presentazione della domanda!", 0, "Attenzione")
            ...
            Else
                ...
                CType(Me.Page, Object).MessJQuery("IMPOSSIBILE PROCEDERE! ISEE NON CALCOLATO!\nPer procedere salvare prima la domanda e poi cliccare su stampa!", 0, "Attenzione")
            End If
        End If
    End Sub
