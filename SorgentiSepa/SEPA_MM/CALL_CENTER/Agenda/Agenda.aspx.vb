...
Partial Class CALL_CENTER_Agenda_Agenda
    Inherits System.Web.UI.Page
    Dim par As New CM.Global
    Public connData As CM.datiConnessione = Nothing
    Public dataAppuntamento As Date
    ...
    Private Sub SettaFiliali()
        Try
            ...
            par.cmd.CommandText = "SELECT ID_STRUTTURA FROM SISCOM_MI.SEGNALAZIONI WHERE ID=" & idSegnalazione.Value
            Dim idFiliale As String = par.IfNull(par.cmd.ExecuteScalar, "-1")
            cmbStruttura.SelectedValue = idFiliale
            connData.chiudi()
    ...
    Private Function TrovaAppuntamentoGiornaliero(ByVal Giorno As Integer, ByVal Appuntamento As Integer) As Boolean
        Try
            ...
            CType(Page.FindControl("Dettaglio" & Appuntamento), Label).Text = "<img onclick=""javascript:DettagliAppuntamenti('" & Format(CInt(Giorno), "00") & "', '" & Format(CInt(MeseSelezionato.Value), "00") & "', '" & Format(CInt(AnnoSelezionato.Value), "0000") & "'," & cmbStruttura.SelectedValue & "," & idSegnalazione.Value & ");"" src=""../../NuoveImm/Aggiungi.png"" style=""cursor:pointer"" width=""12"" height=""12"" />"
            ...
            For i As Integer = 1 To 42 Step 1
                ...
                Else
                    ...
                    If Not par.IsFestivo(dataGiorno, True) Then
                        ...
                        If Format(dataAppuntamento, "dd/MM/yyyy") = dataGiorno Then
    ...
    Private Sub CalcolaQuattroSlotDisponibili()
                ...
                If Not par.IsFestivo(dataMinimaSlot3) And Not par.IsWeekEnd(dataMinimaSlot3, True) Then
                    ...
                    For Each orario As Data.DataRow In dt.Rows
                        ...
                        If cont < totAppSport Then
                            par.cmd.CommandText = "SELECT * FROM SISCOM_MI.TAB_FILIALI WHERE TAB_FILIALI.ID>100 AND ID_TIPO_ST=0 AND ID<>" & cmbStruttura.SelectedValue & " ORDER BY ID"
        ...
        Catch ex As Exception
            connData.chiudi()
            Session.Add("ERRORE", "Provenienza:" & Page.Title & " - CalcolaQuattroSlotDisponibili - " & ex.Message)
            Response.Redirect("../../Errore.aspx", False)
        End Try
    End Sub
    ...
    Private Sub gestioneGiornoAppuntamento()
        Try
            connData.apri()
            par.cmd.CommandText = "SELECT ID_STRUTTURA FROM SISCOM_MI.APPUNTAMENTI_CALL_CENTER WHERE ID_SEGNALAZIONE=" & idSegnalazione.Value _
                & " AND ID=(SELECT MAX(ID) FROM SISCOM_MI.APPUNTAMENTI_CALL_CENTER WHERE ID_SEGNALAZIONE=" & idSegnalazione.Value & ")"

            Dim lettore As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader
            If lettore.HasRows Then
                If lettore.Read Then
                    Try
                        cmbStruttura.SelectedValue = par.IfNull(lettore("ID_sTRUTTURA"), "0")
                    Catch ex As Exception
                    End Try
                End If

                par.cmd.CommandText = "SELECT TO_CHAR(TO_DATE(DATA_APPUNTAMENTO,'yyyyMMdd'),'dd/MM/yyyy') AS DATA_APPUNTAMENTO " _
                & " FROM SISCOM_MI.APPUNTAMENTI_CALL_CENTER WHERE ID_SEGNALAZIONE=" & idSegnalazione.Value _
                & " AND ID=(SELECT MAX(ID) FROM SISCOM_MI.APPUNTAMENTI_CALL_CENTER WHERE ID_SEGNALAZIONE=" & idSegnalazione.Value & ")"
                Dim lettoreAppuntamento As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader

                If lettoreAppuntamento.HasRows Then
                    If lettoreAppuntamento.Read Then
                        dataAppuntamento = par.IfNull(lettoreAppuntamento("DATA_APPUNTAMENTO"), Format(Now, "dd/MM/yyyy"))
                    End If
                Else
                    dataAppuntamento = Format(Now, "dd/MM/yyyy")
                End If
                lettoreAppuntamento.Close()
                Dim DataSelezionata As Date
                If IsDate(dataAppuntamento) Then
                    DataSelezionata = dataAppuntamento
                Else
                    DataSelezionata = Format(Now, "dd/MM/yyyy")
                End If
                GiornoSelezionato.Value = DataSelezionata.ToString("dd")
                MeseSelezionato.Value = DataSelezionata.ToString("MM")
                AnnoSelezionato.Value = DataSelezionata.ToString("yyyy")
        ...
        Catch ex As Exception
            connData.chiudi()
            Session.Add("ERRORE", "Provenienza:" & Page.Title & " - gestioneGiornoAppuntamento - " & ex.Message)
            Response.Redirect("../../Errore.aspx", False)
        End Try
        SettaMese()
        CalcolaQuattroSlotDisponibili()
    ...
    Protected Sub ButtonExportAgendaAppuntamenti_Click(sender As Object, e As System.EventArgs) Handles ButtonExportAgendaAppuntamenti.Click
        Try
            ...
            par.cmd.CommandText = " SELECT  ID_sEGNALAZIONE AS ""N°"", " _
                ...
                & " AND APPUNTAMENTI_cALL_cENTER.ID_SPORTELLO=APPUNTAMENTI_SPORTELLI.ID(+) " _
                & " AND APPUNTAMENTI_CALL_CENTER.ID_ORARIO=APPUNTAMENTI_ORARI.ID(+) order by 1 desc"
