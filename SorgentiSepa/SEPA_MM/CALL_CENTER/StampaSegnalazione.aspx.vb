    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            Try
                ...
                par.cmd.CommandText = "SELECT SISCOM_MI.SEGNALAZIONI.*,SISCOM_MI.tab_stati_segnalazioni.descrizione as statos,(SELECT MAX (SOLLECITO) FROM SISCOM_MI.SEGNALAZIONI_NOTE WHERE SEGNALAZIONI_NOTE.ID_SEGNALAZIONE=SEGNALAZIONI.ID) AS SOLLECITO " _
                   & "FROM SISCOM_MI.SEGNALAZIONI,SISCOM_MI.TAB_STATI_SEGNALAZIONI WHERE SISCOM_MI.SEGNALAZIONI.ID_STATO=SISCOM_MI.TAB_STATI_SEGNALAZIONI.ID AND SISCOM_MI.SEGNALAZIONI.ID=" & Request.QueryString("ID")
                ...
                If myReader1.Read Then
                    ...
                    par.cmd.CommandText = "select (case when length(data_ora)>=12 then to_char(to_Date(substr(data_ora,1,8),'yyyyMMdd'),'dd/MM/yyyy')||' '||substr(data_ora,9,2)||':'||substr(data_ora,11,2) else null end )  as data,note from siscom_mi.segnalazioni_note where id_segnalazione=" & Request.QueryString("ID") & " order by data_ora asc"
                    Dim note As String = ""
