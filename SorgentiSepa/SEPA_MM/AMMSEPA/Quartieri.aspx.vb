...
Partial Class AMMSEPA_Commissariati
    ...
    Private Sub BindGrid()
        Try
            ...
            Dim Str As String = "SELECT * FROM SISCOM_MI.TAB_QUARTIERI ORDER BY nome asc"
            Dim da As New Oracle.DataAccess.Client.OracleDataAdapter(Str, par.OracleConn)

            Dim ds As New Data.DataSet()

            da.Fill(ds, "SISCOM_MI.TAB_QUARTIERI")
        ...
        Try
            ...
            ElseIf Me.TextBox1.Value = 1 Then
                ...
                If par.IfEmpty(Me.txtQuartiere.Text, "") <> "" Then
                    ...
                    par.cmd.CommandText = "select * FROM SISCOM_MI.TAB_QUARTIERI WHERE upper(NOME) = '" & par.PulisciStrSql(Me.txtQuartiere.Text.ToUpper) & "'"
                    ...
                    End If
                    par.cmd.CommandText = "INSERT INTO SISCOM_MI.TAB_QUARTIERI (ID,NOME) VALUES (SISCOM_MI.SEQ_TAB_QUARTIERI.NEXTVAL,'" & par.PulisciStrSql(Me.txtQuartiere.Text) & "')"
                    par.cmd.ExecuteNonQuery()
                    Try
                        Dim operatore As String = Session.Item("ID_OPERATORE")
                        Eventi_Gestione(operatore, "F55", "INSERIMENTO QUARTIERE " & txtQuartiere.Text)
                    Catch ex As Exception
                        lblErrore.Text = ex.Message
                        lblErrore.Visible = True
                    End Try
    ...
    Protected Sub ImgBtnAggiungi_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImgBtnAggiungi.Click
        Try
            ...
            If par.IfEmpty(Me.txtid.Value, "") <> "" Then
                ...
                par.cmd.CommandText = "DELETE FROM SISCOM_MI.TAB_QUARTIERI WHERE ID = " & Me.txtid.Value
                par.cmd.ExecuteNonQuery()
                stringa = txtmia.Text
                NomeCommissariato()
                Try
                    Dim operatore As String = Session.Item("ID_OPERATORE")
                    Eventi_Gestione(operatore, "F56", "CANCELLAZIONE QUARTIERE" & stringa)
                Catch ex As Exception
                    lblErrore.Text = ex.Message
                    lblErrore.Visible = True
                End Try
        ...
        Catch EX1 As Oracle.DataAccess.Client.OracleException
            ...
            If EX1.Number = 2292 Then
                lblErrore.Text = "Quartiere in uso. Non è possibile eliminare!"
            Else
    ...
    Protected Sub ImgModifica_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImgModifica.Click
        If txtid.Value.ToString <> "" Then
            ...
            par.cmd.CommandText = "SELECT NOME FROM SISCOM_MI.TAB_QUARTIERI WHERE ID = " & Me.txtid.Value

            Dim myReader As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
            If myReader.Read Then
                Me.txtQuartiere.Text = myReader("NOME").ToString
            End If
            myReader.Close()
        ...
        Try
            ...
            If par.IfEmpty(Me.txtQuartiere.Text, "Null") <> "Null" Then
                ...
                If par.OracleConn.State = Data.ConnectionState.Closed Then
                    ...
                    par.SettaCommand(par)
                End If
                par.cmd.CommandText = "select * FROM SISCOM_MI.TAB_QUARTIERI WHERE upper(NOME) = '" & par.PulisciStrSql(Me.txtQuartiere.Text.ToUpper) & "' AND ID <>" & txtid.Value
                Dim myReader As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                If myReader.Read Then
                    Response.Write("<script>alert('Valore già inserito!')</script>")
                    Me.txtQuartiere.Text = ""
                ...
                par.cmd.CommandText = "UPDATE SISCOM_MI.TAB_QUARTIERI SET NOME = '" & par.PulisciStrSql(Me.txtQuartiere.Text.ToUpper) & "' WHERE ID = " & Me.txtid.Value
                par.cmd.ExecuteNonQuery()
                ...
                Try
                    Dim operatore As String = Session.Item("ID_OPERATORE")
                    Eventi_Gestione(operatore, "F02", "MODIFICA QUARTIERE" & stringa)
                Catch ex As Exception
                    lblErrore.Text = ex.Message
                    lblErrore.Visible = True
                End Try
