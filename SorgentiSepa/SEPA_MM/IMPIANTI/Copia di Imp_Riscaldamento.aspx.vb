...
Partial Class IMPIANTI_Imp_Riscaldamento
                ...
                For i = 0 To lstEdificiCT.Count - 1
                    ...
                    If lstEdificiCT.Item(i).ID = oDataGridItem.Cells(0).Text Then
                        ...
                        If lstEdificiCT.Item(i).CHK = True Then
                            Me.txtContaEdifici.Value = Me.txtContaEdifici.Value & "javascript:void(0)"  & lstEdificiCT(i).ID & "javascript:void(0)" 
                            SommaUI = SommaUI + oDataGridItem.Cells(2).Text
                            SommaMQ = SommaMQ + oDataGridItem.Cells(4).Text
                            SommaEdifici = SommaEdifici + 1
                        Else
                            Me.txtContaEdifici.Value.Replace("javascript:void(0)"  & lstEdificiCT(i).ID & "javascript:void(0)" , "javascript:void(0)" )
                        End If
                        Exit For
                    End If
        ...
        Try
            ...
            If Me.cmbComplesso.SelectedValue <> "-1" Then
                ...
                par.cmd.CommandText = "select distinct ID,(DENOMINAZIONE||' - -Cod.'||COD_EDIFICIO) as DENOMINAZIONE," _
                                                & "(select count(*) from SISCOM_MI.UNITA_IMMOBILIARI where ID_EDIFICIO=EDIFICI.ID and COD_TIPOLOGIA='AL') as TOTALE_UI," _
                                                & "(select SUM (( " _
                                                 ...
                                                 & " replace(replace('<a href=£javascript:void(0)£ onclick=£window.showModalDialog(''Tab_Termico_Unita.aspx?TIPO=NORM&IDVISUAL=" & CType(Me.Page.FindControl("SOLO_LETTURA"), TextBox).Text & "&ID=" & CType(Me.Page.FindControl("txtContaEdifici"), HiddenField).Value & "&ED='||SISCOM_MI.EDIFICI.ID||''',''Dettagli'',''dialogWidth:800px;dialogHeight:550px;'');window.form1.submit();£>Visualizza</a>','$','&'),'£','" & Chr(34) & "') as  UNITA  " _
                ...
                While myReader1.Read
                    ...
                    If vIdImpianto <> 0 Then
                        gen = New Epifani.EdificiCT(par.IfNull(myReader1("ID"), -1), par.IfNull(myReader1("DENOMINAZIONE"), " "), par.IfNull(myReader1("TOTALE_UI"), 0), par.IfNull(myReader1("TOTALE_UI"), 0), par.IfNull(myReader1("TOTALE_MQ"), 0), par.IfNull(myReader1("TOTALE_MQ"), 0), par.IfNull(myReader1("UNITA"), ""), False)
                    Else
                ...
                If vIdImpianto <> 0 And Caricamento = True Then
                    ...
                    par.cmd.CommandText = "select distinct ID,(DENOMINAZIONE||' - -Cod.'||COD_EDIFICIO) as DENOMINAZIONE," _
                                                    ...
                                                    & "(select SUM (( " _
                                                            ...
                                                            & " where ID_UNITA_IMMOBILIARE=SISCOM_MI.UNITA_IMMOBILIARI.ID and COD_TIPOLOGIA='SUP_NETTA')) " _
                                                     & " from SISCOM_MI.UNITA_IMMOBILIARI where ID_EDIFICIO=EDIFICI.ID and COD_TIPOLOGIA='AL') as TOTALE_MQ, " _
                                                     & " replace(replace('<a href=£javascript:void(0)£ onclick=£window.showModalDialog(''Tab_Termico_Unita.aspx?TIPO=EXTRA&IDVISUAL=" & CType(Me.Page.FindControl("SOLO_LETTURA"), TextBox).Text & "&ID=" & CType(Me.Page.FindControl("txtContaEdifici"), HiddenField).Value & "&ED='||SISCOM_MI.EDIFICI.ID||''',''Dettagli'',''dialogWidth:800px;dialogHeight:550px;'');window.form1.submit();£>Visualizza</a>','$','&'),'£','" & Chr(34) & "') as  UNITA  " _
                                           & " from SISCOM_MI.EDIFICI " _
    ...
    Private Sub AddJavascriptFunction()
        Try
            ...
            For i = 0 To CType(Tab_Termico_Edifici.FindControl("DataGridEdifici"), DataGrid).Items.Count - 1
                ...
                CType(di.Cells(0).FindControl("CheckBox1"), CheckBox).Attributes.Add("onclick", "javascript:SommaUI(" & par.VirgoleInPunti(di.Cells(2).Text) & "," & par.VirgoleInPunti(di.Cells(4).Text) & "," & di.Cells(0).Text & ",this);") '"SommaUI(" & di.Cells(1) & ");")
    ...
    Protected Sub carnazza(ByVal sender As Object, ByVal e As System.EventArgs)
        Beep()
    End Sub
End Class
