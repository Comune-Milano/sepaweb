...
Partial Class ANAUT_ApplicazioneGruppoAU
    ...
    Private Function Carica()
        Try
            ...
            Dim sr123 As StreamWriter = New StreamWriter(Server.MapPath("..\FileTemp\da_eseguire.txt"), False, System.Text.Encoding.Default)
                ...
                If DAFARE = True Then
                    ...
                    If myReader1.Read Then
                        ...
                        If AreaEconomica <> -1 Then
                            ...
                            ROW.Item("PSE") = sPSE
                            ROW.Item("VSE") = sVSE
                            ROW.Item("REDDITI_DIPENDENTI") = Format(CDbl(sREDD_DIP), "##,##0.00")
                            ROW.Item("REDDITI_ALTRI") = Format(CDbl(sREDD_ALT), "##,##0.00")
                            ROW.Item("COEFF_NUCLEO_FAM") = sCOEFFFAM
                            ROW.Item("LIMITE_PENSIONI") = sLimitePensione
                            ROW.Item("REDD_PREV_DIP") = sPREVDIP
                            ROW.Item("REDD_COMPLESSIVO") = Format(CDbl(sCOMPLESSIVO), "##,##0.00")
                            ROW.Item("REDD_IMMOBILIARI") = Format(CDbl(sIMMOBILIARI), "##,##0.00")
                            ROW.Item("REDD_MOBILIARI") = Format(CDbl(sMOBILIARI), "##,##0.00")
                            ...
                            Dim url As String = Replace(Replace(Replace(myReader1("cod_contratto") & "_" & myReader1("PRESSO_COR"), "\", "."), "/", "."), "*", ".")
                            ...
                            If System.IO.File.Exists(Server.MapPath("..\FileTemp\") & url & ".TXT") = True Then
                                par.cmd.CommandText = "DELETE FROM SISCOM_MI.CANONI_EC_ELABORAZIONI WHERE ID_BANDO_AU=" & IDAU & " AND ID_CONTRATTO=" & myReader1("id")
                                ...
                                Else
                                    par.cmd.CommandText = "INSERT INTO SISCOM_MI.CANONI_EC_ELABORAZIONI (CANONE_CLASSE,CANONE_SOPPORTABILE,DECADENZA_ALL_ADEGUATO,DECADENZA_VAL_ICI,CANONE_MINIMO_AREA,VALORE_LOCATIVO,SUP_ACCESSORI,MINORI_15,MAGGIORI_65,DATA_STIPULA,COD_CONTRATTO,ID_CONTRATTO,DATA_CALCOLO,ID_AREA_ECONOMICA,ISEE,ISE, ISR, ISP, PSE, VSE, REDDITI_DIP, REDDITI_ATRI," _
                                                        ...
                                                        & par.PulisciStrSql(sLOCALITA) & "','" & sASCENSORE & "','" & par.PulisciStrSql(sDESCRIZIONEPIANO) & "','" & sSUPNETTA & "','" & par.PulisciStrSql(sALTRESUP) & "','" & sISTAT & "','" & sCANONECLASSEISTAT & "','" & CANONE91 & "','" & sANNOINIZIOVAL & "','" & sANNOFINEVAL & "') "

                                End If

                                sr123.WriteLine(par.cmd.CommandText)
                                ...
                                par.cmd.CommandText = "UPDATE SISCOM_MI.RAPPORTI_UTENZA SET IMP_CANONE_INIZIALE=" & par.VirgoleInPunti(sCanone) & " WHERE COD_CONTRATTO='" & myReader1("cod_contratto") & "'"
                                sr123.WriteLine(par.cmd.CommandText)

                                par.cmd.ExecuteNonQuery()

                                par.cmd.CommandText = "INSERT INTO SISCOM_MI.EVENTI_CONTRATTI (ID_CONTRATTO,ID_OPERATORE,DATA_ORA,COD_EVENTO,MOTIVAZIONE) VALUES (" & myReader1("ID") & ",1,'" & Format(Now, "yyyyMMddHHmmss") & "','F02','" & par.PulisciStrSql("APPLICAZIONE CANONE AU 2009 (EURO " & sCanone & "). CANONE PRECEDENTE (" & par.IfNull(myReader1("IMP_CANONE_INIZIALE"), "") & ")") & "')"
                                sr123.WriteLine(par.cmd.CommandText)

                                par.cmd.ExecuteNonQuery()

                                par.cmd.CommandText = "UPDATE UTENZA_GRUPPI_DICHIARAZIONI SET APPLICA_AU=1 WHERE ID_DICHIARAZIONE=" & myReader("ID") & " AND ID_GRUPPO=" & myReader("ID_GRUPPO")
                                par.cmd.ExecuteNonQuery()
            ...
            Else
                Response.Write("<script>alert('Nessuna AU a cui applicare il canone.');</script>")
            End If
