...
Partial Class ANAUT_GestGruppiConv
    ...
    Private Sub BindGrid()
        ...
        Try
            ...
            Dim Str As String = "SELECT replace(replace('<a href=£javascript:void(0)£ onclick=£window.open(''DettagliLista.aspx?IDG='||UTENZA_GRUPPI_CONV.ID||''',''Dettagli'','''');£>'||'DETTAGLI'||'</a>','$','&'),'£','" & Chr(34) & "') as VISUALIZZA,UTENZA_GRUPPI_CONV.*,UTENZA_BANDI.DESCRIZIONE AS NOME FROM UTENZA_BANDI,UTENZA_GRUPPI_CONV WHERE UTENZA_BANDI.ID=UTENZA_GRUPPI_CONV.ID_AU ORDER BY ID_AU DESC, UTENZA_GRUPPI_CONV.DESCRIZIONE ASC"
    ...
    Protected Sub ImgBtnElimina_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ImgBtnElimina.Click
        If eliminato.Value = "1" Then
            Try
                ...
                par.cmd.CommandText = "UPDATE UTENZA_LISTE_CDETT SET ID_GRUPPO_CONV=NULL where ID_GRUPPO_CONV = " & txtid.Value
                par.cmd.ExecuteNonQuery()

                par.cmd.CommandText = "delete from UTENZA_GRUPPI_CONV where id=" & txtid.Value
                par.cmd.ExecuteNonQuery()
