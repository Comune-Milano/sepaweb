...
Partial Class ANAUT_RicercaNonR
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            Try
                ...
                par.RiempiDList(Me, par.OracleConn, "cmbFiliale", "select * from siscom_mi.tab_filiali where id<>58 and id_tipo_st=0 and acronimo is not null ORDER BY nome asc", "NOME", "ID")
                'Else
    ...
    Protected Sub btnCerca_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnCerca.Click
        If AAl.Text = "" Then
            Response.Write("<script>alert('Inserire la data nel campo AL dell\'appuntamento!');</script>")
            Exit Sub
        End If
        If cmbFiliale.SelectedItem.Value <> "-1" Then
            Response.Write("<script>location.replace('RisultatoNonR.aspx?BA=" & cmbBando.SelectedItem.Value & "&FI=" & cmbFiliale.SelectedItem.Value & "&SDAL=" & par.AggiustaData(txtStipulaDal.Text) & "&SAL=" & par.AggiustaData(txtStipulaAl.Text) & "&ADAL=" & par.AggiustaData(ADal.Text) & "&AAL=" & par.AggiustaData(AAl.Text) & "&ES=" & CheckBox1.Checked & "');</script>")
        Else
