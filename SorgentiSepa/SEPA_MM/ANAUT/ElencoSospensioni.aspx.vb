...
Partial Class ANAUT_ElencoSospensioni
    ...
    Private Function Carica()
        ...
        par.cmd.CommandText = "SELECT COUNT(ID) FROM UTENZA_DICHIARAZIONI WHERE id_bando=" & Indice & " AND FL_sospensione='1' AND POSIZIONE IS NULL"
        myReader = par.cmd.ExecuteReader()
        If myReader.Read Then
            If myReader(0) > 0 Then
                lblContatore.Text = lblContatore.Text & " Trovate : " & myReader(0) & " dichiarazioni SOSPESE non legate a nessun rapporto (Cod.Convocazione non trovato)"
            End If
        End If
        myReader.Close()

      
        par.cmd.CommandText = "SELECT (CASE WHEN FL_SOSP_1='1' THEN '1) ' ELSE '' END)||(CASE WHEN FL_SOSP_2='1' THEN '2) ' ELSE '' END)||(CASE WHEN FL_SOSP_3='1' THEN '3) ' ELSE '' END)||(CASE WHEN FL_SOSP_4='1' THEN '4) ' ELSE '' END)||(CASE WHEN FL_SOSP_5='1' THEN '5) ' ELSE '' END)||(CASE WHEN FL_SOSP_6='1' THEN '6) ' ELSE '' END)||(CASE WHEN FL_SOSP_7='1' THEN '7) ' ELSE '' END) AS MOTIVAZIONI,UTENZA_DICHIARAZIONI.*,UTENZA_COMP_NUCLEO.cognome,UTENZA_COMP_NUCLEO.nome,COMUNI_NAZIONI.NOME AS COMUNE,T_TIPO_INDIRIZZO.DESCRIZIONE AS TIPO_INDIRIZZO," _
                    & "TAB_FILIALI.NOME AS FILIALE,GetOperatoreAU(UTENZA_DICHIARAZIONI.ID) AS OPERATORE FROM UTENZA_DICHIARAZIONI, UTENZA_COMP_NUCLEO, COMUNI_NAZIONI, T_TIPO_INDIRIZZO, SISCOM_MI.TAB_FILIALI, SISCOM_MI.UNITA_IMMOBILIARI, " _
                    ...
                    & "AND COMUNI_NAZIONI.ID=UTENZA_DICHIARAZIONI.ID_LUOGO_RES_DNTE AND UTENZA_DICHIARAZIONI.FL_sospensione='1' AND UTENZA_COMP_NUCLEO.id_dichiarazione=UTENZA_DICHIARAZIONI.ID " _
