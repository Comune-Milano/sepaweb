...
Partial Class ANAUT_NuovoGruppoConv
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If Request.QueryString("ID") = "" Then
                Label26.Text = "Nuovo Gruppo - Ricerca da liste Convocabili - Step 1"
            Else
                Label26.Text = "Modifica Gruppo - Ricerca da liste Convocabili - Step 1"
            End If
            CaricaDatiAU()
            CaricaStrutture()
            CaricaTipoContratti()
    ...
    Private Function CaricaStrutture()
        Try
            ...
            PAR.cmd.CommandText = "SELECT DISTINCT filiale,ID_TAB_FILIALI FROM UTENZA_LISTE_CDETT WHERE id_sportello in (select id from utenza_sportelli where id_filiale in (select id from utenza_filiali where id_bando=" & cmbAU.SelectedItem.Value & ")) and ID_TAB_FILIALI IN (SELECT ID_STRUTTURA FROM UTENZA_FILIALI WHERE FL_ELIMINATO=0 AND ID_BANDO=" & cmbAU.SelectedItem.Value & ") AND  motivazione IS NULL AND id_lista NOT IN (SELECT ID from UTENZA_LISTE_CONV where STATO=0 AND id_au=" & cmbAU.SelectedItem.Value & ") order by filiale asc"
            Dim myReader As Oracle.DataAccess.Client.OracleDataReader = PAR.cmd.ExecuteReader()
            Do While myReader.Read
        ...
        '    chListStrutture.Items.Clear()
        '    PAR.cmd.CommandText = "SELECT UTENZA_FILIALI.*,TAB_FILIALI.NOME FROM SISCOM_MI.TAB_FILIALI,UTENZA_FILIALI WHERE TAB_FILIALI.ID=UTENZA_filiali.ID_STRUTTURA AND ID_BANDO=" & cmbAU.SelectedItem.Value & " AND TUTTO_PATRIMONIO=0 order by nome asc"
        '    Dim myReader As Oracle.DataAccess.Client.OracleDataReader = PAR.cmd.ExecuteReader()
        '    Do While myReader.Read
    ...
    Private Function CaricaSportelli()
        Try
            ...
            If s <> "" Then
                s = Mid(s, 1, Len(s) - 1)
                s = " WHERE ID_SPORTELLO IN (SELECT ID FROM UTENZA_SPORTELLI WHERE FL_ELIMINATO=0 AND ID_FILIALE IN (select ID from UTENZA_FILIALI WHERE ID_BANDO=" & cmbAU.SelectedItem.Value & ")) AND ID_TAB_FILIALI IN (" & s & ")"
        ...
        '    For i = 0 To chListStrutture.Items.Count - 1
        '        If chListStrutture.Items(i).Selected = True Then
        '            s = s & chListStrutture.SelectedItem.Value & ","
        '        End If
        '    Next
        '    If s <> "" Then
        '        s = Mid(s, 1, Len(s) - 1)
        '        s = " WHERE ID_FILIALE IN (" & s & ")"
