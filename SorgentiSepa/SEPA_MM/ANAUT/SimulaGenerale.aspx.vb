...
Partial Class ANAUT_SimulaGenerale
    ...
    Private Function Carica()
        Try
            ...
            par.cmd.CommandText = "SELECT ROWNUM,(CASE WHEN UTENZA_DICHIARAZIONI.ID_STATO=0 THEN 'DA COMPLETARE' WHEN UTENZA_DICHIARAZIONI.ID_STATO=1 THEN 'COMPLETA' WHEN UTENZA_DICHIARAZIONI.ID_STATO=2 THEN 'DA CANCELLARE' END) AS STATO_AU,NVL(EDIFICI.SCONTO_COSTO_BASE,-1000) AS SCONTO_COSTO_BASE,RAPPORTI_UTENZA.COD_TIPOLOGIA_CONTR_LOC AS TIPO_CONTRATTO,UTENZA_SPORTELLI.DESCRIZIONE AS SPORTELLO,convocazioni_au.*,RAPPORTI_UTENZA.COD_CONTRATTO,UTENZA_DICHIARAZIONI.PATR_SUPERATO,UTENZA_DICHIARAZIONI.PG,UTENZA_DICHIARAZIONI.ISEE,UTENZA_DICHIARAZIONI.ID_STATO,UTENZA_DICHIARAZIONI.id as idAU,UTENZA_DICHIARAZIONI.FL_SOSP_1,UTENZA_DICHIARAZIONI.FL_SOSP_2,UTENZA_DICHIARAZIONI.FL_SOSP_3,UTENZA_DICHIARAZIONI.FL_SOSP_4,UTENZA_DICHIARAZIONI.FL_SOSP_5,UTENZA_DICHIARAZIONI.FL_SOSP_7,unita_immobiliari.ID AS idunita,(SELECT (CASE WHEN TIPO=0 THEN 'INCOMPLETA' ELSE 'NON RISPONDENTE' END) FROM SISCOM_MI.DIFFIDE_LETTERE WHERE ID_CONTRATTO=CONVOCAZIONI_AU.ID_CONTRATTO AND ID_AU=" & IDAU & ") AS DIFFIDA,(SELECT TO_CHAR(TO_DATE(DATA_GENERAZIONE,'YYYYmmdd'),'DD/MM/YYYY') FROM SISCOM_MI.DIFFIDE_LETTERE WHERE ID_CONTRATTO=CONVOCAZIONI_AU.ID_CONTRATTO AND ID_AU=" & IDAU & ") AS DATA_GENERAZIONE_DIFFIDA , (SELECT distinct(NOME_GRUPPO) FROM UTENZA_GRUPPI_LAVORO,UTENZA_GRUPPI_DICHIARAZIONI WHERE UTENZA_GRUPPI_LAVORO.ID=UTENZA_GRUPPI_DICHIARAZIONI.ID_GRUPPO AND UTENZA_GRUPPI_DICHIARAZIONI.ID_DICHIARAZIONE=UTENZA_DICHIARAZIONI.ID) AS GRUPPO FROM siscom_mi.convocazioni_au,siscom_mi.unita_immobiliari,siscom_mi.unita_contrattuale,siscom_mi.rapporti_utenza,UTENZA_DICHIARAZIONI,UTENZA_SPORTELLI,SISCOM_MI.EDIFICI WHERE " & Session.Item("sx1") & " CONVOCAZIONI_AU.ID_STATO=2 AND EDIFICI.ID=UNITA_IMMOBILIARI.ID_EDIFICIO AND UTENZA_SPORTELLI.ID=CONVOCAZIONI_AU.ID_SPORTELLO AND rapporti_utenza.cod_contratto=UTENZA_DICHIARAZIONI.rapporto  AND UTENZA_DICHIARAZIONI.id_bando=" & IDAU & " AND (NOTE_WEB IS NULL OR NOTE_WEB<>'GENERATA_AUTOMATICAMENTE') AND NVL(FL_GENERAZ_AUTO,0)=0 AND rapporti_utenza.ID=convocazioni_au.id_contratto AND unita_contrattuale.id_unita_principale IS NULL AND unita_contrattuale.id_contratto=convocazioni_au.id_contratto AND unita_immobiliari.ID=unita_contrattuale.id_unita AND convocazioni_au.id_contratto IS NOT NULL AND data_app IS NOT NULL AND id_gruppo IN (SELECT ID FROM siscom_mi.convocazioni_au_gruppi WHERE id_au=" & IDAU & ") ORDER BY ROWNUM DESC"
            'AGGIUNGERE QUESTO PERZZO PER EVITARE CHE IN VERIFICA LE AU VENGANO RIPRESE DOPO L'APPLICAZIONE DEL CANONE
            '--	AND UTENZA_DICHIARAZIONI.ID NOT IN (SELECT id_dichiarazione FROM UTENZA_GRUPPI_DICHIARAZIONI WHERE UTENZA_GRUPPI_DICHIARAZIONI.applica_au=1 AND id_gruppo IN (SELECT ID FROM siscom_mi.convocazioni_au_gruppi WHERE id_au = " & IDAU & "))
            myReader = par.cmd.ExecuteReader()
            While myReader.Read
                ...
                If DAFARE = True Then
                    ...
                    If myReader1.Read Then
                        CANONE91 = ""
                        If myReader1("ID") = "200055000" Then
                            CANONE91 = ""
                        End If
                        ...
                        If AreaEconomica <> -1 Then
                            ...
                            ROW.Item("GRUPPO") = par.IfNull(myReader("GRUPPO"), "")
                        Else
                            ROW.Item("SPORTELLO") = myReader("SPORTELLO")
                            ROW.Item("COD_CONTRATTO") = myReader1("COD_CONTRATTO")
                            ROW.Item("TIPOLOGIA_CONTRATTO") = myReader("TIPO_CONTRATTO")
            ...
            If Not IsNothing(dt) Then
                If dt.Rows.Count > 0 Then
                    ...
                    With myExcelFile
                        ...
                        .WriteValue(CM.ExcelFile.ValueTypes.xlsText, CM.ExcelFile.CellFont.xlsFont1, CM.ExcelFile.CellAlignment.xlsCentreAlign, CM.ExcelFile.CellHiddenLocked.xlsNormal, 1, 75, "GRUPPO DI APPARTANENZA")
                        K = 2
                        For Each row In dt.Rows
                            ...
                            .WriteValue(CM.ExcelFile.ValueTypes.xlsText, CM.ExcelFile.CellFont.xlsFont0, CM.ExcelFile.CellAlignment.xlsLeftAlign, CM.ExcelFile.CellHiddenLocked.xlsNormal, K, 12, par.IfNull(dt.Rows(i).Item("NUM_COMP_100_CON"), ""))
                            .WriteValue(CM.ExcelFile.ValueTypes.xlsText, CM.ExcelFile.CellFont.xlsFont0, CM.ExcelFile.CellAlignment.xlsLeftAlign, CM.ExcelFile.CellHiddenLocked.xlsNormal, K, 13, par.IfNull(dt.Rows(i).Item("NUM_COMP_100"), ""))
                            ...
                            .WriteValue(CM.ExcelFile.ValueTypes.xlsText, CM.ExcelFile.CellFont.xlsFont0, CM.ExcelFile.CellAlignment.xlsLeftAlign, CM.ExcelFile.CellHiddenLocked.xlsNormal, K, 75, par.IfNull(dt.Rows(i).Item("GRUPPO"), ""))
                            i = i + 1
                            K = K + 1
                        Next
                ...
                'End If
                'Response.Write("<script>window.open('../FileTemp/" & FileCSV & ".zip','Export','');</script>")
