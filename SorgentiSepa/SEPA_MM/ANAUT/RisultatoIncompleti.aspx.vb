...
Partial Class ANAUT_RisultatoIncompleti
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            If UCase(sValoreDV) = "TRUE" Then
                sCriteri = sCriteri & " - Escludi i contratti con AU da verificare: SI"
            Else
                sCriteri = sCriteri & " - Escludi i contratti con AU da verificare: NO"
            End If
    ...
    Private Function Cerca()
        ...
        If sValoreFI <> "" Then
            ...
            If bTrovato1 = True Then
                bTrovato1 = True
                sStringaSqlD = sStringaSqlD & " AND NVL(ID_SPORTELLO,-1)=" & par.PulisciStrSql(sValore) & " "
            Else
                sStringaSqlD = sStringaSqlD & " NVL(ID_SPORTELLO,-1)=" & par.PulisciStrSql(sValore) & " "
            End If
        End If
        ...
        sStringaSQL1 = "SELECT (SELECT DESCRIZIONE FROM UTENZA_SPORTELLI WHERE ID in (SELECT ID_SPORTELLO FROM SISCOM_MI.CONVOCAZIONI_AU WHERE ID_CONTRATTO=RAPPORTI_UTENZA.ID AND ID_GRUPPO IN (SELECT ID FROM SISCOM_MI.CONVOCAZIONI_AU_GRUPPI WHERE ID_AU=" & sValoreBA & "))) AS FILIALE,RAPPORTI_UTENZA.ID AS IDC,UTENZA_DICHIARAZIONI.PG,RAPPORTI_UTENZA.COD_CONTRATTO,ANAGRAFICA.cognome,ANAGRAFICA.nome, RAPPORTI_UTENZA.cod_tipologia_contr_loc AS TIPOLOGIA," _
                     ...
                     & "FROM " _
                     & "UTENZA_DICHIARAZIONI, SISCOM_MI.RAPPORTI_UTENZA, SISCOM_MI.ANAGRAFICA, SISCOM_MI.SOGGETTI_CONTRATTUALI, siscom_mi.unita_contrattuale, siscom_mi.indirizzi, siscom_mi.unita_immobiliari " _
                     & "WHERE " _
                     & "UTENZA_DICHIARAZIONI.ID_BANDO = " & sValoreBA & " And UTENZA_DICHIARAZIONI.RAPPORTO = RAPPORTI_UTENZA.COD_CONTRATTO " _
                     & "AND UTENZA_DICHIARAZIONI.ID_STATO=0 AND NVL(UTENZA_DICHIARAZIONI.FL_SOSP_7,0)=1 AND NVL(UTENZA_DICHIARAZIONI.FL_SOSP_1,0)=0 AND NVL(UTENZA_DICHIARAZIONI.FL_SOSP_2,0)=0 AND NVL(UTENZA_DICHIARAZIONI.FL_SOSP_3,0)=0 AND NVL(UTENZA_DICHIARAZIONI.FL_SOSP_4,0)=0 AND NVL(UTENZA_DICHIARAZIONI.FL_SOSP_5,0)=0 AND NVL(UTENZA_DICHIARAZIONI.FL_SOSP_6,0)=0 " _
                     & "AND RAPPORTI_UTENZA.ID IN (SELECT ID_CONTRATTO FROM SISCOM_MI.CONVOCAZIONI_AU WHERE " & sStringaSqlD & " ID_CONTRATTO IS NOT NULL) " _
            ...
            If Not IsNothing(dt) Then
                If dt.Rows.Count > 0 Then
                    ...
                    With myExcelFile
                        ...
                        .WriteValue(CM.ExcelFile.ValueTypes.xlsText, CM.ExcelFile.CellFont.xlsFont1, CM.ExcelFile.CellAlignment.xlsCentreAlign, CM.ExcelFile.CellHiddenLocked.xlsNormal, 1, 12, "PG AU")
                        K = 2
                        For Each row In dt.Rows
                            ...
                            .WriteValue(CM.ExcelFile.ValueTypes.xlsText, CM.ExcelFile.CellFont.xlsFont0, CM.ExcelFile.CellAlignment.xlsLeftAlign, CM.ExcelFile.CellHiddenLocked.xlsNormal, K, 12, par.IfNull(dt.Rows(i).Item("PG"), ""))
    ...
    Protected Sub btnRicerca_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles btnRicerca.Click
        Response.Write("<script>document.location.href=""RicercaIncompleti.aspx""</script>")
    End Sub
        ...
        If txtDataStampa.Text <> "" And txtNumPG.Text <> "" And LBLIDmodello.Value <> "" Then
            Try
                ...
                If trovato = True And i > 0 Then
                    ...
                    If Simulazione = False Then
                        par.cmd.CommandText = "INSERT INTO UTENZA_FILE_DIFFIDE (ID,DATA_CREAZIONE,CRITERI,ID_SPORTELLO,DESCR_SPORTELLO,NOME_FILE,TIPO,ID_AU,PROTOCOLLO,DATA_ANNULLO,NOTE) VALUES (SEQ_FILE_DIFFIDE.NEXTVAL,'" & MIADATA & "','" & par.PulisciStrSql(sCriteri) & "'," & sValoreFI & ",'','" & BaseFile & ".zip',0," & sValoreBA & ",'" & par.PulisciStrSql(txtNumPG.Text) & "','','')"
                    ...
                    Do While myReader1.Read
                        ...
                        If Simulazione = False Then
                            par.cmd.CommandText = "Insert into SISCOM_MI.DIFFIDE_LETTERE (ID, ID_CONTRATTO, ID_AU, DATA_GENERAZIONE, TIPO) Values (SISCOM_MI.SEQ_DIFFIDE_LETTERE.NEXTVAL, " & par.IfNull(myReader1("IDC"), "") & ", " & sValoreBA & " , '" & Mid(MIADATA, 1, 8) & "',  0)"
                            par.cmd.ExecuteNonQuery()
                            ...
                            par.cmd.CommandText = "INSERT INTO SISCOM_MI.EVENTI_CONTRATTI (ID_CONTRATTO, ID_OPERATORE, DATA_ORA, COD_EVENTO, MOTIVAZIONE) VALUES (" & par.IfNull(myReader1("IDC"), "") & "," & Session.Item("ID_OPERATORE") & ", '" & MIADATA & "', 'F226', '" & par.PulisciStrSql(sNomeBandoAU) & "')"
                            par.cmd.ExecuteNonQuery()
                        End If
                        ...
                        If Simulazione = False Then
                            par.cmd.CommandText = "INSERT INTO UTENZA_FILE_DIFFIDE_DETT (ID_FILE_DIFFIDE,RAPPORTO,ID_DICHIARAZIONE) VALUES (" & NumeroFileDiffida & "," & par.IfNull(myReader1("IDC"), "") & "," & IndiceDichiarazione & ")"
                            par.cmd.ExecuteNonQuery()
                        End If

                        contenuto = Replace(contenuto, "$documentimancanti$", documentazionemancante)
                        contenuto = Replace(contenuto, "$tempisticaincomplete$", Tempistica)
                ...
                End If
                    ...
                    If Simulazione = False Then
                        Str = "<script  language=" & Chr(34) & "javascript" & Chr(34) & " type=" & Chr(34) & "text/javascript" & Chr(34) & ">document.getElementById('dvvvPre').style.visibility = 'hidden';document.location.href = 'ElencoDiffideIncomplete.aspx';</script>"
                    Else
                        'Str = "<script  language=" & Chr(34) & "javascript" & Chr(34) & " type=" & Chr(34) & "text/javascript" & Chr(34) & ">document.getElementById('dvvvPre').style.visibility = 'hidden';window.open('../ALLEGATI/ANAGRAFE_UTENZA/DIFFIDE/" & BaseFile & ".zip','Simulazione','');document.location.href = 'pagina_home.aspx';</script>"
