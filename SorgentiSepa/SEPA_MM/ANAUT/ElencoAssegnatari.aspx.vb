...
Partial Class ANAUT_ElencoAssegnatari
    ...
    Private Function CaricaDatiAU()
        Try
            ...
            If myReader.Read Then
                ...
                Tipo11 = PAR.IfNull(myReader("OA"), 0) 'OCCUPAZIONI ABUSIVE
                ...
                Label1.Text = "AU: " & PAR.IfNull(myReader("descrizione"), "") & "(" & ss & ") - Elenco Assegnatari Completo - " & Format(Now, "dd/MM/yyyy HH:mm") & " - " & Session.Item("operatore")
    ...
    Private Function CaricaDati()
        Try
            ...
            '& "tipo_cor,via_cor,civico_cor,luogo_cor,sigla_cor,cap_cor, " _
            '& "TAB_FILIALI.NOME AS FILIALE, " _
            '& "UTENZA_SPORTELLI.DESCRIZIONE AS SEDE, " _
            ...
            '& " " _
            '& "NVL(EC1.NUM_COMP,'0') AS NUM_COMP,NVL(EC1.NUM_COMP_66,'0') AS NUM_COMP_66,NVL(EC1.NUM_COMP_100,'0') AS NUM_COMP_100,NVL(EC1.NUM_COMP_100_CON,'0') AS NUM_COMP_100_CON,NVL(EC1.minori_15,'0') AS minori_15,NVL(EC1.maggiori_65,'0') AS maggiori_65,CASE WHEN EC1.REDD_PREV_DIP='1' THEN 'X'  ELSE '' END AS PREVALENTE_DIPENDENTE, " _
            ...
            '& "UTENZA_FILIALI.ID_BANDO=" & INDICEBANDO & " AND " _
            ...
            '& "bozza=0 AND (data_riconsegna IS NULL OR NVL(data_riconsegna,'29991231')>='" & DataInzio & "') ORDER BY FILIALE,SEDE,INTESTATARIO"
...
& "siscom_mi.complessi_immobiliari,siscom_mi.edifici,siscom_mi.unita_contrattuale,siscom_mi.unita_immobiliari " _
& "WHERE  " & ss _
& "EC1.ID_CONTRATTO=RAPPORTI_UTENZA.ID AND EC1.DATA_CALCOLO=(SELECT MAX(DATA_CALCOLO) FROM SISCOM_MI.CANONI_EC EC WHERE EC.ID_CONTRATTO=EC1.ID_CONTRATTO AND EC1.INIZIO_VALIDITA_CAN<='" & DataInzio & "') AND " _
& "CANONI_EC.ID_CONTRATTO=RAPPORTI_UTENZA.ID AND CANONI_EC.DATA_CALCOLO=(SELECT MAX(DATA_CALCOLO) FROM SISCOM_MI.CANONI_EC EC WHERE EC.ID_CONTRATTO=CANONI_EC.ID_CONTRATTO) AND " _
& "UTENZA_SPORTELLI.fl_eliminato=0 and UTENZA_SPORTELLI.ID=UTENZA_SPORTELLI_PATRIMONIO.ID_SPORTELLO AND " _
& "UTENZA_SPORTELLI_PATRIMONIO.ID_UNITA=UNITA_IMMOBILIARI.ID AND " _
& "UTENZA_FILIALI.ID=UTENZA_SPORTELLI.ID_FILIALE (+) AND " _
& "UTENZA_FILIALI.ID_BANDO=" & INDICEBANDO & " AND " _
& "TAB_FILIALI.ID=UTENZA_FILIALI.ID_STRUTTURA (+) AND " _
...
& "SOGGETTI_CONTRATTUALI.ID_CONTRATTO=RAPPORTI_UTENZA.ID AND  " _
& "ANAGRAFICA.ID=SOGGETTI_CONTRATTUALI.ID_ANAGRAFICA AND SOGGETTI_CONTRATTUALI.COD_TIPOLOGIA_OCCUPANTE = 'INTE' AND " _
& "bozza=0 and edifici.id<>1 and unita_immobiliari.COD_TIPOLOGIA='AL' AND UNITA_IMMOBILIARI.COD_TIPO_DISPONIBILITA<>'VEND' AND (data_riconsegna IS NULL OR NVL(data_riconsegna,'29991231')>='" & DataInzio & "') "
