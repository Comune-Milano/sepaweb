...
Partial Class ANAUT_com_detrazioni
    ...
    Private Sub ScriviDetrazioni()
        Try
            ...
            Dim tipoDetrazione As String = ""

            tipoDetrazione = cmbDetrazione.SelectedValue

            If tipoDetrazione = "-1" Then
                tipoDetrazione = "NULL"
            End If


            If txtOperazione.Text = "0" Then
                par.cmd.CommandText = "INSERT INTO UTENZA_COMP_DETRAZIONI (ID,ID_COMPONENTE,ID_TIPO,IMPORTO) VALUES (SEQ_UTENZA_COMP_DETRAZIONI.NEXTVAL," & cmbComponente.SelectedValue & "," & tipoDetrazione & "," & par.VirgoleInPunti(txtImporto.Text) & ")"
                par.cmd.ExecuteNonQuery()
            Else
                par.cmd.CommandText = "SELECT * FROM UTENZA_COMP_DETRAZIONI WHERE ID=" & txtRiga.Text
                Dim myReaderI As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                If myReaderI.Read Then
                    par.cmd.CommandText = "UPDATE UTENZA_COMP_DETRAZIONI SET ID_COMPONENTE=" & cmbComponente.SelectedValue & ",ID_TIPO=" & tipoDetrazione & ",IMPORTO=" & par.VirgoleInPunti(txtImporto.Text) & " WHERE ID=" & txtRiga.Text
                    par.cmd.ExecuteNonQuery()
                End If
                myReaderI.Close()
            End If
