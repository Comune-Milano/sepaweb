...
Partial Class ANAUT_com_patrimonio
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            txtPerc.Text = par.Elimina160(Request.QueryString("PRP"))
    ...
    Protected Sub btnSalva_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSalva.Click
        ...
        If CDbl(par.PuntiInVirgole(txtPerc.Text)) < 1 Then
            L4.Visible = True
            L4.Text = "(Valore minimo 1)"
        Else
            L4.Visible = False
        End If
    ...
    Private Sub ScriviPatrMobiliare()
        Try
            ...
            If txtOperazione.Text = "0" Then
                par.cmd.CommandText = "INSERT INTO UTENZA_COMP_PATR_MOB (ID, ID_COMPONENTE, COD_INTERMEDIARIO, INTERMEDIARIO, IMPORTO, ID_TIPO,PERC_PROPRIETA) VALUES (SEQ_UTENZA_COMP_PATR_MOB.NEXTVAL," & cmbComponente.SelectedValue & ",'" & txtABI.Text & "','" & par.PulisciStrSql(txtInter.Text) & "'," & par.VirgoleInPunti(txtImporto.Text) & "," & tipoPatrMob & "," & txtPerc.Text & ")"
                par.cmd.ExecuteNonQuery()
            Else
                par.cmd.CommandText = "SELECT * FROM UTENZA_COMP_PATR_MOB WHERE ID=" & txtRiga.Text
                Dim myReaderI As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                If myReaderI.Read Then
                    par.cmd.CommandText = "UPDATE UTENZA_COMP_PATR_MOB SET PERC_PROPRIETA=" & txtPerc.Text & ",ID_COMPONENTE=" & cmbComponente.SelectedValue & ",ID_TIPO=" & tipoPatrMob & ",IMPORTO=" & par.VirgoleInPunti(txtImporto.Text) & ",COD_INTERMEDIARIO='" & txtABI.Text & "',INTERMEDIARIO='" & par.PulisciStrSql(txtInter.Text) & "' WHERE ID=" & txtRiga.Text
                    par.cmd.ExecuteNonQuery()
                End If
                myReaderI.Close()
            End If
