...
Partial Class ANAUT_CreaAU_new
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsPostBack = False Then
            ...
            idcont.Value = Request.QueryString("IDC")
            IDCONVOCAZIONE.Value = Request.QueryString("IC")
            AUS.Value = Request.QueryString("AUS")
            t.Value = Request.QueryString("T")
            S392.Value = Request.QueryString("S392")
            If Verifica(idcont.Value) = True Then
                procedi.Value = "1"
                If S392.Value = "1" Then
                    CreaAU392()
                Else
                    PrendiUltima()
    ...
    Private Sub CreaAU392()
        Dim trovata As Boolean = VerificaEsiste(idcont.Value)
        If trovata = False Then
            CreaSchedaAU_392(idcont.Value, t.Value)
        End If
    End Sub

    Private Function CreaSchedaAU_392(ByVal idc As String, ByVal t As String)
        Try
            ...
            'par.cmd.CommandText = "SELECT ANNO_ISEE,ID,DATA_INIZIO FROM UTENZA_BANDI WHERE ID=6 order by id desc"
            ...
            If myReader.Read Then
                ...
                par.cmd.CommandText = "Insert into UTENZA_DICHIARAZIONI  (ID, PG, DATA_PG, LUOGO, DATA, NOTE, ID_STATO,ANNO_SIT_ECONOMICA,ID_BANDO,DATA_INIZIO_VAL,DATA_FINE_VAL,FL_AUSI,FL_392) values (seq_utenza_dichiarazioni.nextval, '" & Format(lValoreCorrente, "0000000000") & "','" & Format(Now, "yyyyMMdd") & "','Milano','" & Format(Now, "yyyyMMdd") & "','', 0," & sAnnoIsee & "," & lIndice_Bando & ",'" & sAnnoIsee + 2 & "0101','" & CInt(sAnnoIsee) + 3 & "1231'," & AUS.Value & ",1)"
                par.cmd.ExecuteNonQuery()
                ...
                par.cmd.CommandText = "update utenza_dichiarazioni set " _
                                  ...
                                  & "',TIPO_DOCUMENTO=" & par.IfEmpty(tipo_documento, "0") & " WHERE ID=" & lIdDichiarazioneAU
                par.cmd.ExecuteNonQuery()
                ...
                '                                        & par.PulisciStrSql(par.IfNull(myReader2("cod_fiscale"), "")) & "', '" _
                '                                        & par.PulisciStrSql(par.IfNull(myReader2("cognome"), "")) _
                '                                        & "', '" & par.PulisciStrSql(par.IfNull(myReader2("nome"), "")) & "', '" _
            ...
            strScript = "<script language='javascript'>var conf = window.confirm('Operazione effettuata con successo. Cliccare su OK per visualizzare la dichiarazione.');if (conf){window.open('DichAUnuova.aspx?ID=" & lIdDichiarazioneAU & "&CHIUDI=1&CH=1&ANNI=" & anni & "&N=1','Dettagli','');self.close();}" _
    ...
    Private Function PrendiUltima()
        Try
            ...
            Dim MESSAGGIO As String = "La situazione anagrafica più recente corrisponde a quella contrattuale."
            Dim MESSAGGIO1 As String = ""
            Dim MESSAGGIO2 As String = ""


            par.cmd.CommandText = "SELECT * FROM DOMANDE_BANDO_VSA,T_MOTIVO_DOMANDA_VSA,DICHIARAZIONI_VSA,SISCOM_MI.RAPPORTI_UTENZA WHERE DICHIARAZIONI_VSA.DATA_FINE_VAL<>'29991231' and RAPPORTI_UTENZA.ID=" & idcont.Value & " AND DICHIARAZIONI_VSA.ID=DOMANDE_BANDO_VSA.ID_DICHIARAZIONE AND DOMANDE_BANDO_VSA.ID_MOTIVO_DOMANDA = T_MOTIVO_DOMANDA_VSA.ID AND CONTRATTO_NUM=RAPPORTI_UTENZA.COD_CONTRATTO ORDER BY DICHIARAZIONI_VSA.DATA_FINE_VAL DESC"
            Dim myReader1 As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
            If myReader1.Read Then
                If par.IfNull(myReader1("fl_autorizzazione"), 0) = 1 Or par.IfNull(myReader1("id_stato_istanza"), 0) = 5 Then
                    ...
                    MESSAGGIO = "<br/>La situazione anagrafica e reddituale più recente corrisponde alla domanda di <b>""" & myReader1("DESCRIZIONE") & """</b> presentata in data " & par.FormattaData(myReader1("DATA_presentazione")) & ".<br/>Premendo il pulsante sotto 'Procedi' sarà importata la situazione ANAGRAFICA nella nuova scheda AU."
                Else
                    id_dichia = myReader1("ID_DICHIARAZIONE")
            ...
            Else
                ...
                If myReader11.Read Then
                    If par.IfNull(myReader11("fl_autorizzazione"), 0) = 1 Or par.IfNull(myReader11("id_stato_istanza"), 0) = 5 Then
                        id_dichia = myReader11("ID_DICHIARAZIONE")
                        data_Fine = par.IfNull(myReader11("DATA_FINE_VAL"), Format(Now, "yyyyMMdd"))
                        MESSAGGIO = "<br/>La situazione anagrafica e reddituale più recente corrisponde alla domanda di <b>""" & myReader11("DESCRIZIONE") & """</b> presentata in data " & par.FormattaData(myReader11("DATA_presentazione")) & ".<br/>Premendo il pulsante sotto 'Procedi' sarà importata la situazione ANAGRAFICA nella nuova scheda AU."
                    Else
                        id_dichia = myReader11("ID_DICHIARAZIONE")
                        MESSAGGIO2 = "Attenzione, esistono DATI ANAGRAFICI da verificare relativi alla domanda di <a href='javascript:void(0)' onclick=" & Chr(34) & "window.open('../VSA/NuovaDichiarazioneVSA/DichAUnuova.aspx?ID=" & id_dichia & "&CH=2&LE=1','DichVSA','top=250,left=650,toolbar=no, location=no,status=no,menubar=no,scrollbars=yes,resizable=yes');" & Chr(34) & ">" & myReader11("DESCRIZIONE") & "</a>" & " del " & par.FormattaData(par.IfNull(myReader11("DATA_presentazione"), "")) & ".<br />Si consiglia di porre maggiore accuratezza nella compilazione della scheda.<br/>"
                        imgAlert.Visible = True
                    End If
                End If
                myReader11.Close()
            ...
            If myReader2.Read Then
                ...
                MESSAGGIO1 = "<br/>La situazione anagrafica e reddituale più recente corrisponde a: <b>""" & myReader2("NOME_BANDO") & """</b> (redditi " & myReader2("ANNO_ISEE") & ").<br/>Premendo il pulsante sotto 'Procedi' sarà importata la situazione ANAGRAFICA nella nuova scheda AU."
            End If
            myReader2.Close()
    ...
    Private Function Intestatari1(ByVal idc As Long)
        Try
            ...
            par.cmd.CommandText = "SELECT * FROM COMP_NUCLEO_VSA WHERE cod_fiscale = '" & codFiscaleIntest.ToUpper & "' and id_dichiarazione=" & iddich.Value
            myReader = par.cmd.ExecuteReader()
            If myReader.Read Then
                For Each R As ListItem In ListaInt.Items
                    If R.Value = par.IfNull(myReader("ID"), 0) Then
                        R.Selected = True
                        Trovato = True
                        Exit For
                    End If
                Next
            Else
                Label1.Text = "ATTENZIONE..SELEZIONARE L'INTESTATARIO DALLA LISTA"
    ...
    Private Function Intestatari2(ByVal idc As Long)
        Try
            ...
            codFiscaleIntest = RicavaCFIntestatario(idcont.Value)
            Dim Trovato As Boolean = False
            par.cmd.CommandText = "SELECT * FROM UTENZA_COMP_NUCLEO WHERE cod_fiscale = '" & codFiscaleIntest.ToUpper & "' and id_dichiarazione=" & iddich.Value
            myReader = par.cmd.ExecuteReader()
            If myReader.Read Then
                For Each R As ListItem In ListaInt.Items
                    If R.Value = par.IfNull(myReader("ID"), 0) Then
    ...
    Private Function Intestatari(ByVal idc As String, ByVal t As String)
        Try
            ...
            Dim Trovato As Boolean = False
            par.cmd.CommandText = "SELECT ANAGRAFICA.ID FROM SISCOM_MI.ANAGRAFICA,SISCOM_MI.SOGGETTI_CONTRATTUALI WHERE soggetti_contrattuali.cod_tipologia_occupante='INTE' AND ANAGRAFICA.ID=SOGGETTI_CONTRATTUALI.ID_ANAGRAFICA AND SOGGETTI_CONTRATTUALI.ID_CONTRATTO=" & idc
            myReader = par.cmd.ExecuteReader()
            If myReader.Read Then
                For Each R As ListItem In ListaInt.Items
                    If R.Value = par.IfNull(myReader("ID"), 0) Then
    ...
    Private Function CreaAUAU()
        ...
        Try
            ...
            Else
                IDD = iddich.Value
                par.cmd.CommandText = "SELECT * FROM COMUNI_NAZIONI WHERE COD = '" & Mid(codFiscaleIntest, 12, 4) & "'"
                Dim myReaderCom As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader
                If myReaderCom.Read Then
                    idLuogoNascita = par.IfNull(myReaderCom("ID"), "")
                End If
                myReaderCom.Close()
                If idLuogoNascita = 0 Then
                    par.cmd.CommandText = "SELECT * FROM COMUNI_NAZIONI WHERE COD = (SELECT COD_COMUNE_NASCITA FROM SISCOM_MI.ANAGRAFICA,SISCOM_MI.SOGGETTI_CONTRATTUALI WHERE ANAGRAFICA.ID=SOGGETTI_CONTRATTUALI.ID_ANAGRAFICA AND SOGGETTI_CONTRATTUALI.COD_TIPOLOGIA_OCCUPANTE='INTE' AND SOGGETTI_CONTRATTUALI.ID_CONTRATTO=" & idcont.Value & ")"
                    Dim myReaderCom1 As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader
                    If myReaderCom1.Read Then
                        idLuogoNascita = par.IfNull(myReaderCom1("ID"), "")
                    End If
                    myReaderCom1.Close()
                End If

                id_intest = ListaInt.SelectedValue
            End If



            If ListaInt.SelectedValue <> id_intest Then
                par.cmd.CommandText = "SELECT * FROM utenza_COMP_NUCLEO WHERE ID=" & ListaInt.SelectedValue
                Dim myReaderUC As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                If myReaderUC.Read Then
                    dichiarante = "Dichiarante: " & par.IfNull(myReaderUC("cognome"), "") & " " & par.IfNull(myReaderUC("nome"), "")
                End If
                myReaderUC.Close()
            End If
            ...
            If myReader.Read Then
                ...
                par.cmd.CommandText = "select id_unita,COD_UNITA_IMMOBILIARE from siscom_mi.unita_contrattuale where id_unita_principale is null and id_contratto=" & idcont.Value
                Dim myReaderD As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                If myReaderD.Read Then
                    COD_ALLOGGIO = par.IfNull(myReaderD("COD_UNITA_IMMOBILIARE"), "")
                    par.cmd.CommandText = "SELECT SCALE_EDIFICI.DESCRIZIONE AS SCALA,UNITA_IMMOBILIARI.*,IDENTIFICATIVI_CATASTALI.FOGLIO,IDENTIFICATIVI_CATASTALI.SEZIONE," _
                ...
                par.cmd.CommandText = "UPDATE UTENZA_DICHIARAZIONI SET ID_LUOGO_NAS_DNTE=" & idLuogoNascita _
                                    ...
                                    & "',NOTE_WEB='" & par.PulisciStrSql(dichiarante) _
                ...
                'par.cmd.ExecuteNonQuery()
                par.cmd.CommandText = "insert into siscom_mi.AU_CONTRATTI (id_au,ID_CONTRATTO) values (" & lIdDichiarazioneAU & "," & idcont.Value & ")"
                par.cmd.ExecuteNonQuery()
            End If
            myReader.Close()
            ...
            While myReader.Read '-------------- INTESTATARIO --------------'
                ...
                par.cmd.CommandText = "INSERT INTO UTENZA_COMP_NUCLEO (ID,ID_DICHIARAZIONE,PROGR,COD_FISCALE,COGNOME,NOME,SESSO,DATA_NASCITA,GRADO_PARENTELA,PERC_INVAL,INDENNITA_ACC,TIPO_INVAL,NATURA_INVAL) " _
                    ...
                    & par.VirgoleInPunti(par.IfNull(myReader("PERC_INVAL"), 0)) & ",'" & par.IfNull(myReader("INDENNITA_ACC"), "") & "','" & par.IfNull(myReader("TIPO_INVAL"), "") & "','" & par.IfNull(myReader("NATURA_INVAL"), "") & "')"
                par.cmd.ExecuteNonQuery()
                ...
                While myReader2.Read
                    par.cmd.CommandText = "INSERT INTO UTENZA_COMP_PATR_IMMOB (ID,ID_COMPONENTE,ID_TIPO,PERC_PATR_IMMOBILIARE,VALORE,MUTUO,F_RESIDENZA,CAT_CATASTALE,COMUNE,N_VANI,SUP_UTILE,FL_70KM,PIENA_PROPRIETA,INDIRIZZO,CIVICO,REND_CATAST_DOMINICALE,ID_TIPO_PROPRIETA,FL_VENDUTO) " _
                        ...
                        & "','" & par.IfNull(myReader2("CIVICO"), "") & "'," & par.IfNull(myReader2("REND_CATAST_DOMINICALE"), "NULL") & "," & par.IfNull(myReader2("ID_TIPO_PROPRIETA"), "NULL") & "," & par.IfNull(myReader2("FL_VENDUTO"), "NULL") & ")"
                    par.cmd.ExecuteNonQuery()
                End While
                myReader2.Close()
            ...
            par.cmd.CommandText = "SELECT * FROM UTENZA_COMP_NUCLEO WHERE ID_DICHIARAZIONE = " & iddich.Value & " AND ID<>" & id_intest & " ORDER BY PROGR ASC"
            myReader = par.cmd.ExecuteReader()
    ...
    Private Function CreaAUVSA()
        ...
        Try
            ...
            par.cmd.CommandText = "SELECT * FROM COMP_NUCLEO_VSA WHERE COD_FISCALE = '" & codFiscaleIntest & "' and id_dichiarazione=" & iddich.Value
            myReader = par.cmd.ExecuteReader()
            If myReader.Read Then
                id_intest = myReader("ID") 'ottengo l'id dell'intestatario che intende presentare la domanda
            End If
            ...
            If ListaInt.SelectedValue <> id_intest Then
                par.cmd.CommandText = "SELECT * FROM COMP_NUCLEO_VSA WHERE ID=" & ListaInt.SelectedValue
                Dim myReaderUC As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                If myReaderUC.Read Then
                    dichiarante = "Dichiarante: " & par.IfNull(myReaderUC("cognome"), "") & " " & par.IfNull(myReaderUC("nome"), "")
                End If
            ...
            While myReader.Read '-------------- INTESTATARIO --------------'
                ...
                par.cmd.CommandText = "INSERT INTO UTENZA_COMP_NUCLEO (ID,ID_DICHIARAZIONE,PROGR,COD_FISCALE,COGNOME,NOME,SESSO,DATA_NASCITA,GRADO_PARENTELA,PERC_INVAL,INDENNITA_ACC,TIPO_INVAL,NATURA_INVAL) " _
                    & "VALUES (" & new_id_compon & "," & lIdDichiarazioneAU & ",0,'" & UCase(par.IfNull(myReader("COD_FISCALE"), "")) & "','" & par.PulisciStrSql(UCase(par.IfNull(myReader("COGNOME"), ""))) & "'," _
                    & "'" & par.PulisciStrSql(UCase(par.IfNull(myReader("NOME"), ""))) & "','" & UCase(par.IfNull(myReader("SESSO"), "")) & "','" & par.IfNull(myReader("DATA_NASCITA"), "") & "','" & par.IfNull(myReader("GRADO_PARENTELA"), "") & "'," _
                    & par.VirgoleInPunti(par.IfNull(myReader("PERC_INVAL"), 0)) & ",'" & par.IfNull(myReader("INDENNITA_ACC"), "") & "','" & par.IfNull(myReader("TIPO_INVAL"), "") & "','" & par.IfNull(myReader("NATURA_INVAL"), "") & "')"
                par.cmd.ExecuteNonQuery()
                ...
                Dim myReader2 As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
                While myReader2.Read
                    par.cmd.CommandText = "INSERT INTO UTENZA_COMP_PATR_IMMOB (ID,ID_COMPONENTE,ID_TIPO,PERC_PATR_IMMOBILIARE,VALORE,MUTUO,F_RESIDENZA,CAT_CATASTALE,COMUNE,N_VANI,SUP_UTILE,FL_70KM,PIENA_PROPRIETA,REND_CATAST_DOMINICALE,ID_TIPO_PROPRIETA,FL_VENDUTO) " _
                        & "VALUES (SEQ_UTENZA_COMP_PATR_IMMOB.NEXTVAL," & new_id_compon & "," & par.IfNull(myReader2("ID_TIPO"), "") & ",'" & par.IfNull(myReader2("PERC_PATR_IMMOBILIARE"), "") & "'," _
                        ...
                        & par.IfNull(myReader2("FL_70KM"), "") & "'," & par.IfNull(myReader2("PIENA_PROPRIETA"), "NULL") & "," & par.IfNull(myReader2("REND_CATAST_DOMINICALE"), "NULL") & "," & par.IfNull(myReader2("ID_TIPO_PROPRIETA"), "NULL") & ",0)"
                    par.cmd.ExecuteNonQuery()
                End While
                myReader2.Close()
            ...
            par.cmd.CommandText = "SELECT * FROM COMP_NUCLEO_VSA WHERE ID_DICHIARAZIONE = " & iddich.Value & " AND ID<>" & id_intest & " ORDER BY PROGR ASC"
            myReader = par.cmd.ExecuteReader()
            While myReader.Read '-------------- CICLO SUI COMPONENTI DEL NUCLEO FAMILIARE --------------'
                id_compon = myReader("ID")
    ...
    Protected Sub btnProcedi_Click(sender As Object, e As System.EventArgs) Handles btnProcedi.Click
        If ListaInt.SelectedIndex <> -1 Then
            ...
            If trovata = False Then
                ...
                End Select
            Else
                Response.Write("<script>alert('Attenzione...Non è possibile procedere in quanto risulta essere presente una scheda AU!')</script>")
            End If
        Else
            'Label2.Visible = True
    ...
    Private Function VerificaEsiste(ByVal idc As String) As Boolean
        Try
            VerificaEsiste = False
            Dim CODICE As String = ""
            Dim lIndice_Bando As String = ""
            ...
            Dim myReader As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
            If myReader.Read() Then
                CODICE = par.IfNull(myReader("COD_CONTRATTO"), "XX")
            End If
            myReader.Close()
            ...
            par.cmd.CommandText = "SELECT id FROM UTENZA_DICHIARAZIONI WHERE ID_BANDO=" & lIndice_Bando & " AND RAPPORTO='" & CODICE & "'"
            myReader = par.cmd.ExecuteReader()
