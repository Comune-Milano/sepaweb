...
Partial Class ANAUT_SimulazioneApplicazione
    ...
    Private Function Carica()
        Try
            ...
            Dim ElencoGruppi As String = Session.Item("ElencoSimulazione")
    ...
    Private Function EliminaIncomplete()
        If Modificato.Value = "1" Then
            Try
                ...
                For Each row In dt.Rows
                    If par.IfNull(dt.Rows(I).Item("STATO_AU"), "-1") = "INCOMPLETA" Then
                ...
                If j > 0 Then
                    Response.Write("<script>alert('Operazione Effettuata! Sono state eliminate " & j & " dichiarazioni dal gruppo. Potranno essere inserite in altri gruppi.');</script>")
                    BindGrid()
                Else
                    Response.Write("<script>alert('Operazione Effettuata! Nessuna dichiarazioni è stata eliminata dal gruppo.');</script>")
                End If
    ...
    Private Function SpostaRegistro()
        Try
            ...
            par.cmd.CommandText = "SELECT ID FROM UTENZA_GRUPPI_LAVORO WHERE FL_REGISTRO=1 AND ID_BANDO_AU=" & lIdAU
            Dim myReader As Oracle.DataAccess.Client.OracleDataReader = par.cmd.ExecuteReader()
            If myReader.Read() Then
    ...
    Protected Sub btnElimina1_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles btnElimina1.Click
        EliminaIncomplete()
    End Sub
